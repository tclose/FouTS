...
parser.add_argument('--samples_per_acs', type=float, default=100, help='The number of strands to use per ACS when renormalising the tractlets at the start of each major iteration (default: %(default)s)')
...
parser.add_argument('--min_length_samples', type=float, default=15, help='The number of strands to use per ACS when renormalising the tractlets at the start of each major iteration (default: %(default)s)')
parser.add_argument('--min_width_samples', type=float, default=4, help='The number of length samples to use per ACS when renormalising the tractlets at the start of each major iteration (default: %(default)s)')
...
init_fibres {work_dir}/init_location_{i}.tct -degree {args.degree} -num_fibres 1 -width_epsilon {args.width_epsilon} \
...
combine_fibres {work_dir}/output/init_0.tct {work_dir}/init_location_{i}.tct {work_dir}/tmp.tct
mv {work_dir}/tmp.tct {work_dir}/output/init_0.tct
mv {work_dir}/tmp.tctx {work_dir}/output/init_0.tct

""".format(work_dir=work_dir, i=i, init_location=init_location, init_extent=args.init_extent)
            # Set required properties and normalise the density of the initial tracts
            cmd_line += """
# Set required properties
set_property {work_dir}/output/init_0.tct -set length_epsilon {args.length_epsilon} \
...
calculate_num_samples --samples_per_acs {args.samples_per_acs} --samples_per_length {args.samples_per_length} \
--strategy max {work_dir}/init_{norm_count}.tct {work_dir}/num_samples --min_length_samples {args.min_length_samples} \
--min_width_samples {args.min_width_samples}
                
# Run metropolis
metropolis {dataset_path} {work_dir}/output/init_{norm_count}.tct {work_dir}/output/samples_{norm_count}.tst \
-like_snr {like_snr} -exp_interp_extent {args.assumed_interp_extent} -walk_step_scale {args.step_scale} \
-num_iter {args.num_iterations} -sample_period {args.sample_period} \
-diff_encodings_location {work_dir}/params/diffusion/encoding_60.b -seed {seed} \
...
-exp_type {args.interp_type} {response_str} -exp_b0 `cat {b0_path}` -diff_warn -walk_step_location \
