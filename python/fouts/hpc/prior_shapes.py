...
"""
 ...
 and submit them to the job que on a sun
 grid engine
...
parser.add_argument('--prior_acs', nargs=2, default=(1000, 0.1), type=float,
                    help=('The standard deviation and mean of the ACS '
                          '(Apparent Connection Strength) prior.'))
parser.add_argument('--img_snr', default=5, type=float,
...
parser.add_argument('--dry_run', action='store_true',
                    help=('Only perform a dry run (create jobscript then '
                          'quit)'))
...
parser.add_argument('--que_name', type=str, default='short',
                    help=('The the que to submit the job to '
                          '(default: %(default)s)'))
parser.add_argument('--prior_freq', default=[15.0], type=float, nargs='+',
                    help=('The scaling of the frequency prior'))
...
parser.add_argument('--prior_density_low', default=[0.01], type=float,
...
parser.add_argument('--combo', action='store_true',
                    help=('Instead of treating each ranging parameter sequence'
                          ...
                          'combinations of the provided parameters are '
                          'tested.'))
...
ranging_param_names = ['prior_freq', 'prior_aux_freq', 'prior_density_low',
                       'prior_density_high', 'prior_hook']
ranging_params = hpc.combo_params(args, ranging_param_names, args.combo)
for i in xrange(args.num_runs):
    for (prior_freq, prior_aux_freq, prior_density_low,
            prior_density_high, prior_hook) in zip(*ranging_params):
        required_dirs = [
            os.path.join(
...
-prior_hook {prior_hook} 100 15
    """.format(work_dir=work_dir, dim=args.img_dims, degree=args.degree,
               acs_mean=args.prior_acs[1], img_snr=args.img_snr,
               like_snr=args.like_snr, acs_stddev=args.prior_acs[0],
               prior_freq=prior_freq, prior_aux_freq=prior_aux_freq,
