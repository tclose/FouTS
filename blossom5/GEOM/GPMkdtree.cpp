...
#include <stdlib.h>
#include <string.h>
#include "blossom5/GEOMGPMkdtree.h"
#include "blossom5/timer.h"
...
template<typename Type> inline void sort(Type* array, int array_skip, int* mapping, int N) {
    ...
    for (step = 2; step < N; step *= 2) {
        ...
        while (1) {
            ...
            pSrc2 = pSrc1End;
            pSrc2End = pSrc2 + step;
            if (pSrc2End > mappingSrc + N)
                pSrc2End = mappingSrc + N;
            while (1) {
    ...
    if (mappingSrc != mapping)
        memcpy(mapping, mappingSrc, N * sizeof(int));
}
...
inline void Neighbors::Add(PointId p, double dist) {
    ...
    if (num < K) {
        ...
        while (k > 0) {
            ...
            if (dist_array[k] <= dist_array[k_parent])
                break;
            Swap(k, k_parent);
            k = k_parent;
        }
    } else {
        if (dist_array[0] <= dist)
            return;
        array[0] = p;
        dist_array[0] = dist;
        k = 0;
        while (1) {
            ...
            if (k_child >= K)
                break;
            if (k_child + 1 < K && dist_array[k_child + 1] > dist_array[k_child])
                k_child++;
            if (dist_array[k] >= dist_array[k_child])
                break;
            Swap(k, k_child);
            k = k_child;
        }
...
GPMKDTree::GPMKDTree(int _D, int _point_num, REAL* coords, GeomPerfectMatching* _GPM)
    ...
    for (d = 0; d < D; d++) {
        ...
        if (d == DIM)
            sum_max = GPM->sums[ptr[point_num - 1]];
        ptr += point_num;
    }
...
#define UPDATE_NODE_PTR(ptr) ptr = (Node*)((char*)ptr + ((char*)nodes-(char*)nodes_old))
            ...
            if (first_unprocessed)
                UPDATE_NODE_PTR(first_unprocessed);
            for (k = 0; k < node_num; k++) {
                if (nodes[k].parent)
                    UPDATE_NODE_PTR(nodes[k].parent);
                if (nodes[k].d >= 0 && nodes[k].first_child)
                    UPDATE_NODE_PTR(nodes[k].first_child);
            }
            for (k = 0; k < point_num; k++)
                if (rev_mapping[k])
                    UPDATE_NODE_PTR(rev_mapping[k]);
        }
        ...
        int num_min = 1;
        if (num_min < num0 / FRACTION)
            num_min = num0 / FRACTION;
        int num_max = num0 - 1;
        if (num_max > (FRACTION - 1) * num0 / FRACTION)
            num_max = (FRACTION - 1) * num0 / FRACTION;
        int num_max_DIM = num0 - 1;
        
        if (D > DIM && (!i->parent || !i->parent->parent || !i->parent->parent->parent))
            d0 = D - 1;
        else {
            d0 = -1;
            REAL diff_max = 0;
            for (d = 0; d < D; d++) {
                ...
                c_max = coords_array[d][ptr[start + ((d < DIM) ? num_max : num_max_DIM)]
                        * skip_array[d]];
        ...
        for (num = num_min; num < num_max; num++) {
            if (coords_array[d0][ptr[start + num] * skip_array[d0]] > split)
                break;
        }
...
void GPMKDTree::AddNegativeEdges(PointId p0, PerfectMatching* pm) {
    ...
    for (k++; k < -i->d; k++) {
        ...
        if (2 * len - GPM->sums[p] < GPM->sums[p0]) {
            ...
            if (pm->AddNewEdge(p0, p, len, true) >= 0)
                GPM->AddInitialEdge(p0, p);
