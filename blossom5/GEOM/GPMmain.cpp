...
#include <stdlib.h>
#include <string.h>
#include "blossom5/GEOMGeomPerfectMatching.h"
...
GeomPerfectMatching::REAL GeomPerfectMatching::Solve() {
    ...
    if (gpm_options.init_Delaunay)
        InitDelaunay();
    if (gpm_options.init_KNN > 0)
        InitKNN(gpm_options.init_KNN);
    if (gpm_options.init_greedy)
        CompleteInitialMatching();
    ...
    for (iter = 0; iter_max <= 0 || iter < iter_max; iter++) {
        if (pm) {
            ...
            for (p = 0; p < node_num; p++) {
                ...
                if (((REAL) 1 / 2) == 0 && ((PerfectMatching::REAL) 1 / 2) != 0)
                    sums[p] = (REAL) ceil((double) s);
                else
                    sums[p] = (REAL) s;
            }
            if (options.verbose) {
            ...
            for (p = 0; p < node_num; p++) {
                ...
                for (e = nodes[p].first[0]; e; e = e->next[0])
                    nodes[e->head[0]].is_marked = 1;
                for (e = nodes[p].first[1]; e; e = e->next[1])
                    nodes[e->head[1]].is_marked = 1;
                kd_tree->AddNegativeEdges(p, pm);
                for (e = nodes[p].first[0]; e; e = e->next[0])
                    nodes[e->head[0]].is_marked = 0;
                for (e = nodes[p].first[1]; e; e = e->next[1])
                    nodes[e->head[1]].is_marked = 0;
            }
            delete kd_tree;
