...
ARGUMENTS = {
  ...
  Argument ("output", "The merged strands file in BTS format").type_file(),
  Argument()
};
...
template <typename T> void combine_fibres(const std::vector<std::string>& input_locations, const std::string& output_location, MR::ProgressBar& progress_bar);

template <typename T> void add_fibres(typename T::Writer& writer, const std::string& location, MR::ProgressBar& progress_bar);
...
template <typename T> void combine_fibres(const std::vector<std::string>& input_locations, const std::string& output_location, MR::ProgressBar& progress_bar) {
  ...
  for (typename std::vector<std::string>::const_iterator input_it = input_locations.begin(); input_it != input_locations.end(); ++input_it)
...
template <typename T> void  add_fibres (typename T::Writer& writer, const std::string& location, MR::ProgressBar& progress_bar) {
  ...
  if (File::is_dir(location)) {
    ...
    std::vector<std::string> filenames = File::list_filenames(location, "", T::FILE_EXTENSION);
