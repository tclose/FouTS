    ...
    case CROSSING: {
      ...
      for (size_t layer_i = 0; layer_i < num_layers/2; layer_i++) {
        ...
        for (size_t col_i = 0; col_i < num_layers; col_i++) {
          ...
          straight_offset[0][2] += 2.0 * strand_diameter * (double) layer_i + layer_offset; 

          curved_offset[0][1] += strand_diameter * (double) col_i + col_offset;
          curved_offset[0][2] += 2.0 * strand_diameter * (double) layer_i + layer_offset + strand_diameter;
      ...
      for (size_t layer_i = 0; layer_i < num_layers/2; layer_i++) {
        ...
        for (size_t col_i = 0; col_i < num_layers; col_i++) {
          ...
          curve1_offset[0][0] += (strand_diameter * (double) col_i + col_offset) * sin(rad_angle);
          curve1_offset[0][1] += -(strand_diameter * (double) col_i + col_offset) * cos(rad_angle);
                    
          curve1_offset[0][2] += 2.0 * strand_diameter * (double) layer_i + layer_offset + strand_diameter;

       
          curve2_offset[0][0] += (strand_diameter * (double) col_i + col_offset) * sin(rad_angle);
          curve2_offset[0][1] += (strand_diameter * (double) col_i + col_offset) * cos(rad_angle);  
                  
          curve2_offset[0][2] += 2.0 * strand_diameter * (double) layer_i + layer_offset; 
