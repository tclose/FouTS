...
EXECUTE {
  ...
  for (std::vector<ParsedArgument>::iterator arg = argument.begin(); arg != argument.end(); ++arg) {
    ...
    if (selected_properties.size()) {
      ...
      for (std::vector<std::string>::iterator select_it = selected_properties.begin(); select_it != selected_properties.end(); ++select_it)
      ...
      for (std::map<std::string,std::string>::map::iterator i = extended_props.begin(); i != extended_props.end(); ++i) {
