...
BTS::Fibre::Track::Set trim_tcks(BTS::Fibre::Track::Set& input_tcks, double sphere_r, double length_reject_threshold, bool trim_to_cube);

BTS::Fibre::Tractlet::Set  trim_tractlets(BTS::Fibre::Tractlet::Set& input_tractlets, double distance, double length_reject_threshold, bool trim_to_cube, size_t num_length_sections, size_t num_width_sections);
...
EXECUTE {
  ...
  if (File::has_or_txt_extension<BTS::Fibre::Strand>(input_location) || File::has_or_txt_extension<BTS::Fibre::Track>(input_location)) {

    BTS::Fibre::Track::Set input(input_location, num_length_sections);

    MR::ProgressBar progress_bar("Trimming strands/tracks...");

    BTS::Fibre::Track::Set output = trim_tcks(input, distance, length_reject_threshold, cube);
    ...
    BTS::Fibre::Tractlet::Set output = trim_tractlets(input, distance, length_reject_threshold, cube, num_length_sections, num_width_sections);
    ...
    BTS::Fibre::Strand::Set::Writer writer(output_location, reader, reader.extend_prop_keys(), elem_header, props);
    ...
    while (reader.next(strands)) {
      ...
      BTS::Fibre::Track::Set trimmed_tcks = trim_tcks(tcks, distance, length_reject_threshold, cube);
    ...
    BTS::Fibre::Track::Set::Writer writer(output_location, reader, reader.extend_prop_keys(), reader.extend_elem_prop_keys(), reader.get_extend_props());
    ...
    BTS::Fibre::Tractlet::Set::Writer writer(output_location, reader, reader.extend_prop_keys(), elem_header, props);

    BTS::Fibre::Tractlet::Set tractlets;

    while (reader.next(tractlets)) {
      writer.append(trim_tractlets(tractlets, distance, length_reject_threshold, cube, num_length_sections, num_width_sections));
      ++progress_bar;
    }


  } else
    throw Exception ("Unrecognised extension for input file '" + File::extension(input_location) + "'.");
...
BTS::Fibre::Track::Set  trim_tcks(BTS::Fibre::Track::Set& input_tcks, double distance, double length_reject_threshold, bool trim_to_cube) {
...
BTS::Fibre::Tractlet::Set  trim_tractlets(BTS::Fibre::Tractlet::Set& input_tractlets, double distance, double length_reject_threshold, bool trim_to_cube, size_t num_length_sections, size_t num_width_sections) {
  ...
  BTS::Fibre::Track::Set input_tcks = input_tractlets.to_tracks(num_length_sections, num_width_sections);

  BTS::Fibre::Track::Set trimmed_tcks = trim_tcks(input_tcks,  distance, length_reject_threshold, trim_to_cube);
