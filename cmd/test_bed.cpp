...
const char* WALKER_STEP_LOCATION_DEFAULT =
        "/home/tclose/data/mcmc/metropolis/params/walker/scaling.sta";
const char* MOMENTUM_STEP_LOCATION_DEFAULT =
        "/home/tclose/data/mcmc/hamiltonian/params/momentum/scaling.sta";
const char* AXIS_SCALES_DEFAULT_LOCATION_DEFAULT =
        "/home/tclose/data/prob/test/gaussian/axis_scales.sta";
OPTIONS= {
...
//                  );
                ...
                MCMC::hamiltonian<MCMC::State, Prob::Test::Landscape, Prob::Test::Landscape>(state,
                        test_peaks, test_peaks, momentum, samples_location, run_properties,
                        num_samples, num_leapfrog_steps, rand_gen, true, save_iterations,
                        suppress_print);
                
            } else
                throw Exception(
                        "Unrecognised value for '-algorithm' option \"" + algorithm_type
                        + "\", can be either \"metropolis\" or \"hamiltonian\".");
...
//                );
                ...
                MCMC::hamiltonian<MCMC::State, Prob::Test::Gaussian, Prob::Test::Gaussian>(state,
                        test_gaussian, test_gaussian, momentum, samples_location, run_properties,
                        num_samples, num_leapfrog_steps, rand_gen, true, save_iterations,
                        suppress_print);
                
            } else if (!strcmp(algorithm_type.c_str(), "riemannian")) {
                
                MCMC::Proposal::Momentum::Weighted::NonSeparable nonseparable_momentum =
                        MCMC::Proposal::Momentum::Weighted::NonSeparable::factory(state,
                                momen_step_scale, momen_step_location, proposal_distribution,
                                num_newton_steps);
                
                Prob::Uniform uniform;
                
                MCMC::riemannian<MCMC::State, Prob::Test::Gaussian, Prob::Uniform>(state,
                        test_gaussian, uniform, nonseparable_momentum, samples_location,
                        run_properties, num_samples, num_leapfrog_steps, num_newton_steps, rand_gen,
                        true, save_iterations, suppress_print);
                
            } else
                throw Exception(
                        "Unrecognised value for '-algorithm' option \"" + algorithm_type
                        + "\", can be either 'metropolis', 'hamiltonian' or 'riemannian'.");
            
        } else if (prob_type == "Prob::Test::BayesLogRegression") {
            
            Prob::Test::BayesLogRegression bayes_log_regression(blr_location, blr_poly_order,
                    blr_prior_variance);
            ...
            if (!strcmp(algorithm_type.c_str(), "riemannian")) {
                ...
                MCMC::riemannian<MCMC::State, Prob::Test::BayesLogRegression, Prob::Uniform>(state,
                        bayes_log_regression, uniform, nonseparable_momentum, samples_location,
                        run_properties, num_samples, num_leapfrog_steps, num_newton_steps, rand_gen,
                        true, save_iterations, suppress_print);
                ...
                throw Exception(
                        "Unrecognised value for '-algorithm' option \"" + algorithm_type
                        + "\", can only be 'riemannian'.");
            
        } else
            throw Exception(
                    "Unrecognised probability type " + prob_type
                    + "', can be either 'Prob::Test::Landscape' or 'Prob::Test::Gaussian'.");
        
        std::ofstream prop_file;
        
        prop_file.open((samples_location + ".prp").c_str());
        
        for (std::map<std::string, std::string>::iterator prop_it = run_properties.begin();
                prop_it != run_properties.end(); ++prop_it)
            prop_file << prop_it->first << ": " << prop_it->second << std::endl;
        
        prop_file.close();
        
        if (prob_type == "Prob::Test::Landscape") {
            std::cout << "\n\nMATLAB plot command:\n\nplot_test_scan " << lnd_location
                      << " -state_filename " << samples_location;
