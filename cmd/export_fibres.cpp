...
EXECUTE {
  ...
  if (tcks.has_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP)) {
    ...
    for (size_t tck_i = 0; tck_i < tcks.size(); ++tck_i) {
      ...
      size_t bundle_index = to<size_t>(tcks.get_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP, tck_i));
  ...
  for (size_t tck_i = 0; tck_i < tcks.size(); ++tck_i) {
    ...
    if (tcks.has_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP))
      bundle_index = to<size_t>(tcks.get_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP, tck_i));
    else
      bundle_index = tck_i;
    ...
    if (tcks.has_extend_elem_prop(Fibre::Track::RADIUS_PROP) && use_loaded_radii)
      track_radius = to<double>(tcks.get_extend_elem_prop(Fibre::Track::RADIUS_PROP, tck_i));
    else
      track_radius = radii;

    std::stringstream filename_stream;

    filename_stream << output_location << std::setfill('0') << "/strand_" << std::setw(num_tck_i_dec_places) <<  tck_i  << "-" << std::setw(num_bundle_dec_places) << bundle_index << "-r" << track_radius << ".txt";
    std::string filename = filename_stream.str();

    std::ofstream out (filename.c_str());

    if (!out) {
     throw Exception ("error opening ascii file \"" + filename + "\": " + strerror (errno));
    }

    for (size_t point_i = 0; point_i < tcks[tck_i].size(); ++point_i)
      out << tcks[tck_i][point_i][X] << " " << tcks[tck_i][point_i][Y] << " " << tcks[tck_i][point_i][Z] << "\n";
