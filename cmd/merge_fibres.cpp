    ...
    for (size_t row_i = 0; row_i < strands.size(); ++row_i) {
      for (size_t col_i = row_i+1; col_i < strands.size(); ++col_i) {
        ...
        if (strands.has_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP))
          if (to<size_t>(strands.get_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP,row_i)) == to<size_t>(strands.get_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP,col_i)))
            same_bundle = true;
        ...
        if (!same_bundle)  {
          ...
          Fibre::Strand merged = row_strand.merge(col_strand, num_length_sections, new_degree);
...
//          if (!(row_i == 3 && col_i == 8))
    ...
    for (std::set<Math::Blossom::Edge>::iterator edge_it = edges.begin(); edge_it !=edges.end();) {
    ...
    for (std::vector<Math::Blossom::Edge>::iterator merge_it = merge_list.begin(); merge_it != merge_list.end(); ++merge_it) {
      not_merge_list.erase(merge_it->first);
      not_merge_list.erase(merge_it->second);
      expected_improvement -= merge_it->weight;
      std::cout << merge_it->first << " " << merge_it->second << ": " << - merge_it->weight << std::endl;
    }
    ...
    for (std::vector<Math::Blossom::Edge>::iterator merge_it = merge_list.begin(); merge_it != merge_list.end(); ++merge_it) {
      merged.push_back(strands[merge_it->first].merge(strands[merge_it->second], num_length_sections, new_degree));
    }

    for (std::set<size_t>::iterator not_merge_it = not_merge_list.begin(); not_merge_it != not_merge_list.end(); ++not_merge_it) {
    ...
    double improvement = likelihood->log_prob(merged) + prior.log_prob(merged) - likelihood->log_prob(strands) - prior.log_prob(strands);

    merged.save("/home/tclose/data/merged.str");

//    if (improvement > 0) {
      merged.save(output_location,0,num_tractlets);
      std::cout << "\n\nMerged " << merge_list.size() << " strand pairs for a combined improvement of the posterior of " << improvement << " (" << expected_improvement << " expected) over the original set." << std::endl;
//    } else {
