...
//  size_t num_length_sections = 0;
        ...
        if (File::has_extension<Fibre::Tractlet>(match_location)) {
            ...
            if (!reference.size())
                throw Exception(
                        "Reference set, loaded from '" + reference_location
                        + "' does not contain any fibres.");
            
            if (!reference.has_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP)) {
                
                std::cout
                        << std::endl
                        << "Warning! Reference set does not have bundle_index property, will use strand order."
                        << std::endl;
                
                reference.add_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP, "-1");
                for (size_t tract_i = 0; tract_i < reference.size(); ++tract_i)
                    reference.set_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP, str(tract_i),
                            tract_i);
            ...
            for (size_t ref_i = 0; ref_i < reference.size(); ++ref_i)
                bundle_indices.push_back(
                        to<size_t>(
                                reference.get_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP,
                                        ref_i)));
            
            size_t new_bundle_index = *std::max_element(bundle_indices.begin(),
                    bundle_indices.end());
            ...
            if (bundle_indices.size() != num_bundles)
                throw Exception(
                        "Bundle indices were not unique in reference set '" + reference_location
                        + "'.");
            ...
            if (reference.size() > match.size()) {
                ...
                MR::ProgressBar progress_bar(
                        "Getting closest match between match template and reference set...");
                ...
                for (size_t tract_i = 0; tract_i < match.size(); ++tract_i) {
                    ...
                    std::vector<size_t>::iterator index = std::find(inv_reference_match.begin(),
                            inv_reference_match.end(), tract_i);
            ...
            if (File::has_extension<Fibre::Tractlet>(input_location)) {
                ...
                if (input_location != match_location) {
                    ...
                    if (input.size() != match.size())
                        throw Exception(
                                "Size of match set (" + str(match.size()) + ") and input set ("
                                + str(input.size()) + ") do not match.");
                } else
                    input = match;
                ...
                for (size_t tract_i = 0; tract_i < match.size(); ++tract_i) {
                    ...
                    input.set_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP,
                            str(bundle_indices[reference_match[tract_i]]), tract_i);
                ...
                if (input_location == output_location)
                    throw Exception(
                            ...
                            + "') cannot be the same as input location ('" + input_location
                            + "') for tractlet sets.");
                ...
                if (std::find(elem_header.begin(), elem_header.end(),
                            Fibre::Track::BUNDLE_INDEX_EPROP)
                    == elem_header.end())
                    elem_header.push_back(Fibre::Track::BUNDLE_INDEX_EPROP);
                
                Fibre::Tractlet::Set::Writer writer(output_location, reader,
                        reader.extend_prop_keys(), elem_header, reader.get_extend_props());
            ...
            if (File::has_extension<Fibre::Strand>(input_location)) {
                ...
                Fibre::Strand::Set::Writer writer(output_location, reader,
                        reader.extend_prop_keys(), elem_header, reader.get_extend_props());
