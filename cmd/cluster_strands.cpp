...
EXECUTE {
        ...
        if (new_num_strands >= input_strands.size())
            throw Exception(
                    ...
                    + ") must be less than the original number of strands ("
                    + str(input_strands.size()) + ")");
...
//----------------------------------------------//
        ...
        if (save_clusters) {
            ...
            for (size_t ctr_i = 0; ctr_i < new_num_strands; ctr_i++)
                clusters[ctr_i].save(
                        File::join(cluster_location,
                                "cluster_" + str(ctr_i, num_cluster_dec_places) + "."
                                + Fibre::Strand::FILE_EXTENSION));
    ...
    void printSummary(const KMlocal& theAlg,		// the algorithm
        ...
        *kmOut << "(Cluster assignments:\n" << "    Point  Center  Squared Dist\n"
        ...
        for (int i = 0; i < dataPts.getNPts(); i++) {
            *kmOut << "   " << setw(5) << i << "   " << setw(5) << closeCtr[i] << "   " << setw(10)
                   << sqDist[i] << "\n";
        }
        *kmOut << ")\n";
        delete[] closeCtr;
