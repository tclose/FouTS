...
ARGUMENTS = {
  ...
  Argument ("output", "The prefix for the outputted files (the two output files will be appended with 'length.txt' and "
...
OPTIONS = {
    ...
    Option ("samples_per_acs","")
     + Argument ("samples_per_acs","").type_integer (0,SAMPLES_PER_ACS_DEFAULT,LARGE_FLOAT),

    Option ("samples_per_length","")
     + Argument ("samples_per_length","").type_integer (0,SAMPLES_PER_LENGTH_DEFAULT,LARGE_FLOAT),

    Option ("min_length_samples","")
     + Argument ("min_length_samples","").type_integer (1,MIN_LENGTH_SAMPLES_DEFAULT,LARGE_INT),

    Option ("min_width_samples","")
     + Argument ("min_width_samples","").type_integer (1,MIN_WIDTH_SAMPLES_DEFAULT,LARGE_INT),
...
EXECUTE {
  ...
  Options opt = get_options("samples_per_length");
  if (opt.size())
    samples_per_length = opt[0][0];

  Options opt = get_options("min_width_sections");
  if (opt.size())
    min_width_sections = opt[0][0];

  Options opt = get_options("min_length_sections");
  if (opt.size())
    min_length_sections = opt[0][0];

  Options opt = get_options("strategy");
  if (opt.size())
    strategy = opt[0][0];
  ...
  // Do length max and acs max/average calculations here
