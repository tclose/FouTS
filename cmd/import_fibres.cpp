...
void import(Fibre::Strand::Set& s_set, const std::string& location, const std::string& prefix, MR::ProgressBar& progress_bar);
void import(Fibre::Track::Set& t_set, const std::string& location, const std::string& prefix, bool nfg_format, MR::ProgressBar& progress_bar);
...
EXECUTE {
  ...
  if (opt.size()) {
    ...
    if (prefix.size())
      throw Exception ("'-prefix' option cannot be used in conjunction with '-nfg_format'.");
...
void import(Fibre::Strand::Set& s_set, const std::string& location, const std::string& prefix, MR::ProgressBar& progress_bar) {
  ...
  if (location.length() >= 21) {
    if (!location.substr(location.length() - 21).compare("isotropic_regions.txt"))
      throw Exception ("Found 'isotropic_regions.txt' file, which is assumed to be from Numerical Fibre Generator (NFG) output, please use '-nfg_format' option to import these tracks, and use the '.tck' for the imported tracks file extension.");
  }
  ...
  if (File::is_dir(location)) {
    ...
    M.load (location);
    if (M.columns() != 3)
      throw Exception ("WARNING: file \"" + location + "\" does not contain 3 columns - ignored");
...
void import(Fibre::Track::Set& t_set, const std::string& location, const std::string& prefix, bool nfg_format, MR::ProgressBar& progress_bar) {
  ...
  if (location.length() >= 21) {
    if (!location.substr(location.length() - 21).compare("isotropic_regions.txt") && !nfg_format)
      throw Exception ("Found 'isotropic_regions.txt' file, which is assumed to be from Numerical Fibre Generator (NFG) output, please use '-nfg_format' option to import these tracks.");
  }
  ...
  if (File::is_dir(location)) {
    ...
    for (size_t filename_i = 0; filename_i < filenames.size(); filename_i++)
      import(t_set, File::join(location,filenames[filename_i]), prefix, nfg_format, progress_bar);
    ...
    if (nfg_format) {
      ...
      sscanf(File::basename(location).c_str(), "strand_%*d-%d-r%lf.txt", &bundle_index, &track_radius);

      t_set.set_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP, str(bundle_index), t_set.size()-1);

      t_set.set_extend_elem_prop(Fibre::Track::RADIUS_PROP, str(track_radius), t_set.size()-1);
