...
//const double FILTER_RADIUS_DEFAULT = 8.0;
//const char* FILTER_TYPE_DEFAULT = "median";
...
OPTIONS= {
//
//    Option("filter_radius", "The radius of the filter applied to the noise estimate image")
//    + Argument("filter_radius").type_float(SMALL_FLOAT, FILTER_RADIUS_DEFAULT, LARGE_FLOAT),
//
//    Option("filter_type", "The type of the filter applied to the noise estimate image ('gauss' or 'median'")
//        + Argument("filter_type").type_text(FILTER_TYPE_DEFAULT),
...
//        //------------------------------------------------------------------------------------------
//        // Set the fraction of voxels to include in the final estimate (the filter_radius '--fraction' is used)
//        double filter_radius = FILTER_RADIUS_DEFAULT;
//        opt = get_options("filter_radius");
//        if (opt.size())
//            filter_radius = opt[0][0];
//        if ((filter_radius <= 0.0) || (filter_radius > 1.0))
//            throw Exception("'--filter_radius' must be within (0-1] as it is the fraction of voxels to include "
//                    "in the estimation (found " + str(filter_radius) + ")");
//
//        //------------------------------------------------------------------------------------------
//        // The relative curvature of the reference tract used to calculate the base intensity from
//        std::string filter_type = FILTER_TYPE_DEFAULT;
//        opt = get_options("filter_type");
//        if (opt.size())
//            filter_type = opt[0][0];
        ...
        for (vox_loop.start (dwi_vox, intens_vox); vox_loop.ok();
            ...
            if (max_value > 0.0) {
                ...
                if (fa > fa_threshold) {
                    ...
                    tcts.zero();
                    // Centre the tract in the filter_radius of the voxel
