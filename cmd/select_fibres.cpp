...
template <typename InputType, typename OutputType> OutputType&    convert(const InputType& input, OutputType& output, size_t degree, size_t num_length_sections, size_t num_tractlets, double width_of_tractlet, double strands_per_acs, size_t num_width_sections);


template <typename InputType, typename OutputType> void    add_extra_element_properties(std::vector<std::string>& elem_header);


template <> Fibre::Strand::Set&                           convert(const Fibre::Track::Set& input, Fibre::Strand::Set& output, size_t degree, size_t num_length_sections, size_t num_tractlets, double width_of_tractlet, double strands_per_acs, size_t num_width_sections) {

  return output = input.to_strands(degree);

}

template <> Fibre::Track::Set&                            convert(const Fibre::Strand::Set& input, Fibre::Track::Set& output, size_t degree, size_t num_length_sections, size_t num_tractlets, double width_of_tractlet, double strands_per_acs, size_t num_width_sections) {

  return output = input.to_tracks(num_length_sections);

}

template <> Fibre::Tractlet::Set&                            convert(const Fibre::Strand::Set& input, Fibre::Tractlet::Set& output, size_t degree, size_t num_length_sections, size_t num_tractlets, double width_of_tractlet, double strands_per_acs, size_t num_width_sections) {
...
template <> Fibre::Strand::Set&                           convert(const Fibre::Tractlet::Set& input, Fibre::Strand::Set& output, size_t degree, size_t num_length_sections, size_t num_tractlets, double width_of_tractlet, double strands_per_acs, size_t num_width_sections) {
...
template <> Fibre::Track::Set&                            convert(const Fibre::Tractlet::Set& input, Fibre::Track::Set& output, size_t degree, size_t num_length_sections, size_t num_tractlets, double width_of_tractlet, double strands_per_acs, size_t num_width_sections) {
...
template <> Fibre::Tractlet::Set&                           convert(const Fibre::Track::Set& input, Fibre::Tractlet::Set& output, size_t degree, size_t num_length_sections, size_t num_tractlets, double width_of_tractlet, double strands_per_acs, size_t num_width_sections) {
...
template <> void                                            add_extra_element_properties<Fibre::Tractlet,Fibre::Strand>(std::vector<std::string>& header) {
  ...
  std::vector<std::string>::iterator prop_it = std::find(header.begin(), header.end(), str("tract_volume"));
...
template <> void                                            add_extra_element_properties<Fibre::Tractlet,Fibre::Track>(std::vector<std::string>& header) {
...
template <typename InputType, typename OutputType>
    void   one_to_one(
                        const typename InputType::Set& input,
  ...
  MR::ProgressBar progress_bar("Selecting from " + class_name(input_location) + " set and outputting to " + class_name(output_location) + " set...");

  convert(selected, output, degree, num_length_sections, num_tractlets, width_of_tractlet, strands_per_acs, num_width_sections);
...
template <typename InputType, typename OutputType, typename ReaderType, typename WriterType>
    void   set_to_set(  const std::string& input_location,
                        const std::string& output_location,
                        ReaderType& reader,
                        WriterType& writer,
                        std::vector<size_t>& include,
  ...
  MR::ProgressBar progress_bar("Selecting from set of " + class_name(input_location) + "s and outputting to set of " + class_name(output_location) + "s...", num_included_sets);
  ...
  while (reader.next(input_set)) {
    ...
    if (!include.size() || std::find(include.begin(), include.end(), index) != include.end()) {
      ...
      convert(input_set, output_set, degree, num_length_sections, num_tractlets, width_of_tractlet, strands_per_acs, num_width_sections);
...
template <typename InputType, typename OutputType, typename ReaderType>
    void   set_to_one(  const std::string& input_location,
                        const std::string& output_location,
                        ReaderType& reader,
                        std::vector<size_t>& include,
                        size_t degree,
  ...
  Fibre::Base::Writer<OutputType> writer (output_location, Fibre::Base::Object::select_props<OutputType>(reader.elem_prop_keys()), header);
  ...
  MR::ProgressBar progress_bar("Selecting from set of " + class_name(input_location) + "s and outputting to " + class_name(output_location) + " set...", num_included_sets);
...
template <typename InputType, typename OutputType, typename WriterType>
    void   one_to_set(  const typename InputType::Set& input,
                        const std::string& input_location,
                        const std::string& output_location,
                        WriterType& writer,
                        std::vector<size_t>& include,
  ...
  convert(selected, converted, degree, num_length_sections, num_tractlets, width_of_tractlet, strands_per_acs, num_width_sections);

  if (separate_sets) {

    MR::ProgressBar progress_bar("Selecting from " + class_name(input_location) + " set and outputting into set of " + class_name(output_location) + "s, with one fibre per set...",selected.size());
    ...
    MR::ProgressBar progress_bar("Selecting from " + class_name(input_location) + " set and outputting into a singleton set of " + class_name(output_location) + "s...");
...
EXECUTE {
  ...
  if (bundle_include.size()) {
    if (File::has_or_txt_extension<Fibre::Strand>(input_location) || File::has_or_txt_extension<Fibre::Track>(input_location)) {
      ...
      if (!strands.has_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP))
        throw Exception ("'" + Fibre::Track::BUNDLE_INDEX_EPROP + "' was not found in input strand set.");

      for (size_t strand_i = 0; strand_i < strands.size(); ++strand_i) {

        size_t bundle_index = to<size_t>(strands.get_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP, strand_i));
  ...
  if ((strands_per_acs > 0) && num_width_sections)
    throw Exception ("'-num_width_sections' and '-strands_per_acs' option cannot be used in conjunction.");

  else if ((strands_per_acs <= 0.0) && !num_width_sections && (File::has_txt_or_set_extension<Fibre::Tractlet>(input_location) && (File::has_txt_or_set_extension<Fibre::Strand>(output_location) || File::has_txt_or_set_extension<Fibre::Track>(output_location))))
    throw Exception ("Either '-num_width_sections' and '-strands_per_acs' must be supplied for conversion from tractlet to strand/track classes.");
