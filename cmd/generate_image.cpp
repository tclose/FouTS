...
DESCRIPTION = {
    "Generates a set of DW-FTS images from a given strand or tractlet configuration.",
    "",
    NULL
};
...
//----------------------------------------------------//
        ...
        if (!diff_isotropic && (noise_type == "rician"))
            throw Exception(
                    "Rician noise cannot be added to images with isotropic components removed.");
...
//----------------------------------//
        ...
        Image::Expected::Buffer* image = Image::Expected::Buffer::factory(exp_type, img_dims,
