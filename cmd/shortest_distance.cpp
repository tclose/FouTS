...
EXECUTE {
  ...
  std::string samples_location = argument[1];
	std::string output_location = File::strip_extension(samples_location) + ".shrt." + File::extension(samples_location);
    ...
    MR::ProgressBar progress_bar("Calculating shortest distances...", to<size_t>(reader.get_extend_props()["count"]));
