...
//-----------------//
        ...
        if (argument.size() == 1)
            ...
            if (File::extension(origin_location) != File::extension(output_location))
                throw Exception(
                        "Output location extension must match origin location extension (and must be a 'set' extension) when only two arguments are provided.");
        ...
        if (num_axes >= 1) {
            ...
            output_gradient_location = File::strip_extension(output_location) + ".gradient."
                                       + File::extension(output_location);
            
            if (File::extension(origin_location) != File::extension(axis1_location))
                throw Exception(
                        "Extension of axis1 location (2nd argument), '" + File::extension(
                                axis1_location)
                        + "', does not match extension of origin location (1st argument), '"
                        + File::extension(origin_location) + "'.");
        }
        ...
        if (num_axes >= 2) {
            ...
            if (File::extension(origin_location) != File::extension(axis2_location))
                throw Exception(
                        "Extension of second axis location (3rd argument), '" + File::extension(
                                axis2_location)
                        + "', does not match extension of origin location (1st argument), '"
                        + File::extension(origin_location) + "'.");
        }
        ...
        if (num_axes == 3) {
            ...
            if (File::extension(origin_location) != File::extension(axis3_location))
                throw Exception(
                        "Extension of third axis location (4th argument), '" + File::extension(
                                axis3_location)
                        + "', does not match extension of origin location (1st argument), '"
                        + File::extension(origin_location) + "'.");
        ...
        if ((num_axes > 1) && !File::has_extension<Image::Expected::Buffer>(output_location))
            throw Exception(
                    "Output file must be a mrtrix image ('mif' extension) for more scans with multiple axes.");
...
//    num_ref_sections = opt[0][0];
        ...
        if (num_axes > 1) {
            ...
            if (centred)
                std::cout
                        << "Warning: option '-centred' is not required with used with multi-dimension scans as non-centred scans are not available.";
            
            if (no_prob)
                throw Exception(
                        "The '-no_prob' option is meant to be used to generate a sequence of states can can't be used with multi-dimension scans.");
...
//----------------------//
        ...
        Image::Expected::Buffer* exp_image = Image::Expected::Buffer::factory(exp_type, img_dims,
                img_vox_lengths, diffusion_model, exp_num_length_sections, exp_num_width_sections,
                exp_interp_extent, img_offsets, exp_enforce_bounds, exp_half_width);
        ...
        if (only_first) {
            ...
            if (File::has_extension<Fibre::Strand>(origin_location)) {
                ...
                if (num_axes == 0) {
                    ...
                    std::cout << "Total Prior Gradient:" << prior_gradient << std::endl
                              << std::endl;
                    
                    std::map<std::string, double> component_values = prior.get_component_values(
                            origin);
                    
                    for (std::map<std::string, double>::const_iterator comp_it =
                            component_values.begin(); comp_it != component_values.end(); ++comp_it)
                        std::cout << "Prior " << comp_it->first << ": " << comp_it->second
                                  << std::endl;
                    ...
                    Analysis::scan<Fibre::Strand>(*likelihood, prior, origin, axis1, axis2,
                            num_steps, num_steps2, output_location, properties);
                    
                } else if (num_axes == 3) {
                    
                    Fibre::Strand::Set axis1_set(axis1_location), axis2_set(axis2_location),
                            axis3_set(axis3_location);
                    ...
                    Analysis::scan<Fibre::Strand>(*likelihood, prior, origin, axis1, axis2, axis3,
                            num_steps, num_steps2, num_steps3, output_location, properties);
                    
                } else if (num_axes == -1)
                    throw Exception(
                            "Can't do pre-generated set (implied by 2 input arguments) for individual set.");
                ...
                if (num_axes == 0) {
                    ...
                    Analysis::scan<Fibre::Tractlet>(*likelihood, prior, origin, axis1, axis2,
                            num_steps, num_steps2, output_location, properties);
                    ...
                    Analysis::scan<Fibre::Tractlet>(*likelihood, prior, origin, axis1, axis2, axis3,
                            num_steps, num_steps2, num_steps3, output_location, properties);
...
//*********************************************
            ...
            if (File::has_extension<Fibre::Strand>(origin_location)) {
                ...
                if (num_axes == 0) {
                    ...
                    Fibre::Strand::Set prior_gradient(origin), likelihood_gradient(origin),
                            gradient(origin);
                    ...
                    if (save_gradient) {
                        ...
                        likelihood_px = likelihood->log_prob_and_fisher(origin, likelihood_gradient,
                                likelihood_fisher);
                        ...
                        gradient.save(
                                File::strip_extension(origin_location) + ".gradient."
                                + File::extension(origin_location));
                        prior_gradient.save(
                                File::strip_extension(origin_location) + ".prior_gradient."
                                + File::extension(origin_location));
                        likelihood_gradient.save(
                                File::strip_extension(origin_location) + ".likelihood_gradient."
                                + File::extension(origin_location));
                        likelihood_fisher.save(
                                File::strip_extension(origin_location) + ".likelihood_hessian."
                                + File::extension(origin_location) + ".tnr");
                    ...
                    Analysis::scan<Fibre::Strand::Set>(*likelihood, prior, origin, axis1, axis2,
                            num_steps, num_steps2, output_location, properties);
                    
                } else if (num_axes == 3) {
                    
                    Fibre::Strand::Set axis1(axis1_location), axis2(axis2_location), axis3(
                            axis3_location);
                    ...
                    Analysis::scan<Fibre::Strand::Set>(*likelihood, prior, origin, axis1, axis2,
                            axis3, num_steps, num_steps2, num_steps3, output_location, properties);
                ...
                if (num_axes == 0) {
                    ...
                    Fibre::Tractlet::Set prior_gradient(origin), likelihood_gradient(origin),
                            gradient(origin);
                    ...
                    Analysis::scan<Fibre::Tractlet::Set>(*likelihood, prior, origin, axis1, axis2,
                            num_steps, num_steps2, output_location, properties);
                    
                } else if (num_axes == 3) {
                    
                    Fibre::Tractlet::Set axis1(axis1_location), axis2(axis2_location), axis3(
                            axis3_location);
                ...
                if (num_axes != -1)
                    throw Exception(
                            "Set origin only allowed when exactly two arguments are provided.");
...
//      Fibre::Strand::Set true_strands ("/home/tclose/data/fibre/strand/single/z.str");
                ...
                MR::ProgressBar progress_bar("Scanning through strand sets...",
                        to<size_t>(reader.get_extend_props()["count"]));
                
                while (reader.next(strands)) {
                    
                    strands.set_extend_prop("px",
                            str(likelihood->log_prob(strands) + prior.log_prob(strands)));
                ...
                MR::ProgressBar progress_bar("Scanning through tractlet sets...",
                        to<size_t>(reader.get_extend_props()["count"]));
                
                while (reader.next(tractlets)) {
                    
                    tractlets.set_extend_prop("px",
                            str(likelihood->log_prob(tractlets) + prior.log_prob(tractlets)));
