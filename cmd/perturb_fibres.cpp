...
template<typename T> void perturb_fibres(const std::string& input_location,
                                         const std::string& output_location, double stdev,
                                         const std::string& scale_location, gsl_rng* rand_gen,
...
//----------------------------------------------//
        ...
        if (File::has_extension<Fibre::Strand>(input_location)) {
            ...
            perturb_fibres<Fibre::Strand>(input_location, output_location, stdev, scales_location,
                    rand_gen, vector_mask);
            ...
            perturb_fibres<Fibre::Tractlet>(input_location, output_location, stdev, scales_location,
                    rand_gen, vector_mask);
    ...
    template<typename T> void perturb_fibres(const std::string& input_location,
                                             const std::string& output_location, double stdev,
                                             const std::string& scales_location, gsl_rng* rand_gen,
                                             const MR::Math::Vector<double>& flat_mask) {
        ...
        for (size_t elem_i = 0; elem_i < fibres_vector.size(); elem_i++) {
            if (flat_mask[elem_i])
                fibres_vector[elem_i] += (double) gsl_ran_flat(rand_gen, -scales_vector[elem_i],
                        scales_vector[elem_i]);
            else
