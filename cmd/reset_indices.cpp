...
template <typename T> void reset_indices(T& fibres) {
  ...
  for (size_t strand_i = 0; strand_i < fibres.size(); ++strand_i) {
    ...
    std::string bundle_index = fibres.get_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP, strand_i);
  ...
  for (size_t strand_i = 0; strand_i < fibres.size(); ++strand_i) {
    ...
    size_t reset_bundle_index = indices[fibres.get_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP, strand_i)];

    fibres.set_extend_elem_prop(Fibre::Track::BUNDLE_INDEX_EPROP, str(reset_bundle_index), strand_i);
