...
EXECUTE {
  ...
  if (File::extension(input_location) != File::extension(output_location))
    throw Exception ("Input ('" + input_location + "') and output_location ('" + output_location + "') extensions need to match.");

//----------------------//
//  Load original paths
//----------------------//

  if ( File::has_extension<Fibre::Strand>(input_location) || File::has_extension<Fibre::Track>(input_location)) {

    Fibre::Track::Set all_tcks(input_location, num_length_sections);

    MR::ProgressBar progress_bar ("Removing tracks/strands that do not pass through centre of sphere...");
    ...
    MR::ProgressBar progress_bar ("Removing tractlets that do not pass through centre of sphere...");

    Fibre::Tractlet::Set include_tractlets;

    include_tractlets = all_tractlets.peel(distance, num_length_sections, num_width_sections, cube);
    ...
    MR::ProgressBar progress_bar ("Removing strands that do not pass through centre of sphere...");
    ...
    while (reader.next(all_tractlets)) {
      writer.append(all_tractlets.peel(distance, num_length_sections, num_width_sections, cube));
      progress_bar++;
    }
    ...
    MR::ProgressBar progress_bar ("Removing tcks that do not pass through centre of sphere...");
