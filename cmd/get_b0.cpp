...
EXECUTE {
        ...
        if (opt.size())
            ...
            if (!dwi_header.DW_scheme().is_set())
                throw Exception(
                        "no diffusion encoding found in image \"" + dwi_header.name() + "\"");
        ...
        if (mask_header.dim(0) != dwi_header.dim(0) || mask_header.dim(1) != dwi_header.dim(1)
        ...
        if (method == "median") {
            ...
            throw Exception(
                    "Unrecognised value '" + method
                    + "' for method option, can be either 'median', 'max' or 'average'.");
