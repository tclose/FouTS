...
EXECUTE {
        ...
        BTS::Triple<double> vox_size(dwi.vox(X), dwi.vox(Y), dwi.vox(Z))
        ...
        for (size_t ref_i = 1; ref_i < argument.size(); ++ref_i) {
            ...
            BTS::Image::Observed::Buffer buff(BTS::Triple<size_t>::ONES, vox_size,)
            for (loop.start(vox); loop.ok(); loop.next(vox)) {
            }
            
            Coord interp_length = this->interp_extent * this->vox_lengths();
            // Create tract spans the interpolation length of the interpolation kernel which is
            // centred on the bottom left voxel.
            ...
            // Normalize the density of the tract and set the base_intensity of the set to 1.0, to
            // calculate the required base intensity value to match that of the reference.
