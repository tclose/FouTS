...
EXECUTE {
        ...
        if (File::extension(input_location) != File::extension(output_location))
            throw Exception(
                    "Input location ('" + File::extension(input_location)
                    + "') does not match output location ('" + File::extension(output_location)
                    + "').");
    ...
    template<typename T> void svd_fibres(const std::string& input_location,
        ...
        while (reader.next(input_fibres)) {
            ...
            if (!num_cols)    //First one.
                ...
                throw Exception(
                        "Inconsistent lengths (" + str(num_cols) + " and "
                        + str(input_vector.size()) + ") found in input location.");
