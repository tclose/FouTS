...
EXECUTE {
        ...
        MR::ProgressBar progress_bar(
                "Generating " + str(num_strands) + " short strands on sphere_surface...");
        
        for (size_t strand_i = 0; strand_i < num_strands; strand_i++) {
            Fibre::Strand strand(degree, 0.0);
            
            strand[0] = Triple<double>((double) gsl_ran_gaussian(rand_gen, 1.0),
                                (double) gsl_ran_gaussian(rand_gen, 1.0),
                                (double) gsl_ran_gaussian(rand_gen, 1.0)).normalise()
                        * sphere_radius;
