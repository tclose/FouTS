...
void GeomPerfectMatching::InitKNN(int K) {
    ...
    if (options.verbose)
        printf("adding K nearest neighbors (K=%d)\n", K);
    ...
    for (p = 0; p < node_num; p++) {
        ...
        for (k = 0; k < K; k++) {
            if (nodes[neighbors[k]].is_marked)
                continue;
...
#include "blossom5/triangle/triangle.c"
}
