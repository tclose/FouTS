...
int rand_init_collection(Strand_collection *c, double sphere_r, double control_point_freq,
    ...
    ubound_num_strands = (int) (sphere_r * sphere_r / (strand_r_lbound * strand_r_lbound) + 0.5)
            * 2;
    ...
    while (num_attempts < max_attempts && c->num_strands < ubound_num_strands) {
        ...
        reject = seperation_threshold
                < dot_product(&(c->start_points[c->num_strands * 3]),
                        &(c->end_points[c->num_strands * 3]));
        ...
        if (!reject) {
            ...
            for (strand_i = 0; strand_i < c->num_strands; strand_i++) {
                ...
                overlap_dist = strand_r_buffer_ratio
                        * (c->strand_r[c->num_strands] + c->strand_r[strand_i]);
                
                if ((dist_between_points(&(c->start_points[c->num_strands * 3]),
                             &(c->start_points[strand_i * 3]))
                     < overlap_dist)
                    | (dist_between_points(&(c->end_points[c->num_strands * 3]),
                               &(c->end_points[strand_i * 3]))
                       < overlap_dist)
                    | (dist_between_points(&(c->start_points[c->num_strands * 3]),
                               &(c->end_points[strand_i * 3]))
                       < overlap_dist)
                    | (dist_between_points(&(c->end_points[c->num_strands * 3]),
                               &(c->start_points[strand_i * 3]))
        ...
        if (reject) {
            ...
            c->num_strand_control_points[c->num_strands] = (int) floor(
                    dist_between_points(&(c->start_points[c->num_strands * 3]),
                            &(c->end_points[c->num_strands * 3]))
                    * control_point_freq);
    ...
    for (strand_i = 0; strand_i < c->num_strands; strand_i++) {
        ...
        segment_disp[X] = (c->end_points[strand_i * 3 + X] - c->start_points[strand_i * 3 + X])
                / (c->num_strand_control_points[strand_i] + 1);
        segment_disp[Y] = (c->end_points[strand_i * 3 + Y] - c->start_points[strand_i * 3 + Y])
                / (c->num_strand_control_points[strand_i] + 1);
        segment_disp[Z] = (c->end_points[strand_i * 3 + Z] - c->start_points[strand_i * 3 + Z])
                / (c->num_strand_control_points[strand_i] + 1);
        
        for (; point_i < (start_of_strand_triple_i + c->num_strand_control_points[strand_i]);
                point_i++) {
            
            num_segment_disps = (double) (point_i - start_of_strand_triple_i + 1);
            
            c->control_points[point_i * 3 + X] = c->start_points[strand_i * 3 + X]
                    + num_segment_disps * segment_disp[X];
            c->control_points[point_i * 3 + Y] = c->start_points[strand_i * 3 + Y]
                    + num_segment_disps * segment_disp[Y];
            c->control_points[point_i * 3 + Z] = c->start_points[strand_i * 3 + Z]
                    + num_segment_disps * segment_disp[Z];
        ...
        c->pre_points[strand_i * 3 + X] = c->start_points[strand_i * 3 + X]
                * (1.0 + STRAND_EXT_FRACT);
        c->pre_points[strand_i * 3 + Y] = c->start_points[strand_i * 3 + Y]
                * (1.0 + STRAND_EXT_FRACT);
        c->pre_points[strand_i * 3 + Z] = c->start_points[strand_i * 3 + Z]
                * (1.0 + STRAND_EXT_FRACT);
        
        c->post_points[strand_i * 3 + X] = c->end_points[strand_i * 3 + X]
                * (1.0 + STRAND_EXT_FRACT);
        c->post_points[strand_i * 3 + Y] = c->end_points[strand_i * 3 + Y]
                * (1.0 + STRAND_EXT_FRACT);
        c->post_points[strand_i * 3 + Z] = c->end_points[strand_i * 3 + Z]
                * (1.0 + STRAND_EXT_FRACT);
        
        construct_strand(&(c->strands[strand_i]), strand_i, strand_i,
...
void rand_init_grey_matters(Isotropic_region *isotropic_regions, int num_grey_matters,
    ...
    for (grey_matter_i = 0; grey_matter_i < num_grey_matters; grey_matter_i++) {
        ...
        location_radius = gsl_ran_flat(rand_gen, grey_matter_location_lbound,
        ...
        init_isotropic_region(grey_matter, point_on_sphere, region_radius, grey_matter_diffusivity,
                grey_matter_baseline_signal, grey_matter_weighting);
