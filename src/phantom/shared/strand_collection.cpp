...
void collection_alloc(Strand_collection *c, int num_strands, int num_control_points,
    ...
    c->isotropic_regions = (Isotropic_region*) malloc(
            sizeof(Isotropic_region) * num_isotropic_regions);
...
int save_collection(Strand_collection *c, char *output_dir_path, int save_start_index) {
    ...
    for (isotropic_region_i = 0; isotropic_region_i < c->num_isotropic_regions;
            isotropic_region_i++) {
        if (fprintf(file, "%10f\t%10f\t%10f\t%10f\t%10f\t%10f\t%10f\n",
                    c->isotropic_regions[isotropic_region_i].pos[X],
                    c->isotropic_regions[isotropic_region_i].pos[Y],
                    c->isotropic_regions[isotropic_region_i].pos[Z],
                    c->isotropic_regions[isotropic_region_i].radius,
                    c->isotropic_regions[isotropic_region_i].diffusivity,
                    c->isotropic_regions[isotropic_region_i].baseline_signal,
                    c->isotropic_regions[isotropic_region_i].weighting)
            < 0) {
    ...
    for (strand_i = 0; strand_i < c->num_strands; strand_i++) {
        ...
        sprintf(filename, "strand_%0*d-%0*d-r%f.txt", strand_i_max_width,
                (strand_i + save_start_index), bundle_i_max_width, c->bundle_i_of_strand[strand_i],
                c->strand_r[strand_i]);
        ...
        if (fprintf(file, "%10f\t%10f\t%10f\n", c->pre_points[strand_i * 3 + X],
                    c->pre_points[strand_i * 3 + Y], c->pre_points[strand_i * 3 + Z])
            < 0) {
            printf("Error! Could not print pre_point of strand %d\n", strand_i);
            exit(EXIT_FAILURE);
        }
        
        if (fprintf(file, "%10f\t%10f\t%10f\n", c->start_points[strand_i * 3 + X],
                    c->start_points[strand_i * 3 + Y], c->start_points[strand_i * 3 + Z])
            < 0) {
        ...
        for (; point_i < end_of_strand_triple_i; point_i++) {
            ...
            if (fprintf(file, "%10f\t%10f\t%10f\n", c->control_points[point_i * 3 + X],
                        c->control_points[point_i * 3 + Y], c->control_points[point_i * 3 + Z])
                < 0) {
        ...
        if (fprintf(file, "%10f\t%10f\t%10f\n", c->end_points[strand_i * 3 + X],
                    c->end_points[strand_i * 3 + Y], c->end_points[strand_i * 3 + Z])
            < 0) {
            printf("Error! Could not print end_point of strand %d\n", strand_i);
            exit(EXIT_FAILURE);
        }
        
        if (fprintf(file, "%10f\t%10f\t%10f\n", c->post_points[strand_i * 3 + X],
                    c->post_points[strand_i * 3 + Y], c->post_points[strand_i * 3 + Z])
            < 0) {
...
int load_collection(Strand_collection *c, char *input_dir_path) {
    int error;
    DIR *directory;
    FILE *file;
    ...
    int strand_i, saved_strand_i, bundle_i, start_of_strand_triple_i, end_of_strand_triple_i,
            point_i, dim_i;
    ...
    num_strand_control_points = (int*) malloc(
            sizeof(int) * NUM_CONTROL_POINTS_ON_STRAND_BLOCK_SIZE);
    ...
    while ((directory_entry = readdir(directory)) != NULL) {
        ...
        if (sscanf(filename, "strand_%d-%*d-r_%lf.txt", &saved_strand_i, &strand_r) > 0) {
            ...
            while (fgets(line, 100, file) != NULL) {
                ...
                if (sscanf(line, "%lf\t%lf\t%lf", &(control_point[X]), &(control_point[Y]),
                            &(control_point[Z]))
                    == 0) {
            ...
            if (num_strand_control_points[num_strands] >= 4) {
                ...
                if (num_strands % NUM_CONTROL_POINTS_ON_STRAND_BLOCK_SIZE == 0)
                    num_strand_control_points = (int*) realloc(num_strand_control_points,
                            sizeof(int) * (num_strands + NUM_CONTROL_POINTS_ON_STRAND_BLOCK_SIZE));
                
            } else {
                printf(
                        "Error! File %s did not have enough points (min 4) please rectify or remove the file!\n",
                        filename);
                exit(EXIT_FAILURE);
            }
    ...
    while ((directory_entry = readdir(directory)) != NULL) {
        ...
        if ((scans = sscanf(filename, "strand_%d-%d-r%lf.txt", &saved_strand_i, &bundle_i,
                &(c->strand_r[strand_i])))
            > 0) {
            ...
            if (fgets(line, 100, file) == NULL) {
                printf(
                        "Error! Mismatch between number of points (first time %d, second time 0) in subsequent reads of file %s!\n",
                        (c->num_strand_control_points[strand_i]), filename);
                
                exit(EXIT_FAILURE);
            } else {
                sscanf(line, "%lf\t%lf\t%lf", &(c->pre_points[strand_i * 3 + X]),
                        &(c->pre_points[strand_i * 3 + Y]), &(c->pre_points[strand_i * 3 + Z]));
            }
            ...
            if (fgets(line, 100, file) == NULL) {
                ...
                sscanf(line, "%lf\t%lf\t%lf", &(c->start_points[strand_i * 3 + X]),
                        &(c->start_points[strand_i * 3 + Y]), &(c->start_points[strand_i * 3 + Z]));
            }
            
            /* Read control_points */
            start_of_strand_triple_i = point_i;
            end_of_strand_triple_i = start_of_strand_triple_i
                    + c->num_strand_control_points[strand_i];
            
            for (; point_i < end_of_strand_triple_i; point_i++) {
                
                if (fgets(line, 100, file) == NULL) {
                    printf(
                            "Error! Mismatch between number of points (first time %d, second time %d) in subsequent reads of file %s!\n",
                            (c->num_strand_control_points[strand_i]),
                            (point_i - start_of_strand_triple_i), filename);
                    exit(EXIT_FAILURE);
                } else {
                    sscanf(line, "%lf\t%lf\t%lf", &(c->control_points[point_i * 3 + X]),
                            &(c->control_points[point_i * 3 + Y]),
                            &(c->control_points[point_i * 3 + Z]));
                }
            }
            ...
            if (fgets(line, 100, file) == NULL) {
                ...
                sscanf(line, "%lf\t%lf\t%lf", &(c->end_points[strand_i * 3 + X]),
                        &(c->end_points[strand_i * 3 + Y]), &(c->end_points[strand_i * 3 + Z]));
            }
            ...
            if (fgets(line, 100, file) == NULL) {
                ...
                sscanf(line, "%lf\t%lf\t%lf", &(c->post_points[strand_i * 3 + X]),
                        &(c->post_points[strand_i * 3 + Y]), &(c->post_points[strand_i * 3 + Z]));
            }
            
            construct_strand(&(c->strands[strand_i]), strand_i, bundle_i,
                    &(c->control_points[start_of_strand_triple_i * 3]),
                    &(c->start_points[strand_i * 3]), &(c->end_points[strand_i * 3]),
                    &(c->pre_points[strand_i * 3]), &(c->post_points[strand_i * 3]),
                    &(c->segments[start_of_strand_triple_i + 3 * strand_i]),
...
int count_isotropic_regions(char *path) {
    ...
    char line[512];
    double isotropic_region_pos[3], isotropic_region_radius, isotropic_region_diffusivity,
            isotropic_region_baseline_signal, isotropic_region_weighting;
    ...
    while (fgets(line, 100, file) != NULL) {
        ...
        if (sscanf(line, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf", &(isotropic_region_pos[X]),
                    &(isotropic_region_pos[Y]), &(isotropic_region_pos[Z]),
                    &isotropic_region_radius, &isotropic_region_diffusivity,
                    &isotropic_region_baseline_signal, &isotropic_region_weighting)
            == 0) {
...
void load_isotropic_regions(Isotropic_region *destination, char *path, int num_isotropic_regions) {
    ...
    for (isotropic_region_i = 0; isotropic_region_i < num_isotropic_regions; isotropic_region_i++) {
        ...
        if (fgets(line, 100, file) == NULL) {
            ...
            sscanf(line, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf", &(isotropic_region_pos[X]),
                    &(isotropic_region_pos[Y]), &(isotropic_region_pos[Z]),
                    &isotropic_region_radius, &isotropic_region_diffusivity,
                    &isotropic_region_baseline_signal, &isotropic_region_weighting);
            
            init_isotropic_region(&(destination[isotropic_region_i]), isotropic_region_pos,
                    isotropic_region_radius, isotropic_region_diffusivity,
                    isotropic_region_baseline_signal, isotropic_region_weighting);
        }
...
void copy_isotropic_regions(Strand_collection *destination, Isotropic_region *isotropic_regions,
                            int num_isotropic_regions) {
    
    destination->isotropic_regions = (Isotropic_region*) realloc(destination->isotropic_regions,
            sizeof(Isotropic_region) * num_isotropic_regions);
    
    destination->num_isotropic_regions = num_isotropic_regions;
    
    memcpy(destination->isotropic_regions, isotropic_regions,
            sizeof(Isotropic_region) * num_isotropic_regions);
