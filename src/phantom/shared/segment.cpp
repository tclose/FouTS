...
void init_segment(Segment *segment, int segment_i, Segment *prev_segment, Control_point point,
                  Control_point next_point, Strand *strand, double sample_acs) {
    ...
    segment->length = sqrt(
            segment->disp[X] * segment->disp[X] + segment->disp[Y] * segment->disp[Y]
            + segment->disp[Z] * segment->disp[Z]);
    
    segment->num_samples = ceil(segment->length * sample_acs);
    
    /* differs from the inverse of 'sample_acs' due to the ceiling num_samples to an int. */
...
void print_segment(Segment *segment, char indent[]) {
    ...
    printf("\n%sNum samples %d, length %g, sample_length %g\n", indent, segment->num_samples,
            segment->length, segment->sample_length);
