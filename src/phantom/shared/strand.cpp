...
void construct_strand(Strand *strand, int strand_i, int bundle_i, double *control_points, double *start_point, double *end_point, double *pre_point, double *post_point, Segment segments[], int num_strand_control_points, double sample_acs, double strand_r) {
	...
	segment = &(segments[0]);
	init_segment(segment, -1, NULL, point, next_point, strand, sample_acs);
...
//	  printf("%g %g %g\n", point.pos[X], point.pos[Y], point.pos[Z]);
		...
		init_segment(segment, segment_i, prev_segment, point, next_point, strand, sample_acs);
