...
void add_strand_to_bundle(Bundle *bundle, Strand *strand) {
    ...
    if ((bundle->num_strands % BUNDLE_STRAND_ALLOC_BLOCK) == 0) {
        ...
        bundle->strands = (Strand**) realloc(bundle->strands,
                (sizeof(Strand*) * (bundle->num_strands + BUNDLE_STRAND_ALLOC_BLOCK)));
...
double calculate_volumes(Bundle *bundle) {
    ...
    for (strand_i = 0; strand_i < bundle->num_strands; strand_i++) {
        ...
        volumes += strand->radius * strand->radius * PI;
...
int compare_bundle_volumes(const void *a, const void *b) {
    ...
    a_cross_section = calculate_volumes((Bundle*) a);
    b_cross_section = calculate_volumes((Bundle*) b);
