...
void resample_collection(Strand_collection *old_c, Strand_collection *resample_c,
    ...
    ubound_num_control_points = (int) ceil(combined_segment_length / resample_length)
            + 10 * old_c->num_strands; /* Add an extra 10 per strand just to be safe */
    
    collection_alloc(resample_c, old_c->num_strands, ubound_num_control_points,
            old_c->num_isotropic_regions);
    copy_isotropic_regions(resample_c, old_c->isotropic_regions, old_c->num_isotropic_regions);
    
    resample_c->num_control_points = 0;
    
    for (resample_c->num_strands = 0; resample_c->num_strands < old_c->num_strands;
            resample_c->num_strands++) {
        
        resample_strand(&(old_c->strands[resample_c->num_strands]), resample_c, resample_length,
                double_back_angle_threshold, forward_angle_threshold);
    }
    
    resample_c->control_points = (double*) realloc(resample_c->control_points,
            sizeof(double) * 3 * resample_c->num_control_points);
    resample_c->control_points_grad = (double*) realloc(resample_c->control_points_grad,
            sizeof(double) * resample_c->num_control_points);
    resample_c->segments = (Segment*) realloc(resample_c->segments,
            sizeof(Segment) * (resample_c->num_control_points + 3 * resample_c->num_strands));
...
void resample_strand(Strand *old_strand, Strand_collection *resample_c, double resample_length,
    ...
    while (old_segment != old_strand->post_segment) {
        ...
        while (1) {
            ...
            a = dot_product(end_point, end_point) - 2.0 * dot_product(start_point, end_point) + dot_product(
                        start_point, start_point);
            
            b = 2.0 * dot_product(start_point, end_point) - 2.0
                    * dot_product(end_point, prev_resample_point)
                - 2.0 * dot_product(start_point, start_point)
                + 2.0 * dot_product(start_point, prev_resample_point);
            
            c = dot_product(start_point, start_point) - 2.0
                    * dot_product(start_point, prev_resample_point)
                + dot_product(prev_resample_point, prev_resample_point)
        ...
        double_back_angle = angle_between_points(prev_resample_point,
                prev_old_segment->end_point.pos, old_segment->end_point.pos);
        ...
        if (double_back_angle > double_back_angle_threshold) {
            ...
            while (double_back_angle > forward_angle_threshold) {
                ...
                double_back_angle = angle_between_points(prev_resample_point,
                        prev_old_segment->end_point.pos, old_segment->start_point.pos);
    ...
    c = dot_product(start_point, start_point) - 2.0 * dot_product(start_point, prev_resample_point) + dot_product(
                prev_resample_point, prev_resample_point)
        - resample_length * resample_length;
    ...
    resample_point[X] = prev_resample_point[X]
            + (STRAND_EXT_FRACT / old_segment->length) * old_segment->disp[X];
    resample_point[Y] = prev_resample_point[Y]
            + (STRAND_EXT_FRACT / old_segment->length) * old_segment->disp[Y];
    resample_point[Z] = prev_resample_point[Z]
            + (STRAND_EXT_FRACT / old_segment->length) * old_segment->disp[Z];
    ...
    construct_strand(&(resample_c->strands[resample_c->num_strands]), resample_c->num_strands,
            resample_c->bundle_i_of_strand[resample_c->num_strands],
            &(resample_c->control_points[start_of_strand_control_point_i]),
            &(resample_c->start_points[resample_c->num_strands]),
            &(resample_c->end_points[resample_c->num_strands]),
            &(resample_c->pre_points[resample_c->num_strands]),
            &(resample_c->post_points[resample_c->num_strands]),
            &(resample_c->segments[start_of_strand_control_point_i + 3 * resample_c->num_strands]),
            resample_c->num_strand_control_points[resample_c->num_strands], 0.0,
            resample_c->strand_r[resample_c->num_strands]);
