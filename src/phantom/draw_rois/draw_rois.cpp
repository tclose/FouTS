...
//			}
            ...
            start_roi_id = get_start_roi_id(strand->bundle_i, cumulative_num_strands,
                    strand->bundle->num_strands, strand_i_in_bundle[strand->bundle_i],
                    subbundle_mask);
            end_roi_id = get_end_roi_id(strand->bundle_i, cumulative_num_strands,
                    strand->bundle->num_strands, strand_i_in_bundle[strand->bundle_i],
                    subbundle_mask);
            ...
            if ((2 * point_depth_l_bound) > strand->num_control_points) {
                printf(
                        "Warning! Point depth %d exceeds half the number of control points in strand %d. Excluding strand from ROI...\n",
                ...
                for (depth_i = 0; depth_i <= point_depth_u_bound; depth_i++) {
                    ...
                    if (depth_i >= point_depth_l_bound) {
                        ...
                        add_to_roi(combined_masks, masks_closest, num_voxels, voxel_size,
                                voxel_radial_l_bound, voxel_radial_u_bound, forward_segment,
                                extra_point_radius, start_roi_id);
                        
                        /*Add point to end ROI*/
                        add_to_roi(combined_masks, masks_closest, num_voxels, voxel_size,
                                voxel_radial_l_bound, voxel_radial_u_bound, backward_segment,
                                extra_point_radius, end_roi_id);
                    }
                }
            }
        }
...
void add_to_roi(int *masks, double *masks_closest, int num_voxels, double voxel_size,
    ...
    double l_bound[3], u_bound[3], voxel_centre[3], fraction, normal[3], disp_to_start_point[3],
            disp_to_end_point[3], dist_to_strand, in_plane[3], in_plane_length, fov,
            voxel_radial_dist;
    int x, y, z, offset;
    ...
    l_bound[X] =
            max_int(
                    (int) floor(
                            (min_dble(segment->start_point.pos[X], segment->end_point.pos[X]) - inflated_radius + fov) / voxel_size),
                    0);
    l_bound[Y] =
            max_int(
                    (int) floor(
                            (min_dble(segment->start_point.pos[Y], segment->end_point.pos[Y]) - inflated_radius + fov) / voxel_size),
                    0);
    l_bound[Z] =
            max_int(
                    (int) floor(
                            (min_dble(segment->start_point.pos[Z], segment->end_point.pos[Z]) - inflated_radius + fov) / voxel_size),
                    0);
    
    u_bound[X] =
            min_int(
                    (int) ceil(
                            (max_dble(segment->start_point.pos[X], segment->end_point.pos[X]) + inflated_radius
                             + fov)
                            / voxel_size), num_voxels);
    u_bound[Y] =
            min_int(
                    (int) ceil(
                            (max_dble(segment->start_point.pos[Y], segment->end_point.pos[Y]) + inflated_radius
                             + fov)
                            / voxel_size), num_voxels);
    u_bound[Z] =
            min_int(
                    (int) ceil(
                            (max_dble(segment->start_point.pos[Z], segment->end_point.pos[Z]) + inflated_radius
                             + fov)
                            / voxel_size), num_voxels);
    ...
    for (z = l_bound[Z]; z < u_bound[Z]; z++) {
        for (y = l_bound[Y]; y < u_bound[Y]; y++) {
            for (x = l_bound[X]; x < u_bound[X]; x++) {
                ...
                if (voxel_radial_dist >= voxel_radial_l_bound && voxel_radial_dist
...
int get_start_roi_id(int bundle_i, int *cumulative_num_strands, int num_strands_in_bundle,
...
int get_end_roi_id(int bundle_i, int *cumulative_num_strands, int num_strands_in_bundle,
                   int strand_i_in_bundle, int subbundle_mask) {
    
    int end_roi_id;
    
    if (subbundle_mask) {
        end_roi_id = cumulative_num_strands[bundle_i] * 2 + num_strands_in_bundle
                     + strand_i_in_bundle + 2;
    } else {
