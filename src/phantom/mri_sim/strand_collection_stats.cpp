...
Strand_collection_stats *strand_collection_stats_alloc(Strand_collection *collection,
    ...
    if (save_ext_fill_radial_stats) {
        ...
        stats->subvoxel_in_sphere_radial_dist = (double*) malloc(
                sizeof(double) * RADIAL_DISTR_BLOCK_SIZE); /* Could be calculated analytically but it is easier to store while going. */
    }
    
    stats->segment_stats = (Segment_stats**) malloc(
            sizeof(Segment_stats*) * collection->num_strands);
    
    for (strand_i = 0; strand_i < collection->num_strands; strand_i++) {
        stats->segment_stats[strand_i] = (Segment_stats*) malloc(
                sizeof(Segment_stats) * (collection->num_strand_control_points[strand_i] + 1)); /* It is the size of the number of points on the strand plus one for the extra segment. */
    }
    ...
    for (z = 0; z < voxel->num_subvoxels[Z]; z++) {
        for (y = 0; y < voxel->num_subvoxels[Y]; y++) {
            for (x = 0; x < voxel->num_subvoxels[X]; x++) {
                ...
                offset = z * voxel->num_subvoxels[X] * voxel->num_subvoxels[Y]
                        + y * voxel->num_subvoxels[X] + x;
                ...
                if (stats->save_ext_fill_radial_stats) {
                    ...
                    if (radial_dist <= sphere_fov) {
                        if ((stats->subvoxel_in_sphere != 0) && (stats->subvoxel_in_sphere
                                % RADIAL_DISTR_BLOCK_SIZE
                                                                 == 0)) {
                            stats->subvoxel_in_sphere_radial_dist = (double*) realloc(
                                    stats->subvoxel_in_sphere_radial_dist,
                                    sizeof(double) * (stats->subvoxel_in_sphere
                                            + RADIAL_DISTR_BLOCK_SIZE));
                        }
                        stats->subvoxel_in_sphere_radial_dist[stats->subvoxel_in_sphere] =
                                radial_dist;
                    }
                }
                ...
                if (subvoxel->closest_segment != NULL) {
                    ...
                    if (stats->save_ext_fill_radial_stats) {
                        if (stats->save_ext_fill_radial_stats) {
                            if ((stats->fill_net != 0) & (stats->fill_net % RADIAL_DISTR_BLOCK_SIZE
                                    == 0)) {
                                stats->fill_net_radial_dist =
                                        (double*) realloc(stats->fill_net_radial_dist,
                                                sizeof(double) * (stats->fill_net
                                                        + RADIAL_DISTR_BLOCK_SIZE));
                            }
                ...
                while (overlap_strand != NULL) {
                    ...
                    if (stats->save_ext_fill_radial_stats) {
                        ...
                        if ((stats->fill_gross != 0) & (stats->fill_gross % RADIAL_DISTR_BLOCK_SIZE
                                == 0)) {
                            stats->fill_gross_radial_dist = (double*) realloc(
                                    stats->fill_gross_radial_dist,
                                    sizeof(double) * (stats->fill_gross + RADIAL_DISTR_BLOCK_SIZE));
                        }
...
void add_length_curv_stats(Strand_collection_stats *stats, Strand_collection *c) {
    ...
    for (strand_i = 0; strand_i < stats->collection->num_strands; strand_i++) {
        ...
        while (segment->next_segment != NULL) {
            ...
            if ((stats->radius_curv_avg >= RADIUS_CURVATURE_UBOUND) | (segment_stats->radius_curv
                    >= RADIUS_CURVATURE_UBOUND)) {
    ...
    for (strand_i = 0; strand_i < stats->collection->num_strands; strand_i++) {
        ...
        for (segment_i = 1; segment_i < stats->collection->num_strand_control_points[strand_i];
                segment_i++) {
            ...
            radius_curv_diff = stats->segment_stats[strand_i][segment_i].radius_curv
...
int save_strand_collection_stats(Strand_collection_stats *stats, char *output_dir_path) {
    ...
    overlap_fraction = ((double) (stats->fill_gross_in_sphere - stats->fill_net_in_sphere))
            / (double) (stats->fill_gross_in_sphere);
    ...
    for (strand_i = 0; strand_i < stats->collection->num_strands; strand_i++) {
        ...
        for (segment_i = 1;
                segment_i < (stats->collection->num_strand_control_points[strand_i] - 1);
                segment_i++) {
            ...
            if (segment_fill_gross == 0) {
                ...
                segment_overlap_fraction = ((double) (segment_fill_gross - segment_fill_net))
                        / (double) segment_fill_gross;
            }
            ...
            stats->overlap_fraction_stdev += (segment_overlap_fraction - overlap_fraction)
    ...
    stats->overlap_fraction_stdev = sqrt(
            stats->overlap_fraction_stdev / (double) segment_stats_size);
    ...
    if (stats->save_ext_fill_radial_stats) {
        ...
        fwrite(stats->subvoxel_in_sphere_radial_dist, sizeof(double), stats->subvoxel_in_sphere,
                file);
    ...
    for (strand_i = 0; strand_i < stats->collection->num_strands; strand_i++) {
        ...
        for (segment_i = 1;
            ...
            fill_gross_output[output_i] =
                    stats->segment_stats[strand_i][segment_i].spatial_extent_gross;
            fill_net_output[output_i] =
                    stats->segment_stats[strand_i][segment_i].spatial_extent_net;
            
            if (fill_gross_output[output_i] == 0) {
                overlap_fraction_output[output_i] = 0.0;
            } else {
                overlap_fraction_output[output_i] = ((double) (fill_gross_output[output_i]
                        - fill_net_output[output_i]))
                                                    / (double) (fill_gross_output[output_i]);
            }
    ...
    overlap_fraction_median = (1 - remaining_fraction) * overlap_fraction_output[landmark_index]
            + remaining_fraction * overlap_fraction_output[landmark_index + 1];
    angle_median = (1 - remaining_fraction) * angle_output[landmark_index]
            + remaining_fraction * angle_output[landmark_index + 1];
    length_median = (1 - remaining_fraction) * length_output[landmark_index]
            + remaining_fraction * length_output[landmark_index + 1];
    radius_curv_median = (1 - remaining_fraction) * radius_curv_output[landmark_index]
            + remaining_fraction * radius_curv_output[landmark_index + 1];
    
    landmark_index_unrounded = ((double) segment_stats_size) * 0.025;
    landmark_index = (int) floor(landmark_index_unrounded);
    remaining_fraction = landmark_index_unrounded - (double) landmark_index;
    
    overlap_fraction_95conf_low = (1 - remaining_fraction) * overlap_fraction_output[landmark_index]
            + remaining_fraction * overlap_fraction_output[landmark_index + 1];
    angle_95conf_low = (1 - remaining_fraction) * angle_output[landmark_index]
            + remaining_fraction * angle_output[landmark_index + 1];
    length_95conf_low = (1 - remaining_fraction) * length_output[landmark_index]
            + remaining_fraction * length_output[landmark_index + 1];
    radius_curv_95conf_low = (1 - remaining_fraction) * radius_curv_output[landmark_index]
            + remaining_fraction * radius_curv_output[landmark_index + 1];
    ...
    overlap_fraction_95conf_high =
            (1 - remaining_fraction) * overlap_fraction_output[landmark_index] + remaining_fraction
                    * overlap_fraction_output[landmark_index + 1];
    angle_95conf_high = (1 - remaining_fraction) * angle_output[landmark_index]
            + remaining_fraction * angle_output[landmark_index + 1];
    length_95conf_high = (1 - remaining_fraction) * length_output[landmark_index]
            + remaining_fraction * length_output[landmark_index + 1];
    radius_curv_95conf_high = (1 - remaining_fraction) * radius_curv_output[landmark_index]
            + remaining_fraction * radius_curv_output[landmark_index + 1];
    ...
    overlap_fraction_1quartile = (1 - remaining_fraction) * overlap_fraction_output[landmark_index]
            + remaining_fraction * overlap_fraction_output[landmark_index + 1];
    angle_1quartile = (1 - remaining_fraction) * angle_output[landmark_index]
            + remaining_fraction * angle_output[landmark_index + 1];
    length_1quartile = (1 - remaining_fraction) * length_output[landmark_index]
            + remaining_fraction * length_output[landmark_index + 1];
    radius_curv_1quartile = (1 - remaining_fraction) * radius_curv_output[landmark_index]
            + remaining_fraction * radius_curv_output[landmark_index + 1];
    ...
    overlap_fraction_3quartile = (1 - remaining_fraction) * overlap_fraction_output[landmark_index]
            + remaining_fraction * overlap_fraction_output[landmark_index + 1];
    angle_3quartile = (1 - remaining_fraction) * angle_output[landmark_index]
            + remaining_fraction * angle_output[landmark_index + 1];
    length_3quartile = (1 - remaining_fraction) * length_output[landmark_index]
            + remaining_fraction * length_output[landmark_index + 1];
    radius_curv_3quartile = (1 - remaining_fraction) * radius_curv_output[landmark_index]
            + remaining_fraction * radius_curv_output[landmark_index + 1];
