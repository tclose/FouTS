...
float* mri_sim(Strand_collection *c, double fa, double diffusivity, int num_voxels,
    ...
    int x, y, z, vox_offset, grad_i, strand_i, ubound_x, ubound_y, ubound_z, lbound_x, lbound_y,
            lbound_z, isotropic_region_i;
    int image_size;
    float *images;
    ...
    if (save_subvoxels) {
        ...
        *subvoxel_orientations = (double*) calloc(sizeof(double),
                total_num_subvoxels * total_num_subvoxels * total_num_subvoxels * 3);
    ...
    for (isotropic_region_i = 0; isotropic_region_i < c->num_isotropic_regions;
        ...
        ubound_x = min_int(
                (int) ceil(
                        (isotropic_region->pos[X] + isotropic_region->radius + fov) / voxel_size),
                num_voxels);
        ubound_y = min_int(
                (int) ceil(
                        (isotropic_region->pos[Y] + isotropic_region->radius + fov) / voxel_size),
                num_voxels);
        ubound_z = min_int(
                (int) ceil(
                        (isotropic_region->pos[Z] + isotropic_region->radius + fov) / voxel_size),
                num_voxels);
    ...
    for (strand_i = 0; strand_i < c->num_strands; strand_i++) {
        ...
        while (segment != strand->post_segment) {
            ...
            ubound_x =
                    min_int(
                            (int) ceil(
                                    (max_dble(segment->start_point.pos[X],
                                             segment->end_point.pos[X])
                                     + strand->radius + fov)
                                    / voxel_size), num_voxels);
            ubound_y =
                    min_int(
                            (int) ceil(
                                    (max_dble(segment->start_point.pos[Y],
                                             segment->end_point.pos[Y])
                                     + strand->radius + fov)
                                    / voxel_size), num_voxels);
            ubound_z =
                    min_int(
                            (int) ceil(
                                    (max_dble(segment->start_point.pos[Z],
                                             segment->end_point.pos[Z])
                                     + strand->radius + fov)
                                    / voxel_size), num_voxels);
            ...
            lbound_x =
                    max_int(
                            (int) floor(
                                    (min_dble(segment->start_point.pos[X],
                                             segment->end_point.pos[X])
                                     - strand->radius
                                     + fov)
                                    / voxel_size), 0);
            lbound_y =
                    max_int(
                            (int) floor(
                                    (min_dble(segment->start_point.pos[Y],
                                             segment->end_point.pos[Y])
                                     - strand->radius
                                     + fov)
                                    / voxel_size), 0);
            lbound_z =
                    max_int(
                            (int) floor(
                                    (min_dble(segment->start_point.pos[Z],
                                             segment->end_point.pos[Z])
                                     - strand->radius
                                     + fov)
                                    / voxel_size), 0);
    ...
    for (z = 0; z < num_voxels; z++) {
        for (y = 0; y < num_voxels; y++) {
            for (x = 0; x < num_voxels; x++) {
                ...
                intensities = sim_voxel_intensities(voxel, num_grad_directions, grad_directions,
                        b_values, fa, diffusivity);
                    ...
                    for (sub_z = 0; sub_z < num_subvoxels; sub_z++) {
                        for (sub_y = 0; sub_y < num_subvoxels; sub_y++) {
                            for (sub_x = 0; sub_x < num_subvoxels; sub_x++) {
                                subvoxels[((z * num_subvoxels + sub_z) * total_num_subvoxels
                                           * total_num_subvoxels
                                           + (y * num_subvoxels + sub_y) * total_num_subvoxels
                                           + (x * num_subvoxels + sub_x))
                                          * 3
                                          + X] = voxel->orientations[(sub_z * num_subvoxels
                                                                      * num_subvoxels
                                                                      + sub_y * num_subvoxels
                                                                      + sub_x)
                                                                     * 3
                                ...
                                subvoxels[((z * num_subvoxels + sub_z) * total_num_subvoxels
                                           * total_num_subvoxels
                                           ...
                                           + (x * num_subvoxels + sub_x))
                                          * 3
                                          + Y] = voxel->orientations[(sub_z * num_subvoxels
                                                                      * num_subvoxels
                                                                      + sub_y * num_subvoxels
                                                                      + sub_x)
                                                                     * 3
                                ...
                                subvoxels[((z * num_subvoxels + sub_z) * total_num_subvoxels
                                           * total_num_subvoxels
                                           ...
                                           + (x * num_subvoxels + sub_x))
                                          * 3
                                          + Z] = voxel->orientations[(sub_z * num_subvoxels
                                                                      * num_subvoxels
                                                                      + sub_y * num_subvoxels
                                                                      + sub_x)
                                                                     * 3
