    ...
    for (z = 0; z < voxel->num_subvoxels[Z]; z++) {
        for (y = 0; y < voxel->num_subvoxels[Y]; y++) {
            for (x = 0; x < voxel->num_subvoxels[X]; x++) {
                ...
                offset = z * voxel->num_subvoxels[Y] * voxel->num_subvoxels[X]
...
void plot_strand_orientations_in_voxel(Voxel *voxel) {
    ...
    double subvoxel_centre[3], segment_orientation[3], disp_to_start_point[3], disp_to_end_point[3],
            in_plane[3], in_plane_length, dist_to_strand, dist_to_region, normal[3], fraction;
    int x, y, z, offset;
    
    Segment *segment;
    Isotropic_region *isotropic_region;
    
    voxel->orientations = (double*) malloc(
            sizeof(double) * voxel->num_subvoxels[X] * voxel->num_subvoxels[Y]
            * voxel->num_subvoxels[Z] * 3);
    voxel->subvoxels = (Subvoxel*) malloc(
            sizeof(Subvoxel) * voxel->num_subvoxels[X] * voxel->num_subvoxels[Y]
            * voxel->num_subvoxels[Z]);
    ...
    while (isotropic_region_register != NULL) {
        ...
        ubound_x =
                min_int(
                        (int) ceil(
                                (isotropic_region->pos[X] - voxel->origin[X] + isotropic_region->radius) / subvoxel_size[X]),
                        voxel->num_subvoxels[X]);
        ubound_y =
                min_int(
                        (int) ceil(
                                (isotropic_region->pos[Y] - voxel->origin[Y] + isotropic_region->radius) / subvoxel_size[Y]),
                        voxel->num_subvoxels[Y]);
        ubound_z =
                min_int(
                        (int) ceil(
                                (isotropic_region->pos[Z] - voxel->origin[Z] + isotropic_region->radius) / subvoxel_size[Z]),
                        voxel->num_subvoxels[Z]);
        
        lbound_x =
                max_int(
                        (int) floor(
                                (isotropic_region->pos[X] - voxel->origin[X]
                                 - isotropic_region->radius)
                                / subvoxel_size[X]), 0);
        lbound_y =
                max_int(
                        (int) floor(
                                (isotropic_region->pos[Y] - voxel->origin[Y]
                                 - isotropic_region->radius)
                                / subvoxel_size[Y]), 0);
        lbound_z =
                max_int(
                        (int) floor(
                                (isotropic_region->pos[Z] - voxel->origin[Z]
                                 - isotropic_region->radius)
                                / subvoxel_size[Z]), 0);
        ...
        for (z = lbound_z; z < ubound_z; z++) {
            for (y = lbound_y; y < ubound_y; y++) {
                for (x = lbound_x; x < ubound_x; x++) {
                    ...
                    if (fraction <= 1.0) {
                        add_isotropic_region(
                                &(voxel->subvoxels[z * voxel->num_subvoxels[X]
                                                   * voxel->num_subvoxels[Y]
                                                   + y * voxel->num_subvoxels[X] + x]), fraction,
                                isotropic_region);
                    }
                }
            }
    ...
    while (segment_register != NULL) {
        ...
        ubound_x =
                min_int(
                        (int) ceil(
                                (max_dble(segment->start_point.pos[X], segment->end_point.pos[X]) - voxel->origin[X] + segment->strand->radius) / subvoxel_size[X]),
                        voxel->num_subvoxels[X]);
        ubound_y =
                min_int(
                        (int) ceil(
                                (max_dble(segment->start_point.pos[Y], segment->end_point.pos[Y]) - voxel->origin[Y] + segment->strand->radius) / subvoxel_size[Y]),
                        voxel->num_subvoxels[Y]);
        ubound_z =
                min_int(
                        (int) ceil(
                                (max_dble(segment->start_point.pos[Z], segment->end_point.pos[Z]) - voxel->origin[Z] + segment->strand->radius) / subvoxel_size[Z]),
                        voxel->num_subvoxels[Z]);
        
        lbound_x =
                max_int(
                        (int) floor(
                                (min_dble(segment->start_point.pos[X], segment->end_point.pos[X]) - voxel->origin[X]
                                 - segment->strand->radius)
                                / subvoxel_size[X]), 0);
        lbound_y =
                max_int(
                        (int) floor(
                                (min_dble(segment->start_point.pos[Y], segment->end_point.pos[Y]) - voxel->origin[Y]
                                 - segment->strand->radius)
                                / subvoxel_size[Y]), 0);
        lbound_z =
                max_int(
                        (int) floor(
                                (min_dble(segment->start_point.pos[Z], segment->end_point.pos[Z]) - voxel->origin[Z]
                                 - segment->strand->radius)
                                / subvoxel_size[Z]), 0);
            ...
            for (y = lbound_y; y < ubound_y; y++) {
                for (x = lbound_x; x < ubound_x; x++) {
                    ...
                    if (dot_product(disp_to_start_point, segment->disp) < 0.0) {
                        dist_to_strand = sqrt(
                                dot_product(disp_to_start_point, disp_to_start_point));
                        ...
                        normal[X] = disp_to_start_point[Y] * segment->disp[Z]
                                - disp_to_start_point[Z] * segment->disp[Y];
                        normal[Y] = disp_to_start_point[Z] * segment->disp[X]
                                - disp_to_start_point[X] * segment->disp[Z];
                        normal[Z] = disp_to_start_point[X] * segment->disp[Y]
                                - disp_to_start_point[Y] * segment->disp[X];
                    ...
                    if (fraction <= 1.0) {
                        add_orientation(
                                &(voxel->subvoxels[z * voxel->num_subvoxels[X]
                                                   * voxel->num_subvoxels[Y]
                                                   + y * voxel->num_subvoxels[X] + x]),
                                segment_orientation, fraction, segment);
                    }
                }
            }
        }
