...
void trim(Strand_collection *trim_c, Strand_collection *c, double trim_dist, double length_reject_threshold, int save_seperate_bundles, int trim_to_cube) {
	...
	for (strand_i = 0; strand_i < c->num_strands; strand_i++) {
		...
		while (segment->next_segment != NULL) {
			...
			if ( ( (strand_section == NULL) || !(strand_section->in_roi) ) && (is_in_roi(segment->start_point.pos, trim_dist, trim_to_cube) ) ) {
				...
				if (!is_in_roi(segment->start_point.pos, trim_dist, trim_to_cube) ) {
					...
					set_section_exit(strand_section, segment->prev_segment, trim_dist, trim_to_cube);
