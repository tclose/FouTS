...
void trim(Strand_collection *trim_c, Strand_collection *c, double trim_dist,
          double length_reject_threshold, int save_seperate_bundles, int trim_to_cube) {
    
    int strand_i, num_trimmed_strands, num_trim_control_points, start_of_strand_control_point_i,
            control_point_i;
    ...
    for (strand_i = 0; strand_i < c->num_strands; strand_i++) {
        ...
        if (is_in_roi(segment->start_point.pos, trim_dist, trim_to_cube)) {
            
            strand_section = strand_sections[strand_i] = (Strand_section*) malloc(
                    sizeof(Strand_section));
        ...
        while (segment->next_segment != NULL) {
            ...
            if (((strand_section == NULL) || !(strand_section->in_roi)) && (is_in_roi(
                        segment->start_point.pos, trim_dist, trim_to_cube))) {
                ...
                set_section_entry(strand_section, segment->prev_segment, trim_dist, trim_to_cube);
                
            } else if ((strand_section != NULL) && strand_section->in_roi) {
                
                if (!is_in_roi(segment->start_point.pos, trim_dist, trim_to_cube)) {
                    
                    set_section_exit(strand_section, segment->prev_segment, trim_dist,
                            trim_to_cube);
        ...
        if ((strand_section != NULL) && strand_section->in_roi) {
    ...
    collection_alloc(trim_c, num_trimmed_strands, num_trim_control_points,
            c->num_isotropic_regions);
    ...
    trim_c->sphere_r = trim_dist;
    trim_c->fov = trim_dist;
    ...
    for (strand_i = 0; strand_i < c->num_strands; strand_i++) {
        ...
        while (strand_section != NULL) {
            ...
            if (strand_section->length_accepted) {
                ...
                trim_c->start_points[(trim_c->num_strands * 3) + X] =
                        strand_section->entry_point[X];
                trim_c->start_points[(trim_c->num_strands * 3) + Y] =
                        strand_section->entry_point[Y];
                trim_c->start_points[(trim_c->num_strands * 3) + Z] =
                        strand_section->entry_point[Z];
                ...
                if (strand_section->pre_point != NULL) {
                    trim_c->pre_points[(trim_c->num_strands * 3) + X] =
                            strand_section->pre_point[X];
                    trim_c->pre_points[(trim_c->num_strands * 3) + Y] =
                            strand_section->pre_point[Y];
                    trim_c->pre_points[(trim_c->num_strands * 3) + Z] =
                            strand_section->pre_point[Z];
                    
                } else {
                    
                    trim_c->pre_points[(trim_c->num_strands * 3) + X] =
                            strand_section->entry_point[X] * (1.0 + STRAND_EXT_FRACT);
                    trim_c->pre_points[(trim_c->num_strands * 3) + Y] =
                            strand_section->entry_point[Y] * (1.0 + STRAND_EXT_FRACT);
                    trim_c->pre_points[(trim_c->num_strands * 3) + Z] =
                            strand_section->entry_point[Z] * (1.0 + STRAND_EXT_FRACT);
                    
                }
                
                if (strand_section->post_point != NULL) {
                    trim_c->post_points[(trim_c->num_strands * 3) + X] =
                            strand_section->post_point[X];
                    trim_c->post_points[(trim_c->num_strands * 3) + Y] =
                            strand_section->post_point[Y];
                    trim_c->post_points[(trim_c->num_strands * 3) + Z] =
                            strand_section->post_point[Z];
                    
                } else {
                    
                    trim_c->post_points[(trim_c->num_strands * 3) + X] =
                            strand_section->exit_point[X] * (1.0 + STRAND_EXT_FRACT);
                    trim_c->post_points[(trim_c->num_strands * 3) + Y] =
                            strand_section->exit_point[Y] * (1.0 + STRAND_EXT_FRACT);
                    trim_c->post_points[(trim_c->num_strands * 3) + Z] =
                            strand_section->exit_point[Z] * (1.0 + STRAND_EXT_FRACT);
                    
                }
                
                trim_c->num_strand_control_points[trim_c->num_strands] =
                        strand_section->num_control_points;
                ...
                if (save_seperate_bundles) {
                    ...
                    if (strand_section->is_first_section) {
                        trim_c->bundle_i_of_strand[trim_c->num_strands] =
                                strand_section->strand->bundle_i;
                    } else {
                ...
                for (control_point_i = 0; control_point_i < strand_section->num_control_points;
                        control_point_i++) {
                    
                    trim_c->control_points[trim_c->num_control_points * 3 + X] =
                            segment->start_point.pos[X];
                    trim_c->control_points[trim_c->num_control_points * 3 + Y] =
                            segment->start_point.pos[Y];
                    trim_c->control_points[trim_c->num_control_points * 3 + Z] =
                            segment->start_point.pos[Z];
                ...
                construct_strand(&(trim_c->strands[trim_c->num_strands]), trim_c->num_strands,
                        trim_c->bundle_i_of_strand[trim_c->num_strands],
                        &(trim_c->control_points[start_of_strand_control_point_i]),
                        &(trim_c->start_points[trim_c->num_strands]),
                        &(trim_c->end_points[trim_c->num_strands]),
                        &(trim_c->pre_points[trim_c->num_strands]),
                        &(trim_c->post_points[trim_c->num_strands]),
                        &(trim_c->segments[start_of_strand_control_point_i + 3 * trim_c->num_strands]),
                        trim_c->num_strand_control_points[trim_c->num_strands], 0.0,
                        trim_c->strand_r[trim_c->num_strands]);
