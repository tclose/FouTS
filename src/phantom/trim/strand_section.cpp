...
void set_section_entry(Strand_section *section, Segment *segment, double trim_dist,
                       int trim_to_cube) {
    
    roi_intersect(section->entry_point, segment->start_point.pos, segment->disp, trim_dist,
            trim_to_cube);
...
int is_in_roi(double* pos, double trim_dist, int trim_to_cube) {
    ...
    //Trim to cube.
        ...
        in_roi = (fabs(pos[0]) < trim_dist) && (fabs(pos[1]) < trim_dist)
                 && (fabs(pos[2]) < trim_dist);
...
void set_section_exit(Strand_section *section, Segment *segment, double trim_dist,
                      int trim_to_cube) {
    
    roi_intersect(section->exit_point, segment->start_point.pos, segment->disp, trim_dist,
            trim_to_cube);
