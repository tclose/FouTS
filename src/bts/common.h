...
namespace BTS {
  ...
  template<typename T, typename U> class CompareFirst {
    public:
      bool operator()(const std::pair<T, U>& p1, const std::pair<T, U>& p2) {
        return p1.first < p2.first;
      }
  };
  template<typename T, typename U> class CompareSecond {
    public:
      bool operator()(const std::pair<T, U>& p1, const std::pair<T, U>& p2) {
  ...
  template<typename T> T min2(T const x, T const y) throw () {
    return (x < y) ? x : y;
  }

  template<typename T> T max2(T const x, T const y) throw () {
    return (x > y) ? x : y;
  }
  ...
  inline std::string str(double number, size_t dec_places, size_t precision) {
    ...
    ss << std::fixed << std::setprecision(precision) << std::setfill('0')
        << std::setw(dec_places + precision + (precision > 0) + (number < 0))
        << number;
    return ss.str();
  }
  ...
  template<typename T> std::vector<T> parse_sequence(const std::string& spec,
      T last = std::numeric_limits < T > ::max()) {
    std::vector<T> V;
    if (!spec.size())
      throw Exception("unsigned integer sequence specifier is empty");
    ...
    try {
      do {
        ...
        if (str == "end") {
          ...
          num[i] = to<T>(spec.substr(start, end - start));
      ...
      throw Exception(E,
          "can't parse unsigned integer sequence specifier \"" + spec + "\"");
    }
