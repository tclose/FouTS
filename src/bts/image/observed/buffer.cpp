...
namespace BTS {
  ...
  namespace Image {
    ...
    namespace Observed {
      ...
      void                         Buffer::load(const std::string& location, const Diffusion::Encoding::Set& default_encodings) {

        MR::Image::Header header(location);

        if (header.ndim() != 4)
          throw Exception ("dwi image should contain 4 dimensions, found " + str(header.ndim()) + ".");
        ...
        if (header.get_DW_scheme().rows()) {
          ...
          throw Exception ("No encodings found in loaded image, and no defaults supplied.");

        if (header.dim(3) != (int) encodings.size())
          throw Exception ("Number of encoding directions in loaded image (" + str(header.dim(DW)) + ") do not match that in encoding file (" + str(encodings.size()) + ").");
