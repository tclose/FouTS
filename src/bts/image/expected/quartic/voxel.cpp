...
//----------------------//
        ...
        double         Voxel::interpolate(const Coord& pos) {
          ...
          if (disp.lower_bounded(-1.0) && disp.upper_bounded(1.0)) {
            ...
            for (size_t dim_i = 0; dim_i < 3; ++dim_i)
              interpolate[dim_i] = MR::Math::pow4(disp[dim_i]) - 2.0 * MR::Math::pow2(disp[dim_i]) + 1;
        ...
        double         Voxel::interpolate(const Coord& pos, Coord& pos_gradient) {
          ...
          if (disp.lower_bounded(-1.0) && disp.upper_bounded(1.0)) {
            ...
            pos_gradient[X] =  (4.0 * MR::Math::pow3(disp[X]) - 4.0 * disp[X]) * interpolate[Y] * interpolate[Z];
            pos_gradient[Y] =  (4.0 * MR::Math::pow3(disp[Y]) - 4.0 * disp[Y]) * interpolate[X] * interpolate[Z];
            pos_gradient[Z] =  (4.0 * MR::Math::pow3(disp[Z]) - 4.0 * disp[Z]) * interpolate[X] * interpolate[Y];
        ...
        double              Voxel::interpolate(const Fibre::Strand::BasicSection& section, Fibre::Strand::BasicSection& gradient) {
...
//------------------//
        ...
        double        	   Voxel::interpolate(const Coord& pos, Coord& gradient, Coord::Tensor& hessian) {
          ...
          if (disp.lower_bounded(-1.0) && disp.upper_bounded(1.0)) {
            ...
            hessian[X][X] = (12.0 * MR::Math::pow2(disp[X]) - 4.0) * interpolate[Y] * interpolate[Z];
            hessian[X][Y] = hessian[Y][X] = partial[X] * partial[Y] * interpolate[Z];
            hessian[X][Z] = hessian[Z][X] = partial[X] * interpolate[Y] * partial[Z];

            hessian[Y][Y] = interpolate[X] * (12.0 * MR::Math::pow2(disp[Y]) - 4.0) * interpolate[Z];
            hessian[Y][Z] = hessian[Z][Y]  = interpolate[X] * partial[Y] * partial[Z];

            hessian[Z][Z] = interpolate[X] * interpolate[Y] * (12.0 * MR::Math::pow2(disp[Z]) - 4.0);
        ...
        double              Voxel::interpolate(const Fibre::Strand::BasicSection& section, Fibre::Strand::BasicSection& gradient, Fibre::Strand::BasicSection::Tensor& hessian) {
          ...
          double pos_interp = interpolate(section.position(), pos_interp_gradient, interp_hessian);
        ...
        double              Voxel::interpolate(const Fibre::Tractlet::Section& section, Fibre::Tractlet::Section& gradient, Fibre::Tractlet::Section::Tensor& hessian) {

          Fibre::Strand::BasicSection::Tensor strand_hessian;

          double interp = interpolate((const Fibre::Strand::BasicSection&)section, (Fibre::Strand::BasicSection&)gradient, strand_hessian);
