  ...
  namespace Image {
    ...
    namespace Expected {
      ...
      namespace Realistic {
        ...
        double         Expected::Realistic::Voxel::interpolate(const Coord& pos) {
          ...
          if (disp.lower_bounded(-image->get_extent()) && disp.upper_bounded(image->get_extent())) {
            ...
            if ((disp[Z] < 1) && (disp[Z] > -1))
              interpolate[Z] = (MR::Math::pow4(disp[Z]) - 2.0 * MR::Math::pow2(disp[Z]) + 1);
            else
              interpolate[Z] = 0.0;
        ...
        double         Expected::Realistic::Voxel::interpolate(const Coord& pos, Coord& pos_gradient) {

          throw Exception ("Expected::Realistic::Voxel::interpolate(const Coord&, Coord&) not implemented yet.");
