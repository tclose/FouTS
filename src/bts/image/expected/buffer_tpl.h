    ...
    namespace Expected {
      ...
      template<typename T> class Buffer_tpl: public Observed::Buffer_tpl<T> {
        ...
        public:
          ...
          std::map<size_t, std::vector<Fibre::Tractlet::Section> >
              tractlet_sections;

          //Holds all strand or tractlet sections used to generate the image. Storage here allows data structures to be
          //reused between image generations.
          Reference::Buffer<Fibre::Strand::Section>::Set
              strand_section_references;
          Reference::Buffer<Fibre::Tractlet::Section>::Set
              tractlet_section_references;
        ...
        public:
          ...
          template<typename U> typename Image::Reference::Buffer<
              typename U::Section>::Set& expected_image_with_references(
              const typename U::Set& fibres);

          template<typename U> void part_image(const U& fibre, std::vector<
              typename U::Section>& path, Image::Reference::Buffer<
              typename U::Section>& section_reference);
          ...
          template<typename U> void part_image(const U& fibre,
              Container::Buffer<U>& gradients, Container::Buffer<
                  typename U::Tensor>& hessians);
        ...
        protected:
...

          {
            ...
            if (!number_length_sections)
              throw Exception(
                  "Number of length sections in image cannot be zero.");
          ...
          Index voxel_centre_coord(const Coord& point) {
            ...
            Index coord((int) floor(offset_point[X]), (int) floor(
                offset_point[Y]), (int) floor(offset_point[Z]));
            return coord;
          }
