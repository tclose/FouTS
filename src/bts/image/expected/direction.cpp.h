...
namespace BTS {
    ...
    namespace Image {
        ...
        template<typename T> double Expected::Direction::signal_tpl(
                const T& section, Fibre::Strand::BasicSection& gradient) {
            ...
            const Triple<double>& diffusion_gradient =
                    section.precalc_weight_gradients[response->get_index()];
...
#ifndef GRADIENT_NOT_REQUIRED
            signal_gradient(section, gradient, diffusion_weighting, interpolation_weighting,
                    diffusion_gradient, interpolation_gradient);
#endif
        ...
        template<typename T> double Expected::Direction::signal_tpl(
            ...
            const Coord::Tensor& diffusion_hessian =
                    section.precalc_weight_hessians[response->get_index()];
...
#ifndef HESSIAN_NOT_REQUIRED
            signal_hessian(section, hessian, diffusion_weighting, interpolation_weighting,
                    diffusion_gradient, interpolation_gradient, diffusion_hessian,
                    interpolation_hessian);
#endif
        ...
        inline void Expected::Direction::signal_gradient(
                const Fibre::Strand::Section& section, Fibre::Strand::BasicSection& gradient,
                double diffusion_weighting, double interpolation_weighting,
                const Triple<double>& diffusion_gradient,
                const Fibre::Strand::BasicSection& interpolation_gradient) {
            
            gradient.tangent() = (Coord) diffusion_gradient
                    * (section.intensity() * interpolation_weighting)
                                 + interpolation_gradient.tangent() * (section.intensity()
                                         * diffusion_weighting);
            
            gradient.position() = interpolation_gradient.position()
                    * (section.intensity() * diffusion_weighting);
        ...
        inline void Expected::Direction::signal_gradient(
                const Fibre::Tractlet::Section& section, Fibre::Strand::BasicSection& gradient,
                double diffusion_weighting, double interpolation_weighting,
                const Triple<double>& diffusion_gradient,
                const Fibre::Strand::BasicSection& interpolation_gradient) {
            
            //Call the Fibre::Strand::Section version of the function first, then process the additional parameters.
            signal_gradient((const Fibre::Strand::Section&) section,
                    (Fibre::Strand::BasicSection&) gradient, diffusion_weighting,
                    interpolation_weighting, diffusion_gradient,
                    (const Fibre::Strand::BasicSection&) interpolation_gradient);
...
//      }
        ...
        inline void Expected::Direction::signal_hessian(
                const Fibre::Strand::Section& section, Fibre::Strand::BasicSection::Tensor& hessian,
                double diff_weighting, double interp_weighting, const Triple<double>& diff_gradient,
            ...
            hessian.intensity().tangent() = (Coord) diff_gradient * interp_weighting
                    + interp_gradient.tangent() * diff_weighting;
            ...
            hessian.position(X).position() = interp_hessian.position(X).position() * diff_weighting
                                             * section.intensity();
            hessian.position(Y).position() = interp_hessian.position(Y).position() * diff_weighting
                                             * section.intensity();
            hessian.position(Z).position() = interp_hessian.position(Z).position() * diff_weighting
                                             * section.intensity();
            ...
            Coord::Tensor cross_hessian =
                    section.intensity() * (interp_gradient.position().outer(diff_gradient)
        ...
        inline void Expected::Direction::signal_hessian(
                ...
                Fibre::Strand::BasicSection::Tensor& hessian, double diffusion_weighting,
                double interpolation_weighting, const Triple<double>& diffusion_gradient,
                const Fibre::Strand::BasicSection& interpolation_gradient,
                const Coord::Tensor& diffusion_hessian,
            ...
            signal_hessian((const Fibre::Strand::Section&) section,
                    (Fibre::Strand::BasicSection::Tensor&) hessian, diffusion_weighting,
                    interpolation_weighting, diffusion_gradient,
                    (const Fibre::Strand::Section&) interpolation_gradient, diffusion_hessian,
                    (const Fibre::Strand::BasicSection::Tensor&) interpolation_hessian);
        ...
        inline std::ostream& operator<<(std::ostream& stream,
            ...
            stream << "Voxel: " << direction.voxel << ", Response: " << direction.response
                   << std::endl;
