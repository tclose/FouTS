    ...
    namespace Expected {
      ...
      namespace Gaussian {
        ...
        double         Voxel::interpolate(const Coord& pos) {
          ...
          Coord interpolate ( exp(- MR::Math::pow2(disp[X]) / (2.0 * image->gauss_exp_var())), exp(- MR::Math::pow2(disp[Y]) / (2.0 * image->gauss_exp_var())), exp(- MR::Math::pow2(disp[Z]) / (2.0 * image->gauss_exp_var())));
