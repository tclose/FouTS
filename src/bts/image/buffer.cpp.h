  ...
  namespace Image {
    ...
    template <typename T> T&                       Buffer_tpl<T>::operator() (const Index& c) {
      ...
      if (enforce_bounds && !in_bounds(c))
        throw Exception ("Coordinate " + str(c) + " is out of image bounds " + str(dimensions) + " (set 'enforce_bounds=false' if this is intentional).");
    ...
    template <typename T> const T&                 Buffer_tpl<T>::operator() (const Index& c) const {
      ...
      if (it == voxels.end())
        throw Exception ("Coordinate " + str(c) + " has not been initialised and can't be since this buffer is const. Suggest using 'is_empty(const Coord&)' to check if it has been initialised first.");
    ...
    template <typename T> template <typename U> Buffer_tpl<T>&           Buffer_tpl<T>::operator+= (const Buffer_tpl<U>& buff) {

      if (dims() != buff.dims())
        throw Exception ("Buffer dimensions do not match, " + str(buff.dims()) + " and " + str(dims()) + ".");

      for (typename Buffer_tpl<U>::const_iterator vox_it = buff.begin(); vox_it != buff.end(); ++vox_it) {
    ...
    template <typename T> Buffer_tpl<T>           Buffer_tpl<T>::operator+ (const Buffer_tpl<T>& buff) const {
    ...
    template <typename T> template <typename U> Buffer_tpl<T>&           Buffer_tpl<T>::operator*= (const U& M) {
    ...
    template <typename T> std::ostream& operator<< (std::ostream& stream, const Image::Buffer_tpl<T>& B) {
      ...
      for (typename Image::Buffer_tpl<T>::const_iterator vox_it = B.begin(); vox_it != B.end(); ++vox_it)
