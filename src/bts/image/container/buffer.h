      ...
      template <typename T> class Buffer : public Image::Buffer_tpl< Container::Voxel<T> > {
        ...
        public:
          ...
          Buffer (Triple<size_t> dims, size_t num_encodings)
            : Image::Buffer_tpl< Voxel<T> >(dims,false), number_directions(num_encodings) {}


          Buffer (const Buffer& B)
            : Image::Buffer_tpl< Voxel<T> >(B), number_directions(B.number_directions) {}
          ...
          Buffer&                           operator= (const Buffer& B)
            { Image::Buffer_tpl< Voxel<T> >::operator= (B); number_directions = B.number_directions; return *this; }
          ...
          Buffer&                           clear ()
            { this->Image::Buffer_tpl< Voxel<T> >::clear(); return *this; }

          Buffer&                           zero ()
            { this->Image::Buffer_tpl< Voxel<T> >::zero(); return *this; }

          void                              reset(const Triple<size_t>& dimensions, size_t num_encodings)
            { Image::Buffer_tpl< Voxel<T> >::reset(dimensions); number_directions = num_encodings; }

          void                              reset(const Triple<size_t>& dimensions, size_t num_encodings, bool enforce_bounds)
            { Image::Buffer_tpl< Voxel<T> >::reset(dimensions, enforce_bounds); number_directions = num_encodings; }

          void                              quick_clear ()
            { for (typename Image::Buffer_tpl< Voxel<T> >::iterator vox_it = this->begin(); vox_it != this->end(); ++vox_it) vox_it->second.quick_empty(); }

          inline Voxel<T>&                  operator() (int x, int y, int z)
            { return operator()(Index(x,y,z)); }

          inline Voxel<T>&                  operator() (const Index& c)
            { Voxel<T>& vox = Image::Buffer_tpl< Voxel<T> >::operator()(c); vox.undo_empty(); return vox; }

          bool                              is_empty(const Index& c) const
            { typename Image::Buffer_tpl< Voxel<T> >::const_iterator vox_it = this->voxels.find(c); return (vox_it == this->end()) ? true : vox_it->second.is_empty(); }
