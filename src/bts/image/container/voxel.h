      ...
      template <typename T> class Voxel : public Image::Voxel<T> {
        ...
        public:
          ...
          Voxel&    operator=(const Voxel<T>& v)
            { this->Image::Voxel<T>::operator=(v); fake_empty = v.fake_empty; return *this; }

          Voxel&    operator*=(double M)
            { assert(!fake_empty); LOOP(this->operator[](encode_i) *= M;); return *this; }

          Voxel&    operator/=(double M)
            { assert(!fake_empty); LOOP(this->operator[](encode_i) /= M;); return *this; }

          Voxel&    zero()
            { assert(!fake_empty); this->Image::Voxel<T>::zero(); return *this; }
