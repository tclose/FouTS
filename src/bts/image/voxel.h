...
namespace BTS {
  ...
  namespace Image {
    ...
    template <typename T> class Voxel {
      ...
      public:
        ...
        Voxel&            operator= (const Voxel& voxel)
          { intensities = voxel.intensities;return *this; }
        ...
        Voxel&            negate()
        { LOOP(operator[](encode_i) = -operator[](encode_i);) return *this; }


        Voxel&            operator+= (const Voxel& v)
          { LOOP(operator[](encode_i) += v[encode_i];) return *this; }


        Voxel&            operator+= (T f)
          { LOOP(operator[](encode_i) += f;) return *this; }
        ...
        Voxel             operator+ (const Voxel& v) const 
          { Voxel answer(*this); answer += v; return answer; }


        Voxel             operator+ (T f) const
          { Voxel answer(*this); answer += f; return answer; }
        ...
        Voxel&            zero()
          { LOOP(BTS::zero(operator[](encode_i));) return *this; }
    ...
    template <typename T> Voxel<T>            operator+ (T f, const Voxel<T>& v)
      { Voxel<T> answer(v); answer += f; return answer; }
