        ...
        template<typename T> class Buffer_tpl {
            ...
            public:
                ...
                Buffer_tpl<T>& operator=(const Buffer_tpl<T>& B) {
                    ...
                    voxels = B.voxels;
                    enforce_bounds = B.enforce_bounds;
                    return *this;
                }
    ...
    namespace Image {
        ...
        class Buffer: public Buffer_tpl<Voxel<double> > {
            ...
            public:
                ...
                Buffer(const Triple<size_t>& dimensions, size_t num_encodings, bool enforce_bounds =
                        true)
                ...
                template<typename T> Buffer(const Buffer_tpl<T>& B)
                        : Buffer_tpl<Voxel<double> >(B.dims(), B.bounds_are_enforced()), num_encodings(
                                  0) {
                    
                    int read_num_encodings = -1;
                    
                    for (typename Buffer_tpl<T>::const_iterator vox_it = B.begin();
                            vox_it != B.end(); ++vox_it) {
                        ...
                        if (read_num_encodings == -1)
                            ...
                            throw Exception(
                                    "Number of encodings in copied voxels do not match (" + str(
                                            read_num_encodings)
                                    + " and " + str(vox_it->second.num_encodings()) + ").");
                ...
                Buffer& operator=(const Buffer& B) {
                    this->Buffer_tpl<Voxel<double> >::operator=(B);
                    return *this;
                }
