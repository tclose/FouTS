  ...
  class Coord::Tensor : public MR::Math::Matrix<double> {
    ...
    public:
      ...
      static Tensor       create_diagonal(double value)
        { Tensor t; t.diagonal() = value; return t; }
    ...
    public:
      ...
      Tensor(const Coord& c)
        : MR::Math::Matrix<double>(3,3) { invalidate(); }

      Tensor(const MR::Math::Matrix<double>::View& view)
        : MR::Math::Matrix<double>(view) { assert(rows()==3); assert(columns() == 3); }
      ...
      Tensor&      invalidate()
        { for (size_t i = 0; i < 3; ++i) for (size_t j = 0; j < 3; ++j) operator()(i,j) = NAN; return *this; }
