...
namespace BTS {
  ...
  namespace Prob {
    ...
    namespace Test {
      ...
      Landscape::Peak::Peak(size_t ndims,
                        const MCMC::State& centre,
                        const MCMC::State::Tensor& widths,
                        double height,
                        size_t type)
            : type(type),
              ndims(ndims),
...

      {
        ...
        if (ndims!= widths.rows())
          throw Exception ("Size of centre (" + str(ndims) + ") and length of widths (" + str(widths.rows()) + ") do not match.");
      ...
      double Landscape::Peak::log_prob(const MCMC::State& point, MCMC::State& gradient, MCMC::State::Tensor& hessian) {
      ...
      double  Landscape::Peak::log_prob(const MCMC::State& point, MCMC::State& gradient, MCMC::State::Tensor& hessian, std::vector<MCMC::State::Tensor>& rank3_hessian) {
        ...
        if (type == GAUSSIAN)
          ...
          throw Exception ("Hessian or a 3rd order Hessian is not useful for pyramid type peaks.");
        else
          throw Exception ("Unrecognised peak type, " + str(type) + ".");
      ...
      double Landscape::Peak::gaussian_log_prob(const MCMC::State& point, MCMC::State& gradient, MCMC::State::Tensor& hessian) {
      ...
      double Landscape::Peak::gaussian_log_prob(const MCMC::State& point, MCMC::State& gradient, MCMC::State::Tensor& hessian, std::vector<MCMC::State::Tensor>& rank3_hessian) {
        ...
        MR::Math::Matrix<double> outer_hess_part = Math::outer(widths_mult_disp, widths_mult_disp);
        ...
        for (size_t dim_i = 0; dim_i < ndims; dim_i++) {
          ...
          MR::Math::Matrix<double> outer_hess_part2 = Math::outer(widths.column(dim_i), widths_mult_disp);
      ...
      double Landscape::Peak::pyramid_log_prob(const MCMC::State& point, MCMC::State& gradient) {
      ...
      std::ostream&                  operator<< (std::ostream& stream, const Landscape::Peak& peak) {
        ...
        stream << "centre: " << peak.centre  << ", height: " << peak.height << " type: " << peak.type << ", widths: " << peak.widths << std::endl;
