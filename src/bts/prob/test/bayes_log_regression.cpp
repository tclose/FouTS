...
namespace BTS {
  ...
  namespace Prob {
    ...
    namespace Test {
      ...
      const char* BayesLogRegression::DATA_LOCATION_DEFAULT = "/home/tclose/data/prob/test/bayes_log_regression/australian.txt";
      const size_t BayesLogRegression::POLY_ORDER_DEFAULT = 1;
      const float BayesLogRegression::PRIOR_VARIANCE_DEFAULT = 100.0;


      BayesLogRegression::BayesLogRegression(const std::string& location, size_t poly_order, double alpha)
      : poly_order(poly_order), alpha(alpha) {

        MR::Math::Matrix<double> loaded_data (location);
        MR::Math::Matrix<double> X = loaded_data.sub(0, loaded_data.rows(), 0, loaded_data.columns()-1);
        ...
        for (size_t poly_i = 0; poly_i < poly_order; ++poly_i)
          for (size_t col_i = 0; col_i < X.columns(); ++col_i)
            for (size_t row_i = 0; row_i < X.rows(); ++row_i)
              XX(row_i, poly_i*X.columns() + col_i + 1) = MR::Math::pow(X(row_i, col_i), poly_i+1);
      ...
      double BayesLogRegression::log_prob (const MR::Math::Vector<double>& w) {
        ...
        for (size_t d = 0; d < D; ++d)
          lprior -= 0.5 * MR::Math::log(2.0 * M_PI * alpha) - MR::Math::pow2(w[d]) / (2.0 * alpha);
      ...
      double BayesLogRegression::log_prob (const MR::Math::Vector<double>& w, MR::Math::Vector<double>& d_w) {
      ...
      double BayesLogRegression::log_prob_and_fisher (const MR::Math::Vector<double>& w, MR::Math::Vector<double>& d_w, MR::Math::Matrix<double>& G) {
      ...
      double BayesLogRegression::log_prob_and_fisher (const MR::Math::Vector<double>& w, MR::Math::Vector<double>& d_w, MR::Math::Matrix<double>& G, std::vector<MCMC::State::Tensor>& d_G) {
