...
namespace BTS {
  ...
  namespace Prob {
    ...
    namespace Test {
      ...
      Landscape::Landscape(const std::string& location, double roi_radius, double barrier_rate)
            : roi_radius(roi_radius), barrier_rate(barrier_rate) { load(location); }


      Landscape Landscape::randomly_generate( size_t ndims,
                                              size_t num_peaks,
        ...
        for (size_t peak_i = 0; peak_i < num_peaks; peak_i++) {
          ...
          for (size_t dim_i = 0; dim_i < 2; dim_i++) {
            double width = MR::Math::sqrt(MR::Math::pow2(gsl_ran_gaussian(rand_gen, peak_width_sigma) + peak_width_mu * (1.0 - peak_width_min))
                                        + MR::Math::pow2(gsl_ran_gaussian(rand_gen, peak_width_sigma) + peak_width_mu * (1.0 - peak_width_min))) / M_SQRT2;
      ...
      double                 Landscape::log_prob(const MCMC::State& state, MCMC::State& gradient) {
        ...
        for (std::vector<Peak>::iterator peak_it = peaks.begin(); peak_it != peaks.end(); ++peak_it) {
        ...
        if (state.norm() >  roi_radius) {
          ...
          gradient -= 2.0 * barrier_rate * state * (state.norm() - roi_radius) / state.norm();
      ...
      double                 Landscape::log_prob(const MCMC::State& state, MCMC::State& gradient, MCMC::State::Tensor& hessian) {
        ...
        if (state.norm() >  roi_radius) {
          ...
          hess_part *= 2.0 * barrier_rate * (-1.0 * (state.norm() - roi_radius) / MR::Math::pow3(state.norm()) + 1 / state.norm2());
          hessian  -=  hess_part;

          for (size_t elem_i = 0; elem_i < state.size(); elem_i++)
            hessian(elem_i, elem_i) -= 2 * barrier_rate * (state.norm() - roi_radius)/ state.norm();
      ...
      double                Landscape::log_prob(const MCMC::State& state, MCMC::State& gradient, MCMC::State::Tensor& hessian, std::vector<MCMC::State::Tensor>& rank3_hessian) {
        ...
        for (std::vector<Peak>::iterator peak_it = peaks.begin(); peak_it != peaks.end(); ++peak_it) {
          ...
          log_prob += peak_it->log_prob(state, peak_gradient, peak_hessian, peak_rank3_hessian);
        ...
        if (state.norm() >  roi_radius) {
          ...
          for (size_t dim_i = 0; dim_i < state.size(); dim_i++) {
            ...
            d_scalar_hess *= -2.0 * barrier_rate * state[dim_i] * (3.0 * (state.norm() - roi_radius) / MR::Math::pow5(state.norm()) - 3.0 / MR::Math::pow4(state.norm()));

            MR::Math::Matrix<double> d_outer_hess (state.size(), state.size());
            d_outer_hess = 0.0;
            d_outer_hess.column(dim_i) = state;
            d_outer_hess.row(dim_i) += state;
            d_outer_hess *= -2.0 * barrier_rate * (-1.0 * (state.norm() - roi_radius) / MR::Math::pow3(state.norm()) + 1 / state.norm2());

            d_hessian += d_scalar_hess;
            d_hessian += d_outer_hess;

            for (size_t dim_i2 = 0; dim_i2 < state.size(); dim_i2++)
              d_hessian(dim_i2, dim_i2) -= 2.0 * barrier_rate * state[dim_i] * (1/state.norm2() - (state.norm() - roi_radius)/ MR::Math::pow3(state.norm()));
      ...
      void                  Landscape::load(const std::string& location) {
        ...
        while (reader.next(peak)) {
          ...
          if (!ndims)
            ...
            throw Exception ("Peak dimension (" + str(peak.num_dims()) + ") does not match overall dimension (" + str(ndims) + ").");
        ...
        if (!ndims)
          throw Exception ("Loaded test landscape ('" + location + "') does not have any dimension.");
      ...
      std::ostream&               operator<< (std::ostream& stream, const Landscape& test) {
        ...
        for (std::vector<Landscape::Peak>::const_iterator peak_it = test.peaks.begin(); peak_it != test.peaks.end(); ++peak_it)
