...
namespace BTS {
	...
	namespace Prob {
	  ...
	  template <typename T> double  Likelihood::Gaussian::log_prob_and_fisher_tpl(const typename T::Set& fibres, typename T::Set& gradient, typename T::Set::Tensor& fisher_info, std::vector<typename T::Set::Tensor>& fisher_info_gradients) {

      size_t fibres_vector_size = fibres.vsize();

      typename Image::Reference::Buffer<typename T::Section>::Set& section_references = exp_image->expected_image_with_references(fibres);
...
//      coords.insert(Image::Index(1,1,1));
      ...
      for (std::set<Image::Index>::iterator coord_it = coords.begin(); coord_it != coords.end(); ++coord_it) {
        ...
        for (size_t encode_i = 0; encode_i < exp_image->num_encodings(); encode_i++) {
          ...
          if (exp_image->encoding(encode_i).b_value()) {
            ...
            lprob += log_prob(exp_image->operator()(*coord_it)[encode_i], observed, d_lprob[encode_i], d2_lprob2[encode_i]);
        ...
        for (size_t fibre_i = 0; fibre_i < fibres.size(); fibre_i++)
          if (!section_references[fibre_i].is_empty(*coord_it)) {
            ...
            for (typename std::vector<typename T::Section*>::iterator section_it = section_references[fibre_i](*coord_it).begin();
                                            section_it != section_references[fibre_i](*coord_it).end(); ++section_it)
...
//            }
            ...
            for (std::vector<size_t>::iterator fibre_i_it = nonzero_fibres.begin(); fibre_i_it != nonzero_fibres.end(); ++fibre_i_it) {
              ...
              std::vector<typename T::Section*>& fibre_section_references = section_references[*fibre_i_it](*coord_it);

              for (typename std::vector<typename T::Section*>::iterator section_it = fibre_section_references.begin();
                ...
                voxel.direction(encode_i).signal(**section_it, section_gradient, section_hessian);

                section_gradient.unnormalize_gradient(exp_image->vox_lengths());
                section_hessian.unnormalise_hessian(exp_image->vox_lengths());

                direction_gradient.add_section_gradient(fibres[*fibre_i_it], **section_it, section_gradient);
                direction_hessian.add_section_hessian(fibres[*fibre_i_it], **section_it, section_gradient, section_hessian);
            ...
            for (size_t nonzero_i1 = 0; nonzero_i1 < nonzero_fibres.size(); ++nonzero_i1) {
              ...
              for (size_t i1 = 0; i1 < gradient1.size(); i1++)
                for (size_t i2 = 0; i2 < gradient1.size(); i2++) {
                  fisher_info(start1 + i1, start1 + i2) += d2_lprob2[encode_i] * gradient1[i1] * gradient1[i2];
//                  std::cout << "fisher_info(" << start1 + i1 << ", " << start1 + i2 << ") = " << fisher_info(start1 + i1, start1 + i2) << " (" << d2_lprob2[encode_i] * gradient1[i1] * gradient1[i2] << ")" << std::endl;
                ...
                for (size_t i2 = 0; i2 < gradient1.size(); i2++)
                  for (size_t i3 = 0; i3 < gradient1.size(); i3++)
                    fisher_info_gradients[start1 + i1](start1 + i2, start1 + i3)
                          +=  d2_lprob2[encode_i] * (hessian1(i2, i1) * gradient1[i3] + gradient1[i2] * hessian1(i3,i1));


              for (size_t nonzero_i2 = nonzero_i1+1; nonzero_i2 < nonzero_fibres.size(); ++nonzero_i2) {
                ...
                for (size_t i1 = 0; i1 < gradient1.size(); i1++)
                  for (size_t i2 = 0; i2 < gradient2.size(); i2++) {
                    ...
                    double incr = d2_lprob2[encode_i] * gradient1[i1] * gradient2[i2];
                ...
                for (size_t i1 = 0; i1 < gradient1.size(); i1++)
                  for (size_t i2 = 0; i2 < gradient1.size(); i2++)
                    for (size_t i3 = 0; i3 < gradient2.size(); i3++) {
                      ...
                      double incr = d2_lprob2[encode_i] * hessian1(i2, i1) * gradient2[i3];

                      fisher_info_gradients[start1 + i1](start1 + i2, start2 + i3) += incr;
                      fisher_info_gradients[start1 + i1](start2 + i3, start1 + i2) += incr;
    ...
    template <typename T> double  Likelihood::Gaussian::log_prob_and_fisher_tpl(const typename T::Set& fibres, typename T::Set& gradient, typename T::Set::Tensor& fisher_info) {
    ...
    //            }
            ...
            for (size_t fibre_i1 = 0; fibre_i1 < fibres.size(); fibre_i1++) {
              if (!section_references[fibre_i1].is_empty(*coord_it)) {
                ...
                for (typename std::vector<typename T::Section*>::iterator section_it = section_references[fibre_i1](*coord_it).begin();
                                          section_it != section_references[fibre_i1](*coord_it).end(); ++section_it) {

                  Fibre::Strand::BasicSection section_gradient;

                  voxel.direction(encode_i).signal(**section_it, section_gradient);
                  section_gradient.unnormalize_gradient(exp_image->vox_lengths());
                  gradient1.add_section_gradient(fibres[fibre_i1], **section_it, section_gradient);
    ...
    //                  }
                ...
                for (size_t fibre_i2 = 0; fibre_i2 < fibres.size(); fibre_i2++) {
                  if (!section_references[fibre_i2].is_empty(*coord_it)) {
                    ...
                    gradient2.zero();
                    for (typename std::vector<typename T::Section*>::iterator section_it = section_references[fibre_i2](*coord_it).begin();
                                              section_it != section_references[fibre_i2](*coord_it).end(); ++section_it) {

                      Fibre::Strand::BasicSection section_gradient;

                      voxel.direction(encode_i).signal(**section_it, section_gradient);
                      section_gradient.unnormalize_gradient(exp_image->vox_lengths());
                      gradient2.add_section_gradient(fibres[fibre_i2], **section_it, section_gradient);
                    ...
                    for (size_t fibre_elem_i1 = 0; fibre_elem_i1 < gradient_vector1.size(); fibre_elem_i1++) {

                      size_t elem_i1 = fibre_block_start[fibre_i1] + fibre_elem_i1;

                      for (size_t fibre_elem_i2 = 0; fibre_elem_i2 < gradient_vector2.size(); fibre_elem_i2++) {

                        size_t elem_i2 = fibre_block_start[fibre_i2] + fibre_elem_i2;

                        fisher_info(elem_i1, elem_i2) += gradient_vector1[fibre_elem_i1] * gradient_vector2[fibre_elem_i2] * d2_lprob2[encode_i];
