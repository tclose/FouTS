  ...
  namespace Prob {
    ...
    Likelihood*                       Likelihood::factory(const std::string& type,
      ...
      if (type == "gaussian")
        ...
        likelihood = new Gaussian (obs_image, exp_image, assumed_snr, b0_include, outside_scale, ref_b0, ref_signal);

      else if (type == "one_sided_gaussian")

        likelihood = new OneSidedGaussian (obs_image, exp_image, assumed_snr, b0_include, outside_scale, ref_b0, ref_signal);
        ...
        if (!diffusion_model.includes_isotropic())
          throw Exception ("Only diffusion models that include isotropic components can be used with Rician noise model.");

        if (!exp_image->bounds_are_enforced())
          throw Exception ("Enforce bounds must be set with Rician noise model to avoid -inf values.");

        likelihood = new Rician (obs_image, exp_image, assumed_snr, b0_include, outside_scale, ref_b0, ref_signal);

      } else
        throw Exception ("Unrecognised likelihood type '" + type + "' passed to option '-like_type'.");
    ...
    Likelihood::Likelihood (
                const Image::Observed::Buffer& observed_image,
                ...
                obs_image(observed_image),
                exp_image(expected_image->clone()),
                b0_include(b0_include)
              {

      if (!expected_image->dims_match(obs_image))
        throw Exception ("Expected image dimensions (" + str(expected_image->dims()) + ") do not match and observed image (" + str(obs_image.dims()) + ").");
    ...
    void              Likelihood::set_assumed_snr(double assumed_snr, const std::string& ref_b0, double reference_signal) {
      ...
      if (!isnan(reference_signal))
        ...
        throw Exception ("Unrecognised value for '-like_ref_b0' ('" + ref_b0 + "'), can be either 'average', or 'max'.");
