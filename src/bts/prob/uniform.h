    ...
    namespace Prob {
        ...
        class Uniform {
            ...
            public:
                ...
                double log_prob(const Fibre::Tractlet::Set& strand,
                ...
                double log_prob_and_fisher(const MCMC::State& state, MCMC::State& gradient,
                                           MCMC::State::Tensor& fisher) {
                    gradient.zero();
                    fisher.zero();
                    return 0.0;
                }
                ...
                double log_prob_and_fisher(const MCMC::State& state, MCMC::State& gradient,
                    ...
                    gradient.zero();
                    fisher.zero();
                    for (size_t i = 0; i < fisher_gradient.size(); ++i)
                        fisher_gradient[i] = 0.0;
                    return 0.0;
                }
                
                double component_log_prob(const std::string&, const FTS::Fibre::Strand& strand,
                                          FTS::Fibre::Strand& gradient) {
                    gradient.zero();
                    return 0.0;
                }
                
                double component_log_prob(const std::string&,
                                          const FTS::Fibre::Strand::Set& strands,
                                          FTS::Fibre::Strand::Set& gradient) {
                    gradient.zero();
                    return 0.0;
                }
                
                double component_log_prob(const std::string&, const FTS::Fibre::Tractlet& tractlet,
                                          FTS::Fibre::Tractlet& gradient) {
                    gradient.zero();
                    return 0.0;
                }
                
                double component_log_prob(const std::string&,
                                          const FTS::Fibre::Tractlet::Set& tractlets,
                                          FTS::Fibre::Tractlet::Set& gradient) {
                    gradient.zero();
                    return 0.0;
                }
