        ...
        namespace PriorComponent {
            ...
            double Thinness::log_prob(const Fibre::Tractlet tract, Fibre::Tractlet gradient) {
                ...
                if (!gradient.degree()) {
                    ...
                    throw Exception(
                            "Tractlet degree (" + str(tract.degree())
                ...
                gradient.set_acs(
                        2.0 * scale * MR::Math::pow2(tract(0, 1).norm()) / MR::Math::pow3(
                                tract.acs()));
                gradient(0, 1)[X] = -2.0 * scale * tract(0, 1)[X] * tract(0, 1).norm()
                        / MR::Math::pow2(tract.acs());
                gradient(0, 1)[Y] = -2.0 * scale * tract(0, 1)[Y] * tract(0, 1).norm()
                        / MR::Math::pow2(tract.acs());
                gradient(0, 1)[Z] = -2.0 * scale * tract(0, 1)[Z] * tract(0, 1).norm()
                        / MR::Math::pow2(tract.acs());
