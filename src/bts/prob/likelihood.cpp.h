...
namespace BTS {
  ...
  namespace Prob {
    ...
    template <typename T> double  Likelihood::log_prob_tpl(const typename T::Set& fibres, typename T::Set& gradient) {
      ...
      for (std::set<Image::Index>::iterator coord_it = coords.begin(); coord_it != coords.end(); ++coord_it) {
        ...
        for (size_t encode_i = 0; encode_i < exp_image->num_encodings(); encode_i++) {
          ...
          if (b0_include == "full"  || exp_image->encoding(encode_i).b_value()) {
            ...
            lprob += log_prob(exp_image->operator()(*coord_it)[encode_i], observed, d_lprob[encode_i]);
        ...
        for (size_t fibre_i = 0; fibre_i < fibres.size(); fibre_i++) {
          ...
          if (!section_references[fibre_i].is_empty(*coord_it)) {
            ...
            std::vector<typename T::Section*>& voxel_section_references = section_references[fibre_i](*coord_it);

            for (typename std::vector<typename T::Section*>::iterator section_it = voxel_section_references.begin();
              ...
              for (size_t encode_i = 0; encode_i < exp_image->num_encodings(); encode_i++) {
                ...
                if (b0_include == "full"  || exp_image->encoding(encode_i).b_value()) {
                  ...
                  gradient[fibre_i].add_section_gradient(fibres[fibre_i], section, section_gradient);
    ...
    template <typename T> double  Likelihood::log_prob_tpl(const typename T::Set& fibres, typename T::Set& gradient, typename T::Set::Tensor& hessian) {

      typename Image::Container::Buffer<T>::Set& buffer_gradients                 = get_recycled_gradients<T>();
      typename Image::Container::Buffer<typename T::Tensor>::Set& buffer_hessians = get_recycled_hessians<T>();
...
//      size_t block_start = has_base_intensity;
      ...
      for (std::set<Image::Index>::iterator coord_it = coords.begin(); coord_it != coords.end(); ++coord_it) {
        ...
        for (size_t encode_i = 0; encode_i < exp_image->num_encodings(); encode_i++) {
          ...
          if (b0_include == "full"  || exp_image->encoding(encode_i).b_value()) {
            ...
            lprob += log_prob(exp_image->operator()(*coord_it)[encode_i], observed, d_lprob, d2_lprob2);
            ...
            for (size_t fibre_i = 0; fibre_i < fibres.size(); fibre_i++) {
              ...
              if (!buffer_gradients[fibre_i].is_empty(*coord_it)) {
                ...
                direction_gradient[fibre_i] = buffer_gradients[fibre_i](*coord_it)[encode_i];

                MR::Math::Matrix<double> hessian_fibre_block_view = hessian_fibre_blocks.sub(fibre_block_start[fibre_i], fibre_block_end[fibre_i], fibre_block_start[fibre_i], fibre_block_end[fibre_i]);

                MR::Math::Matrix<double> hessian_fibre_block = buffer_hessians[fibre_i](*coord_it)[encode_i];
            ...
            MR::Math::Matrix<double> hessian_outer_prod = Math::outer(direction_gradient, direction_gradient);
    ...
    template <> inline Image::Container::Buffer<Fibre::Strand>::Set&          Likelihood::get_recycled_gradients<Fibre::Strand>() {
    ...
    //! Used to get the right used gradients for the templated type.
    template <> inline Image::Container::Buffer<Fibre::Tractlet>::Set&            Likelihood::get_recycled_gradients<Fibre::Tractlet>() {
    ...
    template <> inline Image::Container::Buffer<Fibre::Strand::Tensor>::Set&   Likelihood::get_recycled_hessians<Fibre::Strand>() {
    ...
    //! Used to get the right used hessians for the templated type.
    template <> inline Image::Container::Buffer<Fibre::Tractlet::Tensor>::Set&   Likelihood::get_recycled_hessians<Fibre::Tractlet>() {
