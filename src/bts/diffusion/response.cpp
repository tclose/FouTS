...
namespace BTS {
	...
	namespace Diffusion {
...
#ifndef GRADIENT_NOT_REQUIRED
          gradient += coeff_p * 2 * p * cos2p * (Encoding::orient / tan_dot_orient - tangent / tan_dot_tan);
#endif
	  ...
	  double Response::weighting(const Coord& tangent, Coord& gradient, Coord::Tensor& hessian) const {
      ...
      if (tan_dot_orient != 0.0 && tan_dot_tan != 0.0) {
        ...
        for (size_t p_int = 1; p_int < coeffs.size(); ++p_int) {
          ...
          hessian += coeff_p * 2 * p * cos2p  * (
                      ((2 * p - 1) / tan_dot_orient2)                 * orient.outer(orient)
                    - ( 2 * p      / (tan_dot_orient * tan_dot_tan))  * (tangent.outer(orient) + orient.outer(tangent))
