...
namespace BTS {
    ...
    namespace Diffusion {
        ...
        const std::string Model::ENCODINGS_LOCATION_DEFAULT = str(getenv("HOME"))
                + str("/git/FouTS/params/diffusion/encoding_60.b");
        ...
        Model Model::factory(const MR::Math::Matrix<double>& encodings_matrix,
                             const MR::Math::Matrix<double>& response_SHs, double adc, double fa,
        ...
        Model::Model(const MR::Math::Matrix<double>& encodings_matrix,
                     const MR::Math::Matrix<double>& response_SHs, bool include_isotropic) {
            
            if (response_SHs.rows() != encodings_matrix.rows())
                throw Exception(
                        "If multiple rows to thre response_SHs spherical harmonics are provided then they must match" + str(
                                " the number of rows in the encodings matrix."));
        ...
        void Model::precalculate_weightings(Fibre::Strand::Section& section) const {
            ...
            for (size_t response_i = 0; response_i < num_encodings(); ++response_i)
                section.precalc_weightings[response_i] = operator[](response_i).weighting(
                        section.tangent());
        ...
        void Model::precalculate_weightings_and_gradients(Fibre::Strand::Section& section) const {
            ...
            for (size_t response_i = 0; response_i < num_encodings(); ++response_i)
                section.precalc_weightings[response_i] = operator[](response_i).weighting(
                        section.tangent(), section.precalc_weight_gradients[response_i]);
            
        }
        
        void Model::precalculate_weightings_gradients_and_hessians(
            ...
            for (size_t response_i = 0; response_i < num_encodings(); ++response_i)
                section.precalc_weightings[response_i] = operator[](response_i).weighting(
                        section.tangent(), section.precalc_weight_gradients[response_i],
                        section.precalc_weight_hessians[response_i]);
        ...
        MR::Math::Vector<double> Model::SH_to_coeffs(MR::Math::Vector<double> loaded_SH,
            ...
            if (num_loaded_harmonic_coeffs > num_coeffs)
                throw Exception(
                        "Can only calculate polynomial coefficients up to spherical harmonic order " + str(
                                (num_coeffs - 1) * 2)
                        + ".");
            ...
            if (include_isotropic)
                uncollated.push_back(
                        std::vector<double>(M0_HARMONIC_L0, M0_HARMONIC_L0 + num_coeffs));
            else
        ...
        Model::Model(const MR::Math::Matrix<double>& encodings_matrix, double adc, double fa,
                     bool include_isotropic) {
        ...
        Model::Model(const MR::Math::Matrix<double>& encodings_matrix,
            ...
            if (encodings_matrix.columns() != 4)
                throw Exception(
                        "Four columns required in encodings matrix, found " + str(
            ...
            for (size_t row_i = 0; row_i < encodings_matrix.rows(); row_i++) {
                ...
                if (encodings_matrix(row_i, 3) == consistent_b_value || consistent_b_value == 0) {
                    ...
                    if (warn_on_b_mismatch) {
                        std::cout
                                << "Inconsistent b values used in encoding matrix. Previously found b value "
                                << consistent_b_value << ", found " << encodings_matrix(row_i, 3)
                                << " at row " << str(row_i) + "." << std::endl;
                        response_SHs.row(row_i) = response_SH;
                    } else
                        throw Exception(
                                "Inconsistent b values used in encoding matrix, please either supply a seperate " + str(
                                        "response harmonics for each row or use the automatic tensor harmonic calculation. ")
                                + "Previously found b value "
                                + str(consistent_b_value)
                                + ", found "
        ...
        void Model::init(const MR::Math::Matrix<double>& encodings_matrix,
                         const MR::Math::Matrix<double>& response_SHs, bool include_isotropic) {
            ...
            if (encodings_matrix.columns() > 4)
                throw Exception(
                        "More than 4 columns (" + str(encodings_matrix.columns())
                        + ") were found in diffusion-weighting matrix");
            
            if (encodings_matrix.rows() != response_SHs.rows())
                throw Exception(
                        "Number of rows in encodings_matrix (" + str(encodings_matrix.rows())
                        + ") and response_SH matrix (" + str(response_SHs.rows())
                        + ") do not match.");
        ...
        std::ostream& operator<<(std::ostream& stream, const Diffusion::Model& model) {
            ...
            for (std::vector<Response>::const_iterator response_it = model.responses.begin();
                    response_it != model.responses.end(); ++response_it)
