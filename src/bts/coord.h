...
namespace BTS {
    ...
    class Coord: public MR::Math::Vector<double> {
        ...
        public:
            ...
            template<typename U> bool upper_bounded(U bound) const {
                return (operator[](X) <= (double) bound) && (operator[](Y) <= (double) bound)
                       && (operator[](Z) <= (double) bound);
            }
            ...
            template<typename U> bool upper_bounded(const Triple<U>& bound) const {
                return (operator[](X) <= (double) bound[X]) && (operator[](Y) <= (double) bound[Y])
                       && (operator[](Z) <= (double) bound[Z]);
            }
            ...
            double norm2() const {
                return (operator[](X) * operator[](X) + operator[](Y) * operator[](Y)
            ...
            Coord cross(const Coord& A) const {
                return (Coord(operator[](Y) * A[Z] - operator[](Z) * A[Y],
                        operator[](Z) * A[X] - operator[](X) * A[Z],
                        operator[](X) * A[Y] - operator[](Y) * A[X]));
            }
            
            Coord min_axis() const {
                Coord axis(0.0, 0.0, 0.0);
                if (fabs(operator[](X)) < fabs(operator[](Y))) {
                    if (fabs(operator[](X)) < fabs(operator[](Z)))
                        axis[X] = 1.0;
                    else
                        axis[Z] = 1.0;
                } else if (fabs(operator[](Y)) < fabs(operator[](Z)))
                    axis[Y] = 1.0;
                else
                    axis[Z] = 1.0;
                return axis;
            ...
            Coord sign() const {
                ...
                for (size_t i = 0; i < 3; ++i)
                    s[i] = operator[](i) > 0 ? 1 : (operator[](i) < 0 ? -1 : 0);
                return s;
            }
            ...
            void clear() {    // do nothing, overrides MR::Math::Vector<double> clear to make sure it doesn't resize the vector.
    ...
    inline Coord operator*(double M, Coord c) {
        return Coord(M * c[X], M * c[Y], M * c[Z]);
    }
    inline Coord operator+(double inc, Coord c) {
        return Coord(inc + c[X], inc + c[Y], inc + c[Z]);
    }
