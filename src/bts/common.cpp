...
namespace BTS {
  ...
  std::vector<double>                    parse_double_vector(const std::string& s) {
    ...
    while (start_number != std::string::npos) {
     ...
     if (num_string.find_first_of('-', 1) != std::string::npos  || num_string.find('.', num_string.find('.')+1) != std::string::npos)  // If there is a '-' character anywhere other than the first character, or if there is more than one decimal point then throw an error.
        throw Exception("Value, " + num_string + " at position: " + str(i+1) + ", is invalid");
