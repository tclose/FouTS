...
namespace BTS {
    ...
    namespace Fibre {
        ...
        namespace Base {
            ...
            template<typename T> const Coord Set<T>::FILE_SEPARATOR = Triple<double>(-INFINITY, NAN,
                    INFINITY);
            template<typename T> const std::string Set<T>::PROPS_FILE_PREAMBLE =
            ...
            template<typename T> Set<T>::Set(const Set<T>& set)
                    : Object(set), vrows(set.vrows), rsize(set.rsize), elem_dgree(set.elem_dgree), row_ends(
                              set.row_ends ? (size_t*) malloc(sizeof(size_t) * set.size()) : 0), elem_dgrees(
                              set.elem_dgrees ? (size_t*) malloc(sizeof(size_t) * set.size()) : 0), elem_props(
                              new std::vector<const char*>(*set.elem_props)) {
                
                if (size() && var_elem_degrees()) {
                    
                    if (!elem_dgrees)
                        throw Exception(
                                "Allocation of elem_dgrees (" + str(sizeof(size_t) * size())
                                + ") failed.");
                    
                    if (!row_ends)
                        throw Exception(
                                "Allocation of row_ends (" + str(sizeof(size_t) * size())
                                + ") failed.");
                ...
                ext_props =
                        set.ext_props ? new std::map<std::string, std::string>(*set.ext_props) : 0;
                ext_elem_prop_keys =
                        set.ext_elem_prop_keys ? new std::vector<std::string>(
                                                         *set.ext_elem_prop_keys) :
                                                 0;
                ext_elem_prop_defaults =
                        set.ext_elem_prop_defaults ? new std::vector<std::string>(
                                                             *set.ext_elem_prop_defaults) :
                                                     0;
                ext_elem_prop_values =
                        set.ext_elem_prop_values ? new std::vector<std::vector<std::string> >(
            ...
            template<typename T> std::vector<std::string> Set<T>::extend_prop_keys() const {
                ...
                if (ext_props)
                    for (std::map<std::string, std::string>::iterator prop_it = ext_props->begin();
                            prop_it != ext_props->end(); ++prop_it)
            ...
            template<typename T> bool Set<T>::elem_props_match(
                ...
                if (elem_props == elem_properties)    // Point to the same properties
                    ...
                    // Compare the property pointers (relies on the fact that properties are sorted by order of memory location when they are inserted).
                    match = !memcmp(&(elem_props->operator[](0)), &(elem_properties->operator[](0)),
                            num_elem_props());
            ...
            template<typename T> size_t Set<T>::initial_vsize(size_t size, size_t* elem_degrees,
                                                              size_t elem_row_size,
            ...
            template<typename T> void Set<T>::set_extend_props(
            ...
            template<typename T> void Set<T>::set_extend_prop(const std::string& prop,
                ...
                if (is_owner()) {
                    ...
                    *elem_props = *set.elem_props;
                    ...
                    if (set.var_elem_degrees() && set.sze) {
                        ...
                        if (!row_ends)
                            throw Exception(
                                    "Allocation of row_ends (" + str(sizeof(size_t) * set.sze)
                                    + ") failed.");
                        
                        if (!elem_dgrees)
                            throw Exception(
                                    "Allocation of elem_dgrees (" + str(sizeof(size_t) * set.sze)
                                    + ") failed.");
                    ...
                    assert(props_match(set));
                    // Properties need to match exactly when fibre object is not owner of data.
                    assert(rsize == set.rsize);
                    // Row sizes need to be the same (NB: when variable they should both be 0).
                    assert(elem_dgree == set.elem_dgree);
                    assert(
                            !var_elem_degrees() || memcmp(row_ends, set.row_ends, size() * sizeof(size_t)));
                    // All row sizes must match for variable row sizes case.
                    assert(
                            !var_elem_degrees() || memcmp(elem_dgrees, set.elem_dgrees, size() * sizeof(size_t)));
                    // All row sizes must match for variable row sizes case.
            ...
            template<typename T> void Set<T>::resize(size_t new_size, double fill_value,
                                                     size_t new_elem_degree,
                                                     size_t new_elem_vsize) {
                
                //FIXME: Needs some restructuring as functionality from Object was moved in here.
                
                if (!is_owner())
                    throw Exception(
                            "Cannot resize fibre object that does not own the underlying data (i.e. is a view onto part of a larger structure).");
                ...
                if (!var_elem_degrees()) {
                    ...
                    if (new_elem_degree && (new_elem_degree != elem_dgree))
                        throw Exception(
                                "The the new element size does not match that of set, and the set elements are not variable (see free_elem_degrees()).");
                    
                    if (new_elem_vsize && (new_elem_vsize != rsize))
                        throw Exception(
                                "The the new element vsize does not match that of set, and the set elements are not variable (see free_elem_degrees()).");
                    ...
                    //If variable row and the new size is greater than the previous take the multiple of the new vsize.
                        ...
                        new_bsize = (size() ? row_end(size() - 1) : 0)
                                + (new_size - size()) * new_elem_vsize;
                ...
                if (new_bsize > old_bsize) {
                    ...
                    for (int prop_i = num_props() - 1; prop_i >= 0; --prop_i)
                        MR::Math::Vector<double>::operator[](new_bsize + prop_i) = MR::Math::Vector<
                                double>::operator[](old_bsize + prop_i);
                ...
                if (num_extend_elem_props()) {
                    ...
                    for (size_t key_i = 0; key_i < num_extend_elem_props(); ++key_i)
                        ext_elem_prop_values->operator[](key_i).resize(new_size,
                                ext_elem_prop_defaults->operator[](key_i));
            ...
            template<typename T> void Set<T>::push_back(const T& elem) {
                ...
                if (!elem.props_match(elem_props))
                    throw Exception(
                            "Element properties (" + str(elem.prop_keys())
                            + ") do not match that of set " + str(elem_props) + ".");
                
                if (rsize && (elem.vsize() != rsize))
                    throw Exception(
                            "vize() of element (" + str(elem.vsize())
                            + ") does not match that of the other elements in set (" + str(rsize)
                            + "). Call 'free_elem_degree()' on set if this is intentional.");
            ...
            template<typename T> void Set<T>::push_back(const T& elem, bool with_props) {
                ...
                if (with_props)
                    ...
                    if (!var_elem_degrees() && (elem.bsize() != (rsize - num_elem_props())))
                        throw Exception(
                                "bize() of element (" + str(elem.bsize())
                                + ") does not match that of the other elements in set ("
                                + str(rsize - num_elem_props())
                                + "). Call 'free_elem_degree()' on set if this is intentional.");
            ...
            template<typename T> template<typename U> void Set<T>::copy_extend_elem_props(
                    const Set<U>& set) {
                
                if (set.size() != this->size())
                    throw Exception(
                            "Size of set (" + str(this->size())
                            + ") does not match size of source set (" + str(set.size())
                            + ") for properties copy.");
            ...
            template<typename T> template<typename U> void Set<T>::append_extend_elem_props(
                    const Set<U>& set) {
                    ...
                    if (!ext_elem_prop_keys) {
                        ...
                        for (size_t prop_i = 0; prop_i < set.ext_elem_prop_keys->size(); ++prop_i) {
                            ...
                            if (!has_extend_elem_prop(key))
                                add_extend_elem_prop(key,
                                        set.ext_elem_prop_defaults->operator[](prop_i));
                            
                            for (size_t row_i = 0; row_i < size(); ++row_i)
                                set_extend_elem_prop(key,
                                        set.ext_elem_prop_values->operator[](prop_i)[row_i], row_i);
            ...
            template<typename T> Set<T>& Set<T>::select(Set<T>& new_set,
                ...
                for (size_t i = 0; i < indices.size(); i++) {
                    ...
                    if (indices[i] >= this->size())
                        throw Exception(
                                "Supplied index, " + str(indices[i]) + ", is out of range.");
                    
                    std::map<std::string, std::string> props_row = get_extend_elem_prop_row(
                            indices[i]);
            ...
            template<typename T> template<typename V> void Set<T>::load_tpl(
                    const std::string& location, V& reader) {
                
                if (!is_owner())
                    throw Exception(
                            "Cannot load a fibre set that does not own the underlying data (i.e. is a view onto part of a larger structure).");
                ...
                std::map<const char*, double> loaded_props = extract_props<typename T::Set>(
                        loaded_extend_props);
                
                // Extract just the keys for the set properties
                std::vector<const char*> loaded_prop_keys;
                for (std::map<const char*, double>::iterator prop_it = loaded_props.begin();
                        prop_it != loaded_props.end(); ++prop_it)
                ...
                if (loaded_extend_props.count(NUM_ELEMS_EXT_PROP) && loaded_extend_props.count(
                            ELEM_SIZE_EXT_PROP)
                    ...
                    reset(to<size_t>(loaded_extend_props[NUM_ELEMS_EXT_PROP]),
                            ...
                            to<size_t>(loaded_extend_props[ELEM_VSIZE_EXT_PROP]), loaded_prop_keys,
                            loaded_elem_prop_keys);
                
                // Else reset the set to a variable row size set.
                else
                    reset(loaded_prop_keys, loaded_elem_prop_keys);
                ...
                for (std::vector<std::string>::iterator prop_it = extended_elem_prop_header.begin();
                        prop_it != extended_elem_prop_header.end(); ++prop_it)
                ...
                while (reader.next(elem, properties_row)) {
                    ...
                    if (elem.degree() == 0) {
                        std::cout << std::endl << "WARNING!! Omitting element " << count
                                  << " as it's size is 0." << std::endl;
                        continue;
                    }
            ...
            template<typename T> template<typename V> void Set<T>::save_tpl(
                    const std::string& location, V& writer) const {
            ...
            template<typename T> Set<T>& Set<T>::permute(Set<T>& permuted_set,
                                                         const std::vector<size_t>& indices) const {
                
                if (indices.size() != this->size())
                    throw Exception(
                            "Size of indices (" + str(indices.size())
                            + ") does not match size of set (" + str(this->size()) + ").");
                
                if (permuted_set.size() != this->size())
                    throw Exception(
                            "Size of output set (" + str(permuted_set.size())
                            + ") does not match size of set (" + str(this->size()) + ").");
            ...
            template<typename T> double Set<T>::distance(
                ...
                if (reference.size() < this->size())
                    throw Exception(
                            "Size of reference Strand::Set (" + str(reference.size())
                            + ") must be greater than or equal to that of the Strand::Set of interest  ("
                            + str(this->size()) + ").");
                ...
                for (size_t fibre_i = 0; fibre_i < this->size(); fibre_i++) {
                    ...
                    dist += this->operator[](fibre_i).distance(reference[matched_indices[fibre_i]],
                            strands_per_acs);
            ...
            template<typename T> double Set<T>::distance(const Set<T>& reference,
            ...
            template<typename T> void Set<T>::similarity_matrix(
                    const Set<T>& reference, MR::Math::Matrix<double>& similarity,
                    double strands_per_acs) const {
                
                if ((similarity.rows() != size()) || (similarity.columns() != reference.size()))
                    throw Exception(
                            "Dimensions of similarity matrix, " + str(similarity.rows()) + " X "
                            + str(similarity.columns()) + " do not match Strand::Sets, "
                            + str(size()) + ", " + str(reference.size()) + ".");
                
                for (size_t this_i = 0; this_i < size(); this_i++) {
                    for (size_t reference_i = 0; reference_i < reference.size(); reference_i++)
                        similarity(this_i, reference_i) = operator[](this_i).distance(
                                reference[reference_i], strands_per_acs);
                }
                
            }
            
            template<typename T> Set<T>& Set<T>::smallest_distance_set(const Set<T>& reference,
                                                                       Set<T>& smallest) const {
                
                if (size() != reference.size())
                    throw Exception(
                            "Size of set (" + str(size())
            ...
            template<typename T> void Set<T>::add_extend_elem_prop(
                ...
                if (!has_extend_elem_prop(key)) {
                    ...
                    ext_elem_prop_values->push_back(
                            std::vector<std::string>(size(), default_value));
            ...
            template<typename T> template<typename U> void Set<T>::add_extend_elem_props(
                    const Set<U>& set) {
                ...
                if (set.ext_elem_prop_keys) {
                    ...
                    for (size_t key_i = 0; key_i < set.ext_elem_prop_keys->size(); key_i++)
                        add_extend_elem_prop(set.ext_elem_prop_keys->operator[](key_i),
                                set.ext_elem_prop_defaults->operator[](key_i));
                }
            ...
            template<typename T> void Set<T>::set_extend_elem_prop(std::string key,
                                                                   std::string value,
                                                                   size_t elem_index) {
            ...
            template<typename T> std::string Set<T>::get_extend_elem_prop(std::string key,
            ...
            template<typename T> std::map<std::string, std::string> Set<T>::get_extend_elem_prop_row(
                    size_t elem_index) const {
                ...
                if (ext_elem_prop_keys)
                    for (size_t key_i = 0; key_i < ext_elem_prop_values->size(); ++key_i)
                        row[ext_elem_prop_keys->operator[](key_i)] =
                                ext_elem_prop_values->operator[](key_i)[elem_index];
                
                return row;
                
            }
            
            template<typename T> void Set<T>::set_extend_elem_prop_row(
                ...
                if (ext_elem_prop_keys) {
                    ...
                    for (size_t key_i = 0; key_i < ext_elem_prop_keys->size(); key_i++) {
                        ...
                        std::map<std::string, std::string>::const_iterator key_it = properties.find(
                                ext_elem_prop_keys->operator[](key_i));
                        ...
                        if (key_it == properties.end()) {
                            if (ignore_missing)
                                ...
                                throw Exception(
                                        "Property '" + ext_elem_prop_keys->operator[](key_i)
                                        + "' not found in added properties row.");
...
//      }
            ...
            template<typename T> void Set<T>::reset(size_t num_elems, size_t elem_degree,
                                                    size_t elem_vsize,
                                                    const std::vector<const char*>& properties,
                                                    const std::vector<const char*>& elem_properties,
                                                    double fill_value) {
            ...
            template<typename T> void Set<T>::erase(size_t index) {
                ...
                if (!is_owner())
                    throw Exception(
                            "Cannot erase element from fibre object that does not own the underlying data (i.e. is a view onto part of a larger structure).");
                
                if (index > sze)
                    throw Exception(
                            "Index (" + str(index) + ") out of range (" + str(size()) + ").");
                ...
                if (var_elem_degrees()) {
                    ...
                    if (size()) {
                        ...
                        if (!row_ends)
                            throw Exception(
                                    "Failed to allocate size (" + str(sizeof(size_t) * size())
                                    + ") for row_ends array.");
                ...
                if (num_extend_elem_props()) {
                    ...
                    for (size_t key_i = 0; key_i < num_extend_elem_props(); ++key_i) {
                        std::vector<std::string>& value_row = ext_elem_prop_values->operator[](
                                key_i);
            ...
            template<typename T> T Set<T>::insert(const T& element, size_t index) {
                ...
                if (elem_dgree && elem_dgree != element.degree())
                    throw Exception(
                            "Element size does not match remainder of set (NB: Use Set::free_elem_degree() function first if this is intentional).");
                ...
                for (int i = old_bsize - 1; i >= (int) row_start(index); --i)
                    MR::Math::Vector<double>::operator[](i + element.vsize()) = MR::Math::Vector<
                            double>::operator[](i);
                ...
                if (num_extend_elem_props()) {
                    ...
                    for (size_t key_i = 0; key_i < num_extend_elem_props(); ++key_i) {
                        std::vector<std::string>& value_row = ext_elem_prop_values->operator[](
                                key_i);
                        value_row.insert(value_row.begin() + index,
                                ext_elem_prop_defaults->operator[](key_i));
                    }
            ...
            template<typename T> size_t Set<T>::key_index(const std::string& key) const {
                ...
                if (!ext_elem_prop_keys)
                    throw Exception(
                            "Property '" + key
                            + "' was not found (set has no extended properties).");
            ...
            template<typename T> std::ostream& operator<<(std::ostream& stream, const Set<T>& set) {
                ...
                for (size_t elem_i = 0; elem_i < set.size(); elem_i++) {
                    ...
                    for (size_t prop_i = 0; prop_i < header.size(); prop_i++) {
                        stream << header[prop_i] << ": "
                        << set.get_extend_elem_prop(header[prop_i], elem_i);
            ...
            template<typename T> void Set<T>::add_elem_prop(const char* const name, double value,
                                                            bool ignore_present) {
                
                if (!is_owner())
                    throw Exception(
                            "Cannot add element property to a fibre object that does not own the underlying data (i.e. is a view onto part of a larger structure).");
                
                if (!rsize)
                    throw Exception(
                            "Cannot add element property to unfixed row size (theoretically possible but I haven't worked out the best algorithm yet)");
                ...
                for (std::vector<const char*>::iterator prop_it = elem_props->begin();
                    ...
                    if (!compare) {
                        ...
                        if (!ignore_present)
                            throw Exception(
                                    "Element property with the same name ('" + str(name)
                                    + "') already exists in fibre object");
                ...
                for (int prop_i = num_props() - 1; prop_i >= 0; --prop_i)
                    MR::Math::Vector<double>::operator[](bsize() + prop_i) =
                            MR::Math::Vector<double>::operator[](bsize() + prop_i - size());
                ...
                for (int elem_i = size() - 1; elem_i >= 0; --elem_i)
                    for (int elem_elem_i = rsize - 1; elem_elem_i >= 0; --elem_elem_i)
                        ...
                        if ((size_t) elem_elem_i == insert_index + rsize - elem_props->size()) {
                            MR::Math::Vector<double>::operator[](elem_i * rsize + elem_elem_i) =
                                    value;
                            --accum_offset;
                        } else
                            MR::Math::Vector<double>::operator[](elem_i * rsize + elem_elem_i) =
                                    MR::Math::Vector<double>::operator[](
                                            elem_i * rsize + elem_elem_i - accum_offset);
                
            }
            
            template<typename T> void Set<T>::remove_elem_prop(const char* const name,
                                                               bool ignore_missing) {
                
                if (!is_owner())
                    throw Exception(
                            "Cannot remove element property from a fibre object that does not own the underlying data (i.e. is a view onto part of a larger structure).");
                ...
                if (prop_it != elem_props->end()) {
                    ...
                    for (size_t elem_i = 0; elem_i < size(); ++elem_i) {
                        for (size_t elem_elem_i = 0; elem_elem_i < row_size(elem_i);
                                ++elem_elem_i) {
                            ...
                            MR::Math::Vector<double>::operator[](row_start(elem_i) + elem_elem_i) =
                                    MR::Math::Vector<double>::operator[](
                                            row_start(elem_i) + elem_elem_i + accum_offset);
                    ...
                    throw Exception(
                            "Fibre object does not have element property '" + str(name)
                            + "' (use 'ignore_missing' parameter to ignore).");
            ...
            template<typename T> void Set<T>::clear_elem_props() {
                ...
                if (!is_owner())
                    throw Exception(
                            "Cannot clear properties from a fibre object that does not own the underlying data (i.e. is a view onto part of a larger structure).");
                ...
                for (size_t prop_i = 0; prop_i < num_props(); ++prop_i)
                    MR::Math::Vector<double>::operator[](
                            bsize() + prop_i - elem_props->size() * size()) = MR::Math::Vector<
                            double>::operator[](bsize() + prop_i);
            ...
            template<typename T> template<typename U> void Set<T>::copy_elem_props(
                    const Set<U>& set) {
                
                if (size() != set.size())
                    throw Exception(
                            "Set sizes must be the same (" + str(size()) + " and " + str(set.size())
            ...
            template<typename T> size_t Set<T>::elem_prop_index(const char* const name) const {
                ...
                if (prop_index == num_elem_props())
                    throw Exception(
                            "Fibre set does not have element property '" + str(name)
                            + "', it needs to be added by the 'add_elem_prop' command beforehand. "
            ...
            template<typename T> void Set<T>::elem_resize(size_t new_elem_degree,
                                                          size_t new_elem_vsize,
                                                          double fill_value) {
                
                if (!is_owner())
                    throw Exception(
                            "Cannot resize fibre set that does not own the underlying data (i.e. is a view onto part of a larger structure).");
                
                if (!new_elem_degree)
                    throw Exception(
                            "Cannot change fixed element size to 0. However, if row size is 'freed' using 'free_elem_degrees' their size can be 0.");
                ...
                for (size_t elem_i = 0; elem_i < size(); ++elem_i) {
                    ...
                    for (size_t elem_elem_i = 0;
                            elem_elem_i < std::min(rsize, orig.row_size(elem_i)) - num_elem_props();
                            ++elem_elem_i)
                        MR::Math::Vector<double>::operator[](row_start(elem_i) + elem_elem_i) =
                                orig_vector[orig.row_start(elem_i) + elem_elem_i];
                    
                    //Fill in the extra data in the resized set if required
                    for (size_t elem_elem_i = orig.row_size(elem_i) - num_elem_props();
                            elem_elem_i < (rsize - num_elem_props()); ++elem_elem_i)
                        MR::Math::Vector<double>::operator[](row_start(elem_i) + elem_elem_i) =
                                fill_value;
                    ...
                    for (size_t prop_i = 0; prop_i < num_elem_props(); ++prop_i)
                        MR::Math::Vector<double>::operator[](
                                row_end(elem_i) - num_elem_props() + prop_i) =
                                orig_vector[orig.row_end(elem_i) - num_elem_props() + prop_i];
                    
                }
                
                for (size_t prop_i = 0; prop_i < num_props(); ++prop_i)
                    MR::Math::Vector<double>::operator[](bsize() + prop_i) =
                            orig_vector[orig.bsize() + prop_i];
                
            }
            
            template<typename T> std::vector<std::string> Set<T>::parse_extend_elem_props_line(
            ...
            template<typename T> std::vector<std::string> Set<T>::read_extend_elem_prop_header(
                ...
                if (strcmp(file_intro, PROPS_FILE_PREAMBLE.c_str()))
                    throw Exception(
                            "Required file intro '" + PROPS_FILE_PREAMBLE
            ...
            template<typename T> void Set<T>::load_extend_elem_props(std::string location) {
                ...
                *ext_elem_prop_keys = read_extend_elem_prop_header(in);
                ...
                try {
                    ...
                    throw Exception(
                            "Error reading extended properties from " + location + ": " + e[0]
                            + ".");
                }
            ...
            template<typename T> void Set<T>::save_extend_elem_props(std::string location) {
                ...
                for (std::vector<std::string>::const_iterator key_it = ext_elem_prop_keys->begin();
                        key_it != ext_elem_prop_keys->end(); key_it++)
                    out << *key_it << '\t';
            ...
            template<typename T> void Set<T>::add_extend_elem_prop_row(
                    const std::vector<std::string>& values_row) {
                
                if (num_extend_elem_props() != values_row.size())
                    throw Exception(
                            "Number of supplied values " + str(values_row.size())
                            + " does not match extended element properties ("
                            + str(num_extend_elem_props()) + ").");
            ...
            template<typename T> size_t Set<T>::freeze_elem_degree(bool ignore_fail) {
                ...
                if (var_elem_degrees()) {
                    ...
                    if (size()) {
                        ...
                        for (size_t row_i = 0; row_i < size(); ++row_i)
                            if (fix_rsize != row_size(row_i) || fix_elem_dgree
                                    != elem_degree(row_i)) {
                                success = false;
                                break;
                            }
                    ...
                    if (success) {
                        ...
                        throw Exception(
                                "Attempt to fix elem sizes failed as multiple values exist in the fibre object.");
            ...
            template<typename T> void Set<T>::free_elem_degree() {
                ...
                if (!var_elem_degrees()) {
                    ...
                    if (size()) {
                        ...
                        if (!row_ends)
                            throw Exception(
                                    "Could not allocate memory for variable row sizes (" + str(
                                            alloc_size)
                                    + " bytes requested).");
                        ...
                        if (!elem_dgrees)
                            throw Exception(
                                    "Could not allocate memory for variable elem sizes (" + str(
                                            sizeof(size_t) * size())
                                    + " bytes requested).");
            ...
            template<typename T> template<typename U> void Set<T>::copy_relevant_elem_props(
                    const Base::Set<U>& source) {
                
                if (size() != source.size())
                    throw Exception(
                            "Source and destination set sizes must be equal to copy element props (" + str(
                                    source.size())
                            + " and " + str(size()) + " respectively).");
