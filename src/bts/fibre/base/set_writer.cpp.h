...
namespace BTS {
  ...
  namespace Fibre {
    ...
    namespace Base {
      ...
      template <typename T> void     SetWriter<T>::create (const std::string& location,
        ...
        elem_header.insert(elem_header.end(), extend_elem_prop_keys.begin(), extend_elem_prop_keys.end());
      ...
      template <typename T> void     SetWriter<T>::append (const T& set) {
        ...
        for (size_t elem_i = 0; elem_i < set.size(); elem_i++) {
          ...
          std::map<std::string,std::string> properties_row = set.get_extend_elem_prop_row(elem_i); //
      ...
      template <typename T> void     SetWriter<T>::append (const T& set, std::map<std::string,std::string>& properties) {

        T new_set (set);

        std::map<std::string,std::string> extend_props = new_set.get_extend_props();

        for (std::map<std::string,std::string>::iterator prop_it = extend_props.begin(); prop_it != extend_props.end(); ++prop_it)
