...
namespace BTS {
	...
	namespace Fibre {
      ...
      template <typename T> const std::string TensorWriter<T>::HEADER_PREAMBLE = "%%% Keys %%%:";


      template <typename T> void TensorWriter<T>::create (const std::string& location, const typename T::RowType& template_value) {
        ...
        for (std::vector<std::string>::iterator key_it = hdr.begin(); key_it != hdr.end(); ++key_it)
          out << *key_it << " ";
      ...
      template <typename T> void TensorWriter<T>::append(const T& tensor) {
        ...
        if (template_length != tensor.rows())
          throw Exception ("Number of rows in tensor matrix (" + str(tensor.rows()) + ") did not match size of template (" + str(template_length) + ").");

        if (template_length != tensor.columns())
          throw Exception ("Number of columns in tensor matrix (" + str(tensor.columns()) + ") did not match size of template (" + str(template_length) + ").");
      ...
      template <typename T> std::vector<std::string>  TensorWriter<T>::header(const typename T::RowType& tmplate) {
      ...
      template <typename T> void   TensorWriter<T>::header_element( const Strand& strand,
        ...
        for(uint degree_i = 0; degree_i < strand.degree(); degree_i++)
          header_element(strand[degree_i], header, prefix + "degree" + str(degree_i) + ">");

      }


      template <typename T> void   TensorWriter<T>::header_element( const Tractlet& tractlet,
                                                                    std::vector<std::string>& header,
                                                                    const std::string& prefix) {
      ...
      template <typename T> void   TensorWriter<T>::header_element(const Tractlet::Set& set,
                                                                   std::vector<std::string>& header,
                                                                   const std::string& prefix) {
      ...
      template <typename T> void   TensorWriter<T>::header_element(const Strand::Set& set,
                                                                   std::vector<std::string>& header,
                                                                   const std::string& prefix) {
      ...
      template <typename T> void   TensorWriter<T>::header_element(const Coord& element,
                                                                   std::vector<std::string>& header,
                                                                   const std::string& prefix) {
