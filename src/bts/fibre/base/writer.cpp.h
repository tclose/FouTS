	...
	namespace Fibre {
	  ...
	  namespace Base {
	    ...
	    template <typename T> void      Writer<T>::create(const std::string& location,
	      ...
	      prop_header.insert(prop_header.end(), extend_prop_keys.begin(), extend_prop_keys.end());
        ...
        if (!out) throw Exception ("error creating tracks file \"" + location + "\": " + strerror (errno));

        out << "mrtrix tracks\nEND\n";
        for (std::map<std::string,std::string>::const_iterator i = file_props.begin(); i != file_props.end(); ++i)
      ...
      template <typename T> void      Writer<T>::append (const T& fibre_object, std::map<std::string,std::string> properties_row) {
      ...
      template <typename T> template <typename U> inline void      Writer<T>::write(const U& fibre_object) {
      ...
      template <typename T> template <typename U> inline Coord     Writer<T>::write_first(const U& fibre_object) {
      ...
      template <typename T> void      Writer<T>::check_properties_row(const std::map<std::string,std::string>& properties_row) {

        if (properties_row.size() != this->prop_hdr.size()) {

          //Generate list of header for error message.
          std::vector<std::string> header;
          for (std::map<std::string,std::string>::const_iterator key_it = properties_row.begin(); key_it != properties_row.end(); key_it++)
            header.push_back(key_it->first);

          //Throw Exception
          throw Exception ("Supplied row properties (" + str (header) +
      ...
      template <typename T> void      Writer<T>::append_extend_elem_props_row (const T& fibre_object,
      ...
      template <typename T> void      Writer<T>::append_extend_elem_props_row (std::map<std::string,std::string>& properties_row) {
      ...
      template <typename T> void      Writer<T>::write_properties_row(const std::vector<std::string>& properties_header,
                                                                      std::map<std::string,std::string>& properties_row,
                                                                      std::ofstream& fout) {

        for (std::vector<std::string>::const_iterator hdr_it = properties_header.begin(); hdr_it != properties_header.end(); hdr_it++) {

          if (!properties_row.count(*hdr_it))
            throw Exception ("Did not find value corresponding to property '" + *hdr_it + "'.");
      ...
      template <typename T> void      Writer<T>::init() {
        ...
        if (this->dtype == MR::DataType::Float64LE || this->dtype == MR::DataType::Float64BE)
          sizeof_coord = 3 * sizeof(double);
        else if (this->dtype == MR::DataType::Float32LE || this->dtype == MR::DataType::Float32BE)
      ...
      template <typename T> void      Writer<T>::write_props_header(std::ofstream& out, const std::vector<std::string>& header) {
