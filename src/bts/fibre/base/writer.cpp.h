  ...
  namespace Fibre {
    ...
    namespace Base {
      ...
      template<typename T> void Writer<T>::create(const std::string& location,
        ...
        if (!out)
          throw Exception(
              "error creating tracks file \"" + location + "\": "
                  + strerror(errno));

        out << "mrtrix tracks\nEND\n";
        for (std::map<std::string, std::string>::const_iterator i =
            file_props.begin(); i != file_props.end(); ++i)
      ...
      template<typename T> void Writer<T>::append(const T& fibre_object,
      ...
      template<typename T> void Writer<T>::write(const Coord& t) {
        ...
        if (this->dtype == MR::DataType::Float64LE) {
          ...
          throw Exception(
              "Unsupported dataype '" + str(this->dtype.specifier()) + "'.");
        ...
        if (properties_row.size() != this->prop_hdr.size()) {
          ...
          throw Exception(
              "Supplied row properties (" + str(header)
                  + ") does not match the number of supplied header ("
                  + str(prop_hdr) + "\").");
        }
      ...
      template<typename T> void Writer<T>::write_properties_row(
        ...
        for (std::vector<std::string>::const_iterator hdr_it =
            properties_header.begin(); hdr_it != properties_header.end();
            hdr_it++) {
      ...
      template<typename T> void Writer<T>::init() {
        ...
        if (this->dtype == MR::DataType::Float64LE
            || this->dtype == MR::DataType::Float64BE)
