...
namespace BTS {
    ...
    namespace Fibre {
        ...
        namespace Base {
            ...
            const char* Object::ALPHA_PROP = "alpha";
            ...
            void Object::reset(size_t size, size_t row_size,
                               const std::vector<const char*>& properties, double fill_value) {
            ...
            bool Object::props_match(const std::vector<const char*>* properties) const {
                ...
                if (props == properties)    // Point to the same properties
                    ...
                    match = !memcmp(&(props->operator[](0)), &(properties->operator[](0)),
                            num_props());
            ...
            void Object::add_prop(const char* const name, double value) {
                ...
                if (!is_owner())
                    throw Exception(
                            "Cannot add property ('" + str(name)
                            + "') to a fibre object that does not own the underlying data (i.e. is a view onto part of a larger structure).");
                
                //Loop through properties and insert the new property in the appropriate position for alphabetical order
                size_t insert_index = 0;
                
                for (std::vector<const char*>::iterator prop_it = props->begin();
                    ...
                    if (!compare)
                        throw Exception(
                                "Property with the same name ('" + str(name)
                ...
                if (insert_index == props->size()) {
                    ...
                    for (int prop_i = props->size() - 1; prop_i > (int) insert_index; --prop_i)
                        MR::Math::Vector<double>::operator[](bsize() + prop_i) = MR::Math::Vector<
                                double>::operator[](bsize() + prop_i - 1);
            ...
            void Object::remove_prop(const char* const name, bool ignore_missing) {
                ...
                if (!is_owner())
                    throw Exception(
                            "Cannot remove property from a fibre object that does not own the underlying data (i.e. is a view onto part of a larger structure).");
                ...
                if (prop_it != props->end()) {
                    ...
                    for (size_t i = (prop_it - props->begin()) + 1; i < num_props(); ++i)
                        MR::Math::Vector<double>::operator[](bsize() + i - 1) = MR::Math::Vector<
                                double>::operator[](bsize() + i);
                    ...
                    throw Exception(
                            "Fibre object does not have property '" + str(name)
            ...
            Object& Object::clear() {
                ...
                if (num_props()) {
                    ...
                    for (size_t prop_i = 0; prop_i < num_props(); ++prop_i)
                        MR::Math::Vector<double>::operator[](prop_i) =
            ...
            size_t Object::prop_index(const char* const name) const {
                ...
                if (prop_index == num_props())
                    throw Exception(
                            "Fibre object does not have property '" + str(name)
                            + "', it needs to be added by the 'add_prop' command beforehand.\n"
            ...
            Object& Object::push_back(const Coord& c) {
                ...
                if (!is_owner())
                    throw Exception(
                            "Cannot push back to a fibre object that does not own the underlying data (i.e. is a view onto part of a larger structure).");
            ...
            std::string Object::load_matlab_str(const std::string& location, double scale) {
                ...
                if (!location.size())
                    ...
                    throw Exception(
                            "Unrecognised file extension '" + File::extension(location) + "'.");
