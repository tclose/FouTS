    ...
    namespace Base {
      ...
      template <typename T> class Writer {
        ...
        public:
          ...
          static void   write_props_header(std::ofstream& out, const std::vector<std::string>& header);
        ...
        public:
          ...
          std::ofstream&                          get_out_stream()   { return this->out; }

          off64_t                                 get_count_offset() { return this->count_offset; }

          size_t                                  get_count()        { return this->count; }

          size_t                                  get_total_count()  { return this->total_count; }

          void                                    override_count(size_t count) {this->count = count; }

          void                                    override_total_count(size_t count) {this->total_count = count; }
        ...
        protected:
          ...
          void                                    increment_count()
            { ++this->count; ++this->total_count; update_count(); }

          void                                    append_file_seperator();

          void                                    update_count();

          void                                    check_properties_row(const std::map<std::string,std::string>& properties_row);

          void                                    append_extend_elem_props_row(std::map<std::string,std::string>& properties_row);

          void                                    append_extend_elem_props_row(const T& fibre_object, std::map<std::string,std::string>& properties_row);

          static void                             write_properties_row( const std::vector<std::string>& key_list,
      ...
      template <typename T> class TextWriter : public Fibre::Base::Writer<T> {
        ...
        public:
          ...
          Coord                 write_first(const Coord& c)
            {  this->out.seekp((int)this->out.tellp() - 12); write (c); return c; }
