    ...
    namespace Base {
      ...
      template <typename T> class Set : public Object {
        ...
        protected:
          ...
          size_t                        initial_vsize(size_t size,
                                                      size_t* elem_degrees,
        ...
        public:
          ...
          Set ( const std::vector<const char*>& props,
              ...
              : Object(0, props.size(), props),
                vrows(true),
                rsize(0),
                elem_dgree(0),
                row_ends(0),
                elem_dgrees(0),
                elem_props(new std::vector<const char*>(elem_props)),
                ext_props(0),
          ...
          Set ( size_t size,
              ...
              : Object(size, size * elem_vsize + props.size(), props),
                ...
                elem_dgrees(0),
                elem_props(new std::vector<const char*>(elem_props)),
                ext_props(0), ext_elem_prop_keys(0), ext_elem_prop_defaults(0), ext_elem_prop_values(0) {
          ...
          explicit Set (size_t size,
                    ...
                    : Object(size, view, props),
                      ...
                      rsize(0),
                      elem_dgree(0),
                      row_ends (size ? (size_t*)malloc(sizeof(size_t) * size) : 0),
                      elem_dgrees(size ? (size_t*)malloc(sizeof(size_t) * size) : 0),
                      elem_props(elem_props),
                      ext_props(0), ext_elem_prop_keys(0), ext_elem_prop_defaults(0), ext_elem_prop_values(0) {
            ...
            if (size) {
              ...
              if (!row_ends)
                throw Exception("Failed to allocate array for row_ends (" + str(sizeof(size_t) * size) + " bytes requested).");
          ...
          ~Set () {
            ...
            if (this->GSLVector<double>::owner) {
              ...
              if (ext_elem_prop_keys) delete ext_elem_prop_keys;
              if (ext_elem_prop_defaults) delete ext_elem_prop_defaults;
              if (ext_elem_prop_values) delete ext_elem_prop_values;
          ...
          double&                                     var_base_intensity()
            { assert(has_var_base_intensity()); return prop(BASE_INTENSITY_PROP); }
          ...
          size_t                                      elem_degree() const
            { assert(!var_elem_degrees()); return elem_dgree; }
          ...
          void                                        append(const Set& set)
            { for (size_t elem_i = 0; elem_i < set.size(); ++elem_i)push_back(set[elem_i], set.get_extend_elem_prop_row(elem_i)); }


          Set<T>&                                     clear();

          Set<T>&                                     clear(const std::vector<const char*>& properties, const std::vector<const char*>& elem_properties);

          double                                      distance(const Set& reference, double strands_per_acs = 0.0, bool add_extra = false) const;

          /*! Returns the indices of current set in 'matched_indices' that form the closest match with the reference set.
           *
           */
          double                                      distance(const Set& reference,
                                                                std::vector<size_t>& matched_indices, double strands_per_acs = 0.0, bool add_extra = false) const;
          ...
          MR::Math::Matrix<double>                    similarity_matrix(const Set& reference, double strands_per_acs = 0.0) const
            { MR::Math::Matrix<double> matrix(this->size(), reference.size()); similarity_matrix(reference, matrix, strands_per_acs); return matrix; }


          void                                        similarity_matrix(const Set& reference, MR::Math::Matrix<double>& similarity, double strands_per_acs = 0.0) const;
          ...
          std::string                                get_extend_prop(const std::string& key)
            { return (ext_props && ext_props->count(key)) ? (*ext_props)[key] : throw Exception ("'" + key + "' key was not found in extended properties."); }
          ...
          void                                        set_extend_elem_prop(std::string key, std::string value, size_t elem_index);


          std::string                                 get_extend_elem_prop(std::string key, size_t elem_index) const;


          template <typename U> U                     get_extend_elem_prop(std::string key, size_t elem_index) const
          ...
          void                                        set_extend_elem_prop_row(const std::map<std::string,std::string>& properties, size_t row_index, bool ignore_missing = false);


          template <typename U> inline void           copy_extend_elem_prop_row(const Set<U>& set, size_t elem_index, size_t this_index)
            {  this->set_extend_elem_prop_row(set.get_extend_elem_prop_row(elem_index), this_index, true); }


          template <typename U> inline void           copy_extend_elem_prop_row(const Set<U>& set, size_t elem_index)
...
//        protected:
          ...
          template <typename V> void                  save_tpl(const std::string& location, V& writer) const;
...
//        protected:
          ...
          void                                        add_extend_elem_prop_row(const std::map<std::string,std::string>& properties);
           ...
           */
          void                                        add_elem_prop(const char* const name, double value = NAN, bool ignore_present=true);
          ...
          bool      	                                has_elem_prop(const char* name) const
            { for (size_t elem_prop_i = 0; elem_prop_i < num_elem_props(); ++elem_prop_i) { if (name == elem_props->operator[](elem_prop_i)) return true; } return false; }
          ...
          std::vector<std::string>&                   insert_elem_prop_keys(std::vector<std::string>& header) const;
           ...
           */
          void                                        resize(size_t size, double fill_value = NAN, size_t new_elem_degree = 0, size_t new_vsize = 0);
          ...
          MR::Math::Matrix<double>  bmatrix()
            { assert(!var_elem_degrees()); return MR::Math::Matrix<double>(data, size(), rsize, stride()); }
        ...
        protected:
          ...
          size_t            	                        row_size(size_t row_index) const
            { return !var_elem_degrees() ? rsize : (row_ends ? (!row_index ? row_ends[0] : row_ends[row_index] - row_ends[row_index -1]) : 0); }
          ...
          size_t            	                        row_start(size_t row_index) const
            { return !var_elem_degrees() ? row_index * rsize : (row_ends ? (row_index ? row_ends[row_index-1] : 0) : 0); }
          ...
          size_t            	                        row_end(size_t row_index) const
            { return !var_elem_degrees() ? (row_index + 1) * rsize : ( row_ends ? row_ends[row_index] : 0); }
          ...
          void              	                        calculate_row_ends(size_t* elem_degrees, size_t elem_row_size,
            ...
            for (size_t row_i = 0; row_i < size(); ++row_i)
              row_ends[row_i] = row_start + elem_degrees[row_i] * elem_row_size + num_elem_props;
