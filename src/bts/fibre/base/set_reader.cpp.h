...
namespace BTS {
	...
	namespace Fibre {
      ...
      template <typename T> bool   SetReader<T>::next (T& set) {
        ...
        if (Reader<T>::next_basic(set)) {
          ...
          if (elem_prop_hdr.size()) {
            ...
            for (std::vector<std::string>::iterator prop_it = ext_elem_prp_keys.begin(); prop_it != ext_elem_prp_keys.end(); ++prop_it)
            ...
            while (!is_row_seperator(line)) {
              ...
              if (elem_index >= set.size())
                throw Exception ("Loaded properties contain more rows than elements in the set");

              std::map<std::string,std::string> elem_prop_row = Reader<T>::parse_props_line(elem_prop_hdr, line);

              //FIXME: Needs to be implemented
              std::map<std::string,std::string> ext_elem_prop_row = set[elem_index].extract_and_set_props(elem_prop_row);
      ...
      template <typename T> bool   SetReader<T>::is_row_seperator(const std::string& line) {
        ...
        if (line.size() > 12)
          is_seperator = !strcmp(line.substr(0,8).c_str(), "--- END ") && !strcmp(line.substr(line.size()-4, 4).c_str(), " ---");
