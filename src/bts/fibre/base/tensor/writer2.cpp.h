...
namespace BTS {
	...
	namespace Fibre {
      ...
      template <typename T> void Writer2<T>::create (const std::string& location, const T& template_value) {
        ...
        for (std::vector<std::string>::iterator key_it = header.begin(); key_it != header.end(); ++key_it)
          out << *key_it << " ";
      ...
      template <typename T> void Writer2<T>::append(const MR::Math::Matrix<double>& tensor) {
        ...
        if (template_length != tensor.rows())
            throw Exception ("Length of tensor matrix (" + str(tensor.rows()) + ") did not match size of template value vector (" + str(template_length) + ").");
