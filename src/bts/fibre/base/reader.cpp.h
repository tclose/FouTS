...
namespace BTS {
  ...
  namespace Fibre {
    ...
    namespace Base {
      ...
      template <typename T> const std::string   Reader<T>::PROP_FILE_PREAMBLE = "%%% Extended Properties File %%% - keys: ";
      ...
      template <typename T> void                Reader<T>::open (const std::string& location) {
        ...
        while (kv.next()) {
          ...
          if (key == "file") data_file = kv.value();
          else if (key == "datatype") dtype.parse (kv.value());
          else if (key != "roi") ext_props[key] = kv.value();
        }

        if (dtype == MR::DataType::Undefined) throw Exception ("no datatype specified in fibre object file '" + location + "'");
        if (dtype != MR::DataType::Float32LE && dtype != MR::DataType::Float32BE &&
            dtype != MR::DataType::Float64LE && dtype != MR::DataType::Float64BE)
          throw Exception ("only supported datatype for tracks location are Float32LE, Float32BE, Float64LE & Float64BE (in tracks location \"" + location + "\")");

        if (data_file.empty()) throw Exception ("missing 'file' specification in fibre object file '" + location + "'");
        ...
        if (locations_stream.good()) {
          try { locations_stream >> offset; }
          catch (...) { throw Exception ("invalid data offset specified, \"" + fname + "\", in fibre object file '" + location + "'"); }
        }

        if (fname != ".") fname = MR::Path::join (MR::Path::dirname (location), fname);
        else fname = location;

        in.open (fname.c_str(), std::ios::in | std::ios::binary);
        if (!in) throw Exception ("error opening fibre object file \"" + fname + "\": " + strerror(errno));
        in.seekg (offset);
      ...
      template <typename T> bool                Reader<T>::next (T& fibre_object, std::map<std::string,std::string>& properties_row) {
        ...
        if (next_basic(fibre_object)) {
          ...
          if (prop_hdr.size()) {
            ...
            if (!std::getline(prop_in, line))
              throw Exception ("No corresponding line found in extended properties file.");
      ...
      template <typename T> template <typename U> inline void   Reader<T>::read (U& fibre_object, const Coord& first_t) {
      ...
      template <typename T>  std::map<std::string,std::string>   Reader<T>::parse_props_line(const std::vector<std::string>& header, const std::string& line) {
        ...
        while(std::getline(line_stream, value, '\t')) {
          if (count > header.size())
            throw Exception ("More elements found on line  (" + str(count) + ") than exist in header (" + str(header.size()) + "). The line was {" + line + "} and the header is " + str(header) + ".");
      ...
      template <typename T> void       Reader<T>::read_props_preamble(std::ifstream& file_in) {
        ...
        if (strcmp(file_intro, PROP_FILE_PREAMBLE.c_str()))
          throw Exception ("Required file intro '" +PROP_FILE_PREAMBLE + "' was not found at start of extended properties file (found '" + std::string(file_intro) +"' instead).");

      }


      template <typename T> std::vector<std::string>      Reader<T>::read_props_header(std::ifstream& file_in) {
