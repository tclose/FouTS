    ...
    namespace Base {
      ...
      class Object : public MR::Math::Vector<double> {
        ...
        public:
           ...
           */
          static std::string              load_matlab_str(const std::string& location, double scale = 1.0);
           ...
           */
          template <typename T> static std::vector<const char*> extract_props(std::vector<std::string>& combined_header);
        ...
        public:
          ...
          Object (size_t size, size_t v_size, const std::vector<const char*>& properties)
            : MR::Math::Vector<double> (v_size),
              sze(size),
              props(new std::vector<const char*>(properties)) {
        ...
        public:
          ...
          Object(const Object& b)
            : MR::Math::Vector<double>(b), sze(b.sze), props(new std::vector<char const*>(*b.props)) {}
           ...
           */
          size_t      	    bsize() const
             { assert(MR::Math::Vector<double>::size() >= num_props()); return MR::Math::Vector<double>::size() - num_props(); }
          ...
          bool      	      has_prop(const char* name) const
            { for (size_t prop_i = 0; prop_i < num_props(); ++prop_i) { if (name == props->operator[](prop_i)) return true; } return false; }
          ...
          double&   	      prop(size_t index)
            { assert(index < num_props()); return MR::Math::Vector<double>::operator[](bsize() + index); }
          ...
          const char*   	  prop_key(size_t index) const
            { assert(index < num_props()); return props->operator[](index); }
          ...
          Object            operator-() const
            { Object answer (*this); answer.invert(); return answer; }
          ...
          double            inner_product(const Object& fibre)
            { assert(props_match(fibre)); return MR::Math::dot(*this,fibre); }
           ...
           */
          std::map<std::string,std::string>& insert_props(std::map<std::string,std::string>& props) const;
           ...
           */
          std::vector<std::string> insert_prop_keys(const std::vector<std::string>& props) const;
      ...
      template <typename T> std::map<const char*,double>  Object::extract_props(std::map<std::string,std::string>& loaded_props) {
      ...
      template <typename T> std::vector<const char*>   	  Object::extract_props(std::vector<std::string>& header) {
      ...
      template <typename T> std::vector<const char*>      Object::select_props(const std::vector<const char*>& properties) {
      ...
      template <typename T> std::map<std::string,std::string>&  Object::extract_and_set_props(std::map<std::string,std::string>& combined_properties) {

        std::map<const char*, double> props = extract_props<T>(combined_properties);

        for (std::map<const char*, double>::iterator prop_it = props.begin(); prop_it != props.end(); ++prop_it)
      ...
      inline std::map<std::string,std::string>& Object::insert_props(std::map<std::string,std::string>& properties) const {
