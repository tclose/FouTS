...
namespace BTS {
    ...
    namespace Fibre {
        ...
        const char* Tractlet::Set::CHARACTERISTIC_PROPS_LIST[] = {
                Tractlet::ACS_EXT_PROP, Tractlet::AVG_DENSITY_EXT_PROP,
                Tractlet::WIDTH_EPSILON_COMPONENT_EXT_PROP,
                Tractlet::LENGTH_EPSILON_COMPONENT_EXT_PROP, PROPS_LIST_END };
        
        Tractlet::Set::Set(const Strand::Set& strands)
                : Fibre::Base::Set<Tractlet>(select_props<Set>(*strands.props),
                        select_props<Tractlet>(*strands.elem_props), strands.get_extend_props()) {
        ...
        //TODO: Look into preallocating for speed
        Tractlet::Set::Set(const Strand::Set& strands, size_t num_tractlets,
        ...
        //TODO: Look into preallocating for speed
        Tractlet::Set::Set(const Track::Set& tcks, size_t degree, size_t num_tractlets,
                           const std::string& cluster_save_location)
        ...
        void Tractlet::Set::from_strands(const Strand::Set& strands) {
            ...
            if (!strands.has_extend_elem_prop(BUNDLE_INDEX_EPROP))
                throw Exception(
                        "'" + BUNDLE_INDEX_EPROP
                        + "' property was not found in strand set, which is required for conversion to tractlet set.");
            ...
            for (size_t strand_i = 0; strand_i < strands.size(); ++strand_i)
                bundle_indices.insert(
                        to<double>(strands.get_extend_elem_prop(BUNDLE_INDEX_EPROP, strand_i)));
            
            //Loop through the strand set and convert each bundle into a separate tractlet.
            for (std::set<size_t>::iterator bundle_it = bundle_indices.begin();
                    bundle_it != bundle_indices.end(); ++bundle_it) {
...
//    }
        ...
        std::vector<std::string>& Tractlet::Set::append_characteristic_keys(
        ...
        void Tractlet::Set::set_characteristics() {
            ...
            for (size_t tractlet_i = 0; tractlet_i < size(); ++tractlet_i) {
                set_extend_elem_prop(Tractlet::ACS_EXT_PROP, str(operator[](tractlet_i).acs()),
                        tractlet_i);
                set_extend_elem_prop(Tractlet::LENGTH_EPSILON_COMPONENT_EXT_PROP,
                        str(length_epsilon * MR::Math::sqrt(operator[](tractlet_i)(0, 1).norm())),
                        tractlet_i);
                set_extend_elem_prop(Tractlet::WIDTH_EPSILON_COMPONENT_EXT_PROP,
                        str(
                                width_epsilon * (operator[](tractlet_i)(1, 0).norm()
                                        + operator[](tractlet_i)(2, 0).norm())), tractlet_i);
                set_extend_elem_prop(Tractlet::AVG_DENSITY_EXT_PROP,
                        str(operator[](tractlet_i).average_density()), tractlet_i);
            }
        }
        ...
        void Tractlet::Set::load(const std::string& location, size_t degree, size_t num_tractlets) {
            ...
            if (File::has_or_txt_extension<Tractlet>(location)) {
                ...
                this->Base::Set<Tractlet>::load(location);
                    ...
                    || File::has_or_txt_extension<Track>(location)) {
                ...
                if (num_tractlets)
                    ...
                    throw Exception(
                            " \"-width\" parameter is required for implicit conversion to tractlet set ('" + location
                            + "') if '" + Track::BUNDLE_INDEX_EPROP + "' property is not present.");
                
            } else
                throw Exception(
                        "The extension of file \"" + location + "\" is not a recognised type (\""
                        + Tractlet::FILE_EXTENSION + "\", \"" + Strand::FILE_EXTENSION + "\" or \""
                        + Track::FILE_EXTENSION + "\").");
            
        }
        
        void Tractlet::Set::save(const std::string& location, size_t num_length_sections,
                                 double strands_per_acs) const {
            ...
            if (File::has_or_txt_extension<Tractlet>(location)) {
                ...
                if (strands_per_acs <= 0.0)
                    throw Exception(
                            " \"-strands_per_acs\" parameter is required for implicit conversion to strand set ('" + location
                            + "').");
                ...
                if (num_length_sections == 0)
                    throw Exception(
                            " \"-num_length_sections\" parameter is required for implicit conversion to track set ('" + location
                            + "').");
                
                if (strands_per_acs <= 0.0)
                    throw Exception(
                            " \"-strands_per_acs\" parameter is required for implicit conversion to track set ('" + location
                            + "').");
                ...
                throw Exception(
                        "The extension of file \"" + location + "\" is not a recognised type (\""
                        + Tractlet::FILE_EXTENSION + "\" or \"" + Strand::FILE_EXTENSION + "\").");
        ...
        std::string Tractlet::Set::matlab_str() const {
            ...
            for (size_t tractlet_i = 0; tractlet_i < size(); tractlet_i++) {
                ...
                for (size_t ax_i = 0; ax_i < 3; ax_i++) {
                    ...
                    for (size_t degree_i = 0; degree_i < operator[](tractlet_i)[ax_i].size();
                            degree_i++) {
        ...
        void Tractlet::Set::redegree(size_t new_degree, double default_value) {
            ...
            if (!is_owner())
                throw Exception(
                        "Redegree cannot be called when tractlet set does not an own the underlying data");
            ...
            for (size_t tractlet_i = 0; tractlet_i < size(); ++tractlet_i)
                for (size_t ax_i = 0; ax_i < degree(); ++ax_i)
                    for (size_t degree_i = 0; degree_i < degree(); ++degree_i)
                        new_set[tractlet_i](ax_i, degree_i) = operator[](tractlet_i)(ax_i,
                                degree_i);
