    ...
    namespace Fibre {
        ...
        class Tractlet::Set: public Base::Set<Tractlet> {
            ...
            public:
                ...
                static std::vector<std::string>& append_characteristic_keys(
            ...
            public:
                ...
                Set(const std::map<std::string, std::string>& extended_props = Properties())
                        : Base::Set<Tractlet>(std::vector<const char*>(),
                                std::vector<const char*>(), extended_props) {
                }
                 ...
                 * @param extended_props The extended properties of the set
                 */
                Set(size_t size, size_t degree,
                    const std::map<std::string, std::string>& extended_props = Properties())
                        : Base::Set<Tractlet>(size, degree, 9 * degree, std::vector<const char*>(),
                                std::vector<const char*>(), extended_props) {
                }
                 ...
                 * @param extended_props The extended properties of the set
                 */
                Set(size_t size, size_t degree, const std::vector<const char*>& props,
                    const std::vector<const char*>& elem_props,
                    const std::map<std::string, std::string>& extended_props = Properties())
                        : Base::Set<Tractlet>(size, degree, 9 * degree + elem_props.size(), props,
                                elem_props, extended_props) {
                }
                
                Set(std::string file_location, size_t degree = 0, size_t num_tractlets = 0)
                        : Base::Set<Tractlet>(std::vector<const char*>(),
                                std::vector<const char*>(), std::map<std::string, std::string>()) {
                ...
                Set(const Strand::Set& set, size_t num_tractlets,
                ...
                Set(const Track::Set& set, size_t degree, size_t num_tractlets,
                    const std::string& cluster_save_location = "");
                ...
                size_t degree() const {
                    if (var_elem_degrees())
                        throw Exception(
                                "Tractlet set has variable rows, so its 'degree' is undefined");
                    return elem_dgree;
                }
                ...
                void save(const std::string& location, size_t num_length_sections = 0,
                          double strands_per_acs = -1.0) const;
                ...
                Tractlet::Set peel(double inclusion_radius, size_t num_length_sections,
                                   size_t num_width_sections, bool to_cube = false) const;
                ...
                Set smallest_distance_set(const Set& reference) {
                    Set smallest;
                    Base::Set<Tractlet>::smallest_distance_set(reference, smallest);
                    return smallest;
                }
                ...
                Set& operator+=(const Set& set) {
                    Base::Set<Tractlet>::operator+=(set);
                    return *this;
                }
            ...
            private:
                 ...
                 */
                void from_strands(const Strand::Set& strands, size_t num_clusters,
                                  const std::string& cluster_save_location = "");
