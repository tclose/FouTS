...
namespace FTS {
    ...
    namespace Fibre {
        ...
        const std::string Tractlet::Set::Walker::STEP_LOCATION_DEFAULT = str(getenv("HOME"))
                + str("/git/FouTS/params/fibre/tract/masks/mcmc/metropolis/default.tct");
        
        Tractlet::Set::Walker* Tractlet::Set::Walker::factory(
                const Tractlet::Set& state, const std::string& type, double step_scale,
            ...
            if (type == "standard") {
                ...
                if (step_location.size()) {
                    ...
                    if (!step.props_match(state))
                        throw Exception(
                                "Properties do not match between loaded properties ( " + str(
                                        step.prop_keys())
                                + "," + str(step.elem_prop_keys()) + ") and state properties ("
                                + str(state.prop_keys()) + ", " + str(state.elem_prop_keys())
                                + ").");
        ...
        void Tractlet::Set::Walker::step(const Tractlet::Set& current, Tractlet::Set& proposed,
                                         double scalar) const {
