...
//Public member functions
            public:
                ...
                Tensor& operator=(const Tensor& t) {
                    Base::Tensor<Strand>::operator=(t);
                    return *this;
                }
                
                Tensor& operator=(const MR::Math::Matrix<double>& m) {
                    Base::Tensor<Strand>::operator=(m);
                    return *this;
                }
                
                Tensor& operator=(double c) {
                    Base::Tensor<Strand>::operator=(c);
                    return *this;
                }
                ...
                Strand acs() {
                    assert(has_var_acs());
                    return Strand(degree(),
                            MR::Math::Matrix<double>::row(
                                    vsize() + tmpl.prop_index(Strand::ALPHA_PROP)), tmpl.props, 0);
                }
                
                double& operator()(size_t row_degree_i, size_t row_dim_i, size_t col_degree_i,
                                   size_t col_dim_i) {
                    return MR::Math::Matrix<double>::operator()(row_degree_i * 3 + row_dim_i,
                            col_degree_i * 3 + col_dim_i);
                }
                
                double operator()(size_t row_degree_i, size_t row_dim_i, size_t col_degree_i,
                                  size_t col_dim_i) const {
                    return MR::Math::Matrix<double>::operator()(row_degree_i * 3 + row_dim_i,
                            col_degree_i * 3 + col_dim_i);
                }
                ...
                void add_section_hessian(const Strand& strand, const Section& section,
