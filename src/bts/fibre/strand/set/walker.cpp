...
namespace BTS {
	...
	namespace Fibre {
	  ...
	  const char*        Strand::Set::Walker::STEP_LOCATION_DEFAULT = "/home/tclose/data/mcmc/metropolis/params/walker/scaling.str.txt";
    const double       Strand::Set::Walker::STEP_SCALE_DEFAULT = 0.01;

    Strand::Set::Walker*      Strand::Set::Walker::factory(const Strand::Set& state,
          ...
          if (step.size() == 1) {
            ...
            for (size_t tract_i = 0; tract_i < state.size(); ++tract_i) {
              ...
              if (state[tract_i].size() > step_template.size())
                throw Exception ("Strand (" + str(tract_i) + ") is larger than step template.");
    ...
    void                      Strand::Set::Walker::step(const Strand::Set& current, Strand::Set& proposed, double scalar) {
