        ...
        class Strand::Section: public Strand::BasicSection {
            ...
            public:
                ...
                Section(size_t init_precalc_size = 0)
                        : length_fraction(NAN), precalc_weightings(init_precalc_size), precalc_weight_gradients(
                                  init_precalc_size), precalc_weight_hessians(init_precalc_size) {
                }
                
                //TODO: Look into this, this is not the most efficient way to do this but will have to do until image generation
                // is reworked.
                Section(const Strand& strand, const MR::Math::Vector<double>::View& position_coeffs,
                        const MR::Math::Vector<double>::View& tangent_coeffs,
                        double length_fraction, size_t init_precalc_size = 0)
                        
                        : BasicSection(strand, position_coeffs, tangent_coeffs, length_fraction), precalc_weightings(
                                  init_precalc_size), precalc_weight_gradients(init_precalc_size), precalc_weight_hessians(
                                  init_precalc_size)
                ...
                Section(const Section& s)
                        : BasicSection(s), position_coeffs(s.position_coeffs), tangent_coeffs(
                                  s.tangent_coeffs), length_fraction(s.length_fraction), precalc_weightings(
                                  s.precalc_weightings), precalc_weight_gradients(
                                  s.precalc_weight_gradients), precalc_weight_hessians(
                                  s.precalc_weight_hessians), precalc_interpolation(
                                  s.precalc_interpolation), precalc_interp_gradient(
                                  s.precalc_interp_gradient), precalc_interp_hessian(
                                  s.precalc_interp_hessian), parent(s.parent) {
                }
                
                // Used in derived classes
            protected:
                
                Section(size_t size, const std::vector<const char*>& props,
                        size_t init_precalc_size)
                        : BasicSection(size, props), length_fraction(NAN), precalc_weightings(
                                  init_precalc_size), precalc_weight_gradients(init_precalc_size), precalc_weight_hessians(
                                  init_precalc_size) {
                }
                
                Section(size_t size, const MR::Math::Vector<double>::View& view,
            ...
            protected:
                ...
                Section(double intensity, const MR::Math::Vector<double>::View& position_coeffs,
                        const MR::Math::Vector<double>::View& tangent_coeffs,
                        double length_fraction) {
                ...
                void set(double intensity, const MR::Math::Vector<double>::View& position_coeffs,
                         const MR::Math::Vector<double>::View& tangent_coeffs,
                         double length_fraction) {
