...
namespace BTS {
    ...
    namespace Fibre {
        ...
        const char* Strand::Set::PROPS_LIST[] = { Base::Set<Strand>::BASE_INTENSITY_PROP,
                                                  PROPS_LIST_END };
        ...
        Strand::Set::Set(const Track::Set& tcks, size_t degree)
                : Base::Set<Strand>(tcks.size(), degree,
                        3 * degree + select_props<Strand>(*tcks.elem_props).size(),
                        select_props<Set>(*tcks.props), select_props<Strand>(*tcks.elem_props),
            ...
            for (size_t tck_i = 0; tck_i < size(); tck_i++) {
                ...
                if (!degree) {
                    if (has_extend_elem_prop(Track::Set::ORIGINAL_DEGREE_PROP))
                        strand_degree = get_extend_elem_prop<size_t>(
                                Track::Set::ORIGINAL_DEGREE_PROP, tck_i);
                    else
                        throw Exception(
                                "Degree must be supplied (and cannot be 0) for conversion from tracks to strands.");
                }
        ...
        Strand::Set::Set(const Tractlet::Set& set, size_t num_samples)
                : Fibre::Base::Set<Strand>(select_props<Set>(*set.props),
                        select_props<Strand>(*set.elem_props), set.get_extend_props()) {
            ...
            for (size_t tractlet_i = 0; tractlet_i < set.size(); tractlet_i++) {
                ...
                if (new_strands.size()) {
                    ...
                    for (size_t strand_i = 0; strand_i < new_strands.size(); strand_i++) {
                        ...
                        set_extend_elem_prop(BUNDLE_INDEX_EPROP, str(tractlet_i),
                                strand_i + old_size);
                    }
                }
            }
        ...
        Strand::Set::Set(const Tractlet::Set& set, double strands_per_vol)
            ...
            for (size_t tract_i = 0; tract_i < set.size(); tract_i++) {
                ...
                std::string bundle_index =
                        set.has_extend_elem_prop(BUNDLE_INDEX_EPROP) ? set.get_extend_elem_prop(
                                                                               BUNDLE_INDEX_EPROP,
                                                                               tract_i) :
                                                                       str(tract_i);
        ...
        Strand::Set::Set(const Tractlet& tractlet, size_t num_width_sections)
                : Fibre::Base::Set<Strand>(std::vector<const char*>(),
                        select_props<Strand>(*tractlet.props), std::map<std::string, std::string>()) {
            ...
            for (double ax1_frac = (-1.0 + width_fraction); ax1_frac < 1.0;
                ...
                for (double ax2_frac = (-1.0 + width_fraction); ax2_frac < 1.0;
                    ...
                    if (MR::Math::pow2(ax1_frac) + MR::Math::pow2(ax2_frac) <= (1.0
                        ...
                        Strand strand(
                                tractlet[0] + tractlet[1] * ax1_frac + tractlet[2] * ax2_frac);
        ...
        Tractlet::Set Strand::Set::to_tractlets(size_t num_tractlets,
                                                const std::string& cluster_save_location) const {
        ...
        Strand::Set Strand::Set::shortest_span() const {
            ...
            if (size()) {
                ...
                while (num_iterations < MAX_ITERATIONS) {
                    ...
                    for (size_t strand_i = 0; strand_i < size(); ++strand_i)
                        new_distance += strands[strand_i].distance(centroid,
                                (bool&) flip_pattern[strand_i]);
        ...
        Strand::Set Strand::Set::flip(const std::vector<size_t>& to_flip) const {
            ...
            if (to_flip.size() != size())
                throw Exception(
                        "Size of 'to_flip' vector (" + str(to_flip.size())
                        + ") does not match that of strand set (" + str(size()) + ").");
        ...
        Strand::Set Strand::Set::peel(double distance, size_t num_length_sections,
                                      bool to_cube) const {
        ...
        Strand::Set& Strand::Set::cluster(size_t num_clusters) {
            ...
            if (num_clusters > size())
                throw Exception(
                        "New number of clusters (" + str(num_clusters)
                        + ") must be less than or equal to original number of strands ("
                        + str(size()) + ")");
        ...
        Strand::Set Strand::Set::reduce_number(size_t new_num_strands) {
            ...
            if (new_num_strands > this->size())
                throw Exception(
                        "New number of strands (" + str(new_num_strands)
                        + ") must be less than original number of strands (" + str(this->size())
                        + ")");
...
//      ctrs.getAssignments(closeCtr, sqDist);
        ...
        std::vector<size_t> Strand::Set::bundle(size_t bundle_index) const {
            ...
            if (!this->has_extend_elem_prop(Track::BUNDLE_INDEX_EPROP))
                throw Exception(
                        "Bundle index property (" + Track::BUNDLE_INDEX_EPROP
                        + ") not set on strand set.");
        ...
        Strand::Set Strand::Set::bundle_backbones() const {
            ...
            for (size_t bundle_i = 0; bundle_i < bundle_indices.size(); ++bundle_i) {
                ...
                properties_row[Track::BUNDLE_INDEX_EPROP] = str(
                        operator[](bundle_indices[bundle_i]));
        ...
        void Strand::Set::load(const std::string& location, size_t degree, double strands_per_acs) {
            ...
            if (File::has_or_txt_extension<Strand>(location)) {
                ...
                if (degree < 1)
                    throw Exception(
                            " '-degree' parameter is required for implicit conversion from track set ('" + location
                            + "').");
                ...
                for (size_t tck_i = 0; tck_i < tcks.size(); tck_i++)
                    this->set_extend_elem_prop(ORIGINAL_NUM_POINTS_PROP,
                            str(tcks[tck_i].num_points()), tck_i);
                ...
                throw Exception(
                        ...
                        + Strand::FILE_EXTENSION + "\" or \"" + Track::FILE_EXTENSION + "\").");
        ...
        Strand::Set Strand::Set::permute(const std::vector<size_t>& indices) const {
            ...
            if (var_elem_degrees())
                throw Exception(
                        "'permute' is currently not implemented for variable element size sets.");
        ...
        void Strand::Set::save(const std::string& location, size_t num_points,
            ...
            if (File::has_or_txt_extension<Strand>(location)) {
                ...
                if (num_points != 0)
                    ...
                    for (size_t strand_i = 0; strand_i < this->size(); strand_i++) {
                        ...
                        size_t loaded_num_points = get_extend_elem_prop<size_t>(
                                ORIGINAL_NUM_POINTS_PROP, strand_i);
                        tcks.push_back(this->operator[](strand_i).to_track(loaded_num_points),
                                this->get_extend_elem_prop_row(strand_i));
                        
                    }
                    
                } else
                    throw Exception(
                            " \"-num_points\" parameter is required for implicit conversion to track set ('" + location
                            + "').");
                ...
                std::map<std::string, std::string>::const_iterator num_tract_it = properties.find(
                        ORIGINAL_NUM_TRACTLETS_PROP);
                
                if (num_tractlets)
                    tcts = this->to_tractlets(num_tractlets,
                            File::strip_extension(location) + ".clustered.str");
                    ...
                    throw Exception(
                            ...
                            + "') if '" + Track::BUNDLE_INDEX_EPROP + "' is not present.");
        ...
        void Strand::Set::redegree(size_t new_degree, double default_value) {
            ...
            if (!is_owner())
                throw Exception(
                        "Redegree cannot be called when strand set does not an own the underlying data");
