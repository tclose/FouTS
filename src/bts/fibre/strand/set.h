        ...
        class Strand::Set: public Fibre::Base::Set<Strand> {
            ...
            public:
                ...
                Set(size_t size, size_t degree,
                    const std::map<std::string, std::string>& extended_props = Properties())
                        : Fibre::Base::Set<Strand>(size, degree, degree * 3,
                ...
                Set(size_t size, size_t degree, const std::vector<const char*>& props,
                    const std::vector<const char*>& elem_props,
                    const std::map<std::string, std::string>& extended_props = Properties())
                        : Fibre::Base::Set<Strand>(size, degree, degree * 3 + elem_props.size(),
                                props, elem_props, extended_props) {
                }
                ...
                Tractlet::Set to_tractlets(size_t num_tractlets,
...
//        Permutation                                 default_permutation();
                ...
                Set smallest_distance_set(const Set& reference) const {
                    Set smallest;
                    this->Fibre::Base::Set<Strand>::smallest_distance_set(reference, smallest);
                    return smallest;
                }
                ...
                size_t degree() const {
                    if (var_elem_degrees())
                        throw Exception(
                                "Strand set has variable rows, so its 'degree' is undefined");
                    return elem_dgree;
                }
                
                void load(const std::string& path, size_t degree = 0, double strands_per_width =
                        -1.0);
                ...
                void save(const std::string& path, size_t num_sections = 0,
                          size_t num_tractlets = 0) const;

                Set& operator+=(const Set& set) {
                    Fibre::Base::Set<Strand>::operator+=(set);
                    return *this;
                }
                Set& operator-=(const Set& set) {
                    Fibre::Base::Set<Strand>::operator-=(set);
                ...
                Set& operator*=(double c) {
                    Fibre::Base::Set<Strand>::operator*=(c);
                    return *this;
                }
        ...
        double distance(Strand::Set& strands, Strand::Set& reference,
