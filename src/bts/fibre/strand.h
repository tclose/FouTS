    ...
    namespace Fibre {
        ...
        class Strand: public Base::Object {
            ...
            public:
                ...
                static MR::Math::Matrix<double> create_position_matrix(size_t degree,
                                                                       size_t num_sections,
                                                                       bool include_endpoints =
                                                                               INCLUDE_ENDPOINTS);
                // Generate conversion matrix to convert Strand descriptors to path points from arbitrary 'time' points.
                static MR::Math::Matrix<double> create_position_matrix(
                ...
                static MR::Math::Matrix<double> create_tangent_matrix(size_t degree,
                                                                      size_t num_sections,
                                                                      bool include_endpoints =
                                                                              INCLUDE_ENDPOINTS);
                static MR::Math::Matrix<double> create_inverse_position_matrix(
                        size_t num_sections, size_t degree, bool include_endpoints =
                                INCLUDE_ENDPOINTS);
                static MR::Math::Matrix<double> create_inverse_position_matrix(
                        const std::vector<double>& timeline, size_t degree);

                static const MR::Math::Matrix<double>& position_matrix(
                        size_t num_sections, size_t degree, bool include_endpoints =
                                DONT_INCLUDE_ENDPOINTS);
                static const MR::Math::Matrix<double>& tangent_matrix(size_t num_sections,
                                                                      size_t degree);
                static const MR::Math::Matrix<double>& inverse_position_matrix(size_t num_sections,
                                                                               size_t degree);

                static const MR::Math::Matrix<double>& position_matrix_w_endpoints(
                        size_t num_sections, size_t degree);
                static const MR::Math::Matrix<double>& tangent_matrix_w_endpoints(
                        size_t num_sections, size_t degree);
                static const MR::Math::Matrix<double>& inverse_position_matrix_w_endpoints(
                        size_t num_sections, size_t degree);

                static Strand outer_product(const MR::Math::Vector<double>& column_vector,
                                            const Coord& coord);
            ...
            protected:
                 ...
                 */
                Strand(size_t degree, const MR::Math::Vector<double>::View& view,
                       std::vector<const char*>* props, const Base::Set<Strand>* parent)
            ...
            public:
                ...
                //Conversion methods.
                Track to_track(size_t num_points,
                               bool include_endpoints = DONT_INCLUDE_ENDPOINTS) const;

                Track to_tangents(size_t num_points,
                                  bool include_endpoints = DONT_INCLUDE_ENDPOINTS) const;
                ...
                void add_section_gradient(const Strand& strand, const Section& section,
                ...
                double distance(const Strand& reference, double dummy = 0) const {
                    bool flipped;
                    return distance(reference, flipped);
                }
                ...
                static Strand merge(std::vector<const Strand*>& strands, size_t num_sections,
                                    size_t new_degree = 0);

                Strand merge(const Strand& strand, size_t num_sections,
                             size_t new_degree = 0) const;
