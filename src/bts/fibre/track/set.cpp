...
namespace BTS {
  ...
  namespace Fibre {
    ...
    const char*                         Track::Set::PROPS_LIST[] = { Base::Set<Track>::BASE_INTENSITY_PROP, PROPS_LIST_END };
    ...
    Track::Set::Set(const Tractlet::Set& tractlets, size_t num_length_sections, size_t num_width_sections)
      : Base::Set<Track>(tractlets.size(),
                          num_length_sections,
                          num_length_sections * 3 + select_props<Strand>(*tractlets.elem_props).size(),
                          select_props<Set>(*tractlets.props),
                          select_props<Strand>(*tractlets.elem_props),
                          tractlets.get_extend_props()) {
    ...
    Track::Set::Set(const Tractlet::Set& tractlets, size_t num_length_sections, double strands_per_acs)
      : Base::Set<Track>(tractlets.size(),
                        num_length_sections,
                        num_length_sections * 3 + select_props<Strand>(*tractlets.elem_props).size(),
                        select_props<Set>(*tractlets.props),
    ...
    void                                Track::Set::from_strands(const Strand::Set& strands, size_t num_length_sections) {
      ...
      for (size_t strand_i = 0; strand_i < size(); strand_i++) {
        ...
        set_extend_elem_prop(Track::Set::ORIGINAL_DEGREE_PROP, str(strands[strand_i].degree()), strand_i);
    ...
    void                                Track::Set::remove_short_tracks(size_t min_num_points) {
      ...
      for (size_t tck_i = 0; tck_i < this->size(); tck_i++) {
        ...
        if (operator[](tck_i).num_points() < min_num_points) {
          ...
          std::cout << std::endl << "Erased tck " << tck_i << " as it only had " << operator[](tck_i).num_points() << " control points." << std::endl;
    ...
    void                                Track::Set::load(const std::string& location, size_t num_points, double strands_per_acs) {
      ...
      if (File::has_or_txt_extension<Track>(location))
        ...
        if (num_points < 1)
          throw Exception(" '-num_points' parameter is required for implicit conversion from strand set ('" + location + "').");
        ...
        for (size_t strand_i = 0; strand_i < strands.size(); strand_i++)
          this->set_extend_elem_prop(ORIGINAL_DEGREE_PROP, str(strands[strand_i].degree()), strand_i);
        ...
        for (size_t tck_i = 0; tck_i < size(); tck_i++) {
          ...
          set_extend_elem_prop(ORIGINAL_DEGREE_PROP, str(tractlets[tract_i].degree()), tck_i);
        }


      } else
         throw Exception("The extension of file \"" + location + "\" is not a recognised type (\"" + Track::FILE_EXTENSION + "\" or \"" + Strand::FILE_EXTENSION + "\").");
      
    }

    
    void                                Track::Set::save(const std::string& location, size_t degree, size_t num_tractlets) const {
      ...
      if (File::has_or_txt_extension<Track>(location)) {
        ...
        if (degree)
          ...
          for (size_t tck_i = 0; tck_i < this->size(); tck_i++)  {
            size_t loaded_degree = to<int>(this->get_extend_elem_prop(ORIGINAL_DEGREE_PROP, tck_i));
          ...
          throw Exception(" '-degree' is required for implicit conversion to strand set ('" + location + "').");
        ...
        if (!has_extend_elem_prop(BUNDLE_INDEX_EPROP) && !num_tractlets)
          throw Exception(" '-num_tractlets' is required for implicit conversion to tractlet set ('" + location + "') if '" + BUNDLE_INDEX_EPROP + "' is not present.");
