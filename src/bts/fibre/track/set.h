        ...
        class Track::Set: public Base::Set<Track> {
            ...
            public:
                ...
                Set(const std::map<std::string, std::string>& extended_props = Properties())
                        : Fibre::Base::Set<Track>(std::vector<const char*>(),
                 ...
                 * @param extended_props The extended properties of the set
                 */
                Set(size_t size, size_t num_points,
                    const std::map<std::string, std::string>& extended_props = Properties())
                        : Fibre::Base::Set<Track>(size, num_points, num_points * 3,
                 ...
                 * @param extended_props The extended properties of the set
                 */
                Set(size_t size, size_t num_points, const std::vector<const char*>& props,
                    const std::vector<const char*>& elem_props,
                    const std::map<std::string, std::string>& extended_props = Properties())
                        : Fibre::Base::Set<Track>(size, num_points,
                                num_points * 3 + elem_props.size(), props, elem_props,
                                extended_props) {
                }
...
//          : Base::Set<Track>(num_tcks, default_value, properties) {}
                ...
                Set(std::string file_location, size_t num_points = 0, double strands_per_width =
                        -1.0)
                        : Fibre::Base::Set<Track>(std::vector<const char*>(),
                ...
                void load(const std::string& path, size_t num_points = 0, double strands_per_width =
                        -1.0);
                ...
                void save(const std::string& path, size_t degree = 0,
                          size_t num_tractlets = 0) const;

                Set& operator+=(const Set& set) {
                    Base::Set<Track>::operator+=(set);
                    return *this;
                }
