...
namespace BTS {
    ...
    :  Base::Object (degree, (size_t)3, select_props<Strand>(*t.props)) {
      ...
      const MR::Math::Matrix<double>& inverse_position_matrix  = Strand::inverse_position_matrix(t.num_points(), effective_degree);
    ...
    :  Base::Object (degree, (size_t)3, select_props<Strand>(*t.props)) {
      ...
      MR::Math::Matrix<double> inverse_position_matrix  = Strand::create_inverse_position_matrix(timeline, effective_degree);
    ...
    Track                                 Strand::to_tangents(size_t num_points, bool include_endpoints) const {
      ...
      const MR::Math::Matrix<double>& position_matrix  = Strand::tangent_matrix(num_points, degree());
    ...
    Strand                         	      Strand::outer_product(const MR::Math::Vector<double>& column_vector, const Coord& row) {
    ...
    const MR::Math::Matrix<double>&       Strand::position_matrix(size_t num_sections, size_t degree, bool include_endpoints) {
      ...
      if (include_endpoints) {
        ...
        MR::Math::Matrix<double>& position_matrix = Strand::position_matrices_w_endpoints[num_sections][degree];

        if (position_matrix.rows())
         return position_matrix;
        else
         return position_matrix = Strand::create_position_matrix(degree, num_sections, INCLUDE_ENDPOINTS);
        //TODO: Change dont_include_endpoints to enum.

      } else {

        MR::Math::Matrix<double>& position_matrix = Strand::position_matrices[num_sections][degree];

        if (position_matrix.rows())
         return position_matrix;
        else
         return position_matrix = Strand::create_position_matrix(degree, num_sections, DONT_INCLUDE_ENDPOINTS);
    ...
    const MR::Math::Matrix<double>&       Strand::tangent_matrix(size_t num_sections, size_t degree) {
       ...
       MR::Math::Matrix<double>& tangent_matrix = Strand::tangent_matrices[num_sections][degree];

       if (tangent_matrix.rows())
         return tangent_matrix;
       else
         return tangent_matrix = Strand::create_tangent_matrix(degree, num_sections, DONT_INCLUDE_ENDPOINTS);
    ...
    //!Generates, stores and returns position conversion matrices when required.
    const MR::Math::Matrix<double>&       Strand::inverse_position_matrix(size_t num_sections, size_t degree) {

      assert(num_sections != 0);
      assert(degree != 0);

      MR::Math::Matrix<double>& position_matrix = Strand::inverse_position_matrices[degree][num_sections];

      if (position_matrix.rows())
       return position_matrix;
      else
       return position_matrix = Strand::create_inverse_position_matrix(num_sections, degree, DONT_INCLUDE_ENDPOINTS);

    }


    //!Generates, stores and returns position conversion matrices when required.
    const MR::Math::Matrix<double>&       Strand::position_matrix_w_endpoints(size_t num_sections, size_t degree) {
   ...
   //Generates, stores and returns position conversion matrices_w_endpoints when required.
    const MR::Math::Matrix<double>&       Strand::tangent_matrix_w_endpoints(size_t num_sections, size_t degree) {

      assert(num_sections != 0);
      assert(degree != 0);

      MR::Math::Matrix<double>& tangent_matrix = Strand::tangent_matrices_w_endpoints[num_sections][degree];

      if (tangent_matrix.rows())
        return tangent_matrix;
      else
        return tangent_matrix = Strand::create_tangent_matrix(degree, num_sections, INCLUDE_ENDPOINTS);
    }


    //Generates, stores and returns position conversion matrices_w_endpoints when required.
    const MR::Math::Matrix<double>&       Strand::inverse_position_matrix_w_endpoints(size_t num_sections, size_t degree) {

      assert(num_sections != 0);
      assert(degree != 0);

      MR::Math::Matrix<double>& position_matrix = Strand::inverse_position_matrices_w_endpoints[degree][num_sections];

      if (position_matrix.rows())
       return position_matrix;
      else
       return position_matrix = Strand::create_inverse_position_matrix(num_sections, degree, INCLUDE_ENDPOINTS);

    }


    void                                  Strand::add_section_gradient(const Strand& strand, const Section& section, const BasicSection& gradient) {

      this->operator+=(outer_product(section.position_coeffs, gradient.position()));
      this->operator+= (outer_product(section.tangent_coeffs, gradient.tangent() * section.length_fraction));
    ...
    std::vector<Strand::Section>&         Strand::sections(std::vector<Strand::Section>& sections, size_t num_length_sections, size_t num_width_sections, const Triple<double>& vox_lengths, const Triple<double>& offsets, size_t num_encodings) const {
      ...
      for (size_t section_i = 0; section_i < num_length_sections; section_i++) {
        sections[section_i].set(*this, position_matrix.row(section_i), tangent_matrix.row(section_i), length_fraction);
    ...
    MR::Math::Matrix<double>               Strand::create_position_matrix(size_t degree, size_t num_sections, bool include_endpoints) {

      if (!degree)
        throw Exception("The degree (" + str(degree) + ") cannot be 0.");

      if (!num_sections)
        throw Exception("The size of the number of length samples (" + str(num_sections) + ") cannot be 0.");
    ...
    MR::Math::Matrix<double>               Strand::create_position_matrix(size_t degree, const std::vector<double>& timeline) {

      if (degree == 0 || timeline.size() == 0)
        throw Exception("The degree (" + str(degree) + ") or the size of the timeline vector (" + str(timeline.size()) + ") cannot be 0.");
    ...
    MR::Math::Matrix<double>               Strand::create_inverse_position_matrix(size_t num_sections, size_t degree, bool include_endpoints) {

      MR::Math::Matrix<double> Psi = Strand::create_position_matrix(degree, num_sections, include_endpoints);
    ...
    MR::Math::Matrix<double>               Strand::create_inverse_position_matrix(const std::vector<double>& timeline, size_t degree) {
    ...
    MR::Math::Matrix<double>               Strand::create_tangent_matrix(size_t degree, size_t num_sections, bool include_endpoints) {
    ...
    std::pair< MR::Math::Vector<double>, MR::Math::Vector<double> >   Strand::endpoint_coeffs() const {
      ...
      std::pair< MR::Math::Vector<double>, MR::Math::Vector<double> > endpoints_gradient (MR::Math::Vector<double>(this->degree()),MR::Math::Vector<double>(this->degree()));
    ...
    Strand                                Strand::merge(std::vector<const Strand*>& strands, size_t num_points, size_t new_degree) {
      ...
      for (size_t time_i = 0; time_i < timeline.size(); time_i++)
        timeline[time_i] = (MR::Math::cos((timeline[time_i] - min_cos_time) * (M_PI / cos_time_range)) + 1) * 0.5;
