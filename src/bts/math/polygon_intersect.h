...
real inter(point * a, int na, point * b, int nb) {
    ...
    void range(point * x, int c) {
        ...
        while(c--) {
            ...
            void bd(real * X, real y) {
                *X = *X<y ? *X:y;
            }
            void bu(real * X, real y) {
                *X = *X>y ? *X:y;
            }
    ...
    
    {
        ...
        real rngx = B.max.x - B.min.x, sclx = gamut / rngx, rngy = B.max.y - B.min.y, scly = gamut
                / rngy;
        {
    ...
    {
        hp
        area(ipoint a, ipoint p, ipoint q)
        ...
        for (j = 0; j < na; ++j)
            for (k = 0; k < nb; ++k)
                if (ovl(ipa[j].rx, ipb[k].rx) && ovl(ipa[j].ry, ipb[k].ry)) {
                    hp a1 = -area(ipa[j].ip, ipb[k].ip, ipb[k + 1].ip), a2 = area(ipa[j + 1].ip,
                            ipb[k].ip, ipb[k + 1].ip);
                    {
                        int o = a1 < 0;
                        if (o == a2 < 0) {
                            hp a3 = area(ipb[k].ip, ipa[j].ip, ipa[j + 1].ip), a4 = -area(
                                    ipb[k + 1].ip, ipa[j].ip, ipa[j + 1].ip);
                            if (a3 < 0 == a4 < 0) {
                                void cross(vertex * a, vertex * b, vertex * c, vertex * d,
                                    ...
                                    ++a->in; --c->in;}
                                if (o)
                                    cross(&ipa[j], &ipa[j + 1], &ipb[k], &ipb[k + 1], a1, a2, a3,
                                            a4);
                                else
                                    cross(&ipb[k], &ipb[k + 1], &ipa[j], &ipa[j + 1], a3, a4, a1,
                                            a2);
                            }
                        }
                    }
...
int main() {
    ...
    void tst (point * a, int na, point * b, int nb) {
        printf("%11.7f, %11.7f\n", inter(a, na, b, nb), inter(a, na, a, na));
    }

    {
        point a[] = { { 2, 3 }, { 2, 3 }, { 2, 3 }, { 2, 4 }, { 3, 3 }, { 2, 3 }, { 2, 3 } }, b[] =
                { { 1, 1 }, { 1, 4 }, { 4, 4 }, { 4, 1 }, { 1, 1 } };
        tst(a, 7, b, 5);
    }    // 1/2, 1/2
...
// It is not necessary to duplicate the first vertex at the end.
    {
        point a[] =
                { { 1, 7 }, { 4, 7 }, { 4, 6 }, { 2, 6 }, { 2, 3 }, { 4, 3 }, { 4, 2 }, { 1, 2 } },
                b[] = { { 3, 1 }, { 5, 1 }, { 5, 4 }, { 3, 4 }, { 3, 5 }, { 6, 5 }, { 6, 0 },
                        { 3, 0 } };
        tst(a, 8, b, 8);
    }    // 0, 9
    {
        point a[] = { { 1, 1 }, { 1, 2 }, { 2, 1 }, { 2, 2 } }, b[] = { { 0, 0 }, { 0, 4 },
                                                                        { 4, 4 }, { 4, 0 } };
        tst(a, 4, b, 4);
    }    // 0, 1/2
    {
    ...
    {
        point a[] = { { 0, 0 }, { 1, 0 }, { 0, 1 } }, b[] =
                { { 0, 0 }, { 0, 1 }, { 1, 1 }, { 1, 0 } };
        tst(a, 3, b, 4);
    }    // -1/2, 1/2
    {
        point a[] = { { 1, 3 }, { 2, 3 }, { 2, 0 }, { 1, 0 } }, b[] = { { 0, 1 }, { 3, 1 },
                                                                        { 3, 2 }, { 0, 2 } };
        tst(a, 4, b, 4);
    }    // -1, 3
    {
        point a[] = { { 0, 0 }, { 0, 2 }, { 2, 2 }, { 2, 0 } }, b[] = { { 1, 1 }, { 3, 1 },
                                                                        { 3, 3 }, { 1, 3 } };
        tst(a, 4, b, 4);
    }    // -1, 4
    {
        point a[] = { { 0, 0 }, { 0, 4 }, { 4, 4 }, { 4, 0 } }, b[] = { { 1, 1 }, { 1, 2 },
                                                                        { 2, 2 }, { 2, 1 } };
        tst(a, 4, b, 4);
    }    // 1, 16
    return 0;
