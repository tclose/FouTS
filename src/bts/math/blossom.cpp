...
namespace BTS {
	...
	namespace Math {
      ...
      std::vector<Edge>     match_pairs(const std::vector<Edge>& edges,
                                        bool check_match,
        ...
        if (!edges.size())
          throw Exception ("Edges vector passed to 'Math::Blossom::match_pairs' is empty.");
        ...
        if (nodes.size() & 1)
          throw Exception ("Odd number of nodes (" + str(edges.size()) + "), run BTS::Math::Blossom::if_odd_remove_worst() first to remove worst match (in a greedy sense).");


        size_t mapped_node = 0;
        std::map<size_t,size_t> mapped_nodes;

        for (std::set<size_t>::iterator node_it = nodes.begin(); node_it != nodes.end(); ++node_it) {
        ...
        for (size_t edge_i = 0; edge_i < edges.size(); edge_i++) {
          weights_array[edge_i] = (edges[edge_i].weight - min_weight) * WEIGHTS_RANGE / range;
        }
        ...
        for (size_t edge_i = 0; edge_i < edges.size(); ++edge_i)
          f << edges_array[edge_i*2] << " " << edges_array[edge_i*2+1] << " " << weights_array[edge_i] << std::endl;


        PerfectMatching *pm = new PerfectMatching(nodes.size(), edges.size());
        for (size_t edge_i = 0; edge_i < edges.size(); ++edge_i)
          pm->AddEdge(edges_array[edge_i*2], edges_array[edge_i*2+1], weights_array[edge_i]);
        ...
        if (check_match) {
          ...
          int error = CheckPerfectMatchingOptimality(nodes.size(), edges.size(), edges_array, weights_array, pm);

          if (error)
            throw Exception ("Blossom pair matching failed (error code " + str(error) + ").");
      ...
      std::vector<Edge>   make_even(const std::vector<Edge>& edges) {
        ...
        if (num_nodes & 1) {
          ...
          for (std::map<size_t,double>::iterator min_weight_it = node_min_weights.begin(); min_weight_it != node_min_weights.end(); ++min_weight_it)
            sorted_min_weights.push_back(std::pair<size_t,double>(min_weight_it->first, min_weight_it->second));

          std::sort(sorted_min_weights.begin(), sorted_min_weights.end(), CompareSecond<size_t,double>());

          for (std::vector< std::pair<size_t,double> >::reverse_iterator mw_it = sorted_min_weights.rbegin(); mw_it != sorted_min_weights.rend(); mw_it++) {

            size_t node = mw_it->first;

            for (std::vector<Edge>::iterator edge_it = even_edges.begin(); edge_it != even_edges.end();)
          ...
          if (!even_found)
            throw Exception ("An even number of nodes could not be found using the 'make_even' algorithm. Suggest manual removal.");
