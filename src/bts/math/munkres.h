    ...
    namespace Math {
        ...
        template<typename T> class Munkres {
            ...
            public:
                ...
                Munkres(size_t nrows, size_t ncolumns)
                        : nrows(nrows), ncolumns(ncolumns), Costs(nrows, ncolumns), assigned_col(
                                  nrows), assigned_row(ncolumns), proposed_col(nrows) {
                    
                    if (ncolumns < nrows)
                        throw Exception(
                                "Number of columns (" + str(ncolumns)
                                + ") must exceed number of rows (" + str(nrows) + ").");
                    
                    for (size_t row_i = 0; row_i < nrows; row_i++)
                        all_rows.insert(row_i);
                    for (size_t col_i = 0; col_i < ncolumns; col_i++)
                        all_cols.insert(col_i);
            ...
            public:
                ...
                friend std::ostream& operator<<(std::ostream& stream, const Munkres<T>& H) {
                    ...
                    stream << std::endl << "----------------" << std::endl << "Cost: " << std::endl
                    << H.Costs << std::endl;
                    
                    stream << "Assigned cols: ";
                    for (std::vector<int>::const_iterator it = H.assigned_col.begin();
                            it != H.assigned_col.end(); it++)
                        stream << *it << ", ";
                    stream << std::endl;
                    
                    stream << "Assigned rows: ";
                    for (std::vector<int>::const_iterator it = H.assigned_row.begin();
                            it != H.assigned_row.end(); it++)
                        stream << *it << ", ";
                    stream << std::endl;
                    
                    stream << "proposed cols: ";
                    for (std::vector<int>::const_iterator it = H.proposed_col.begin();
                            it != H.proposed_col.end(); it++)
                        stream << *it << ", ";
                    stream << std::endl;
                    
                    stream << "Open rows: ";
                    for (std::set<size_t>::const_iterator row_it = H.open_rows.begin();
                            row_it != H.open_rows.end(); row_it++)
                        stream << *row_it << ", ";
                    stream << std::endl;
                    
                    std::cout << "Open columns: ";
                    for (std::set<size_t>::const_iterator col_it = H.open_cols.begin();
                            col_it != H.open_cols.end(); col_it++)
                        stream << *col_it << ", ";
                    stream << std::endl;
        ...
        template<typename T> void Munkres<T>::match(MR::Math::Matrix<T>& Costs,
                                                    std::vector<size_t>& matched_indices) {
            
            if ((Costs.rows() != nrows) || (Costs.columns() != ncolumns)
                || (matched_indices.size() != nrows))
                throw Exception(
                        "Size of cost matrix " + str(Costs.rows()) + "x" + str(Costs.columns())
                        + " or matched_indices vector " + str(matched_indices.size())
                        + " does not match Munkres object " + str(nrows) + "x" + str(ncolumns)
                        + ".");
        ...
        template<typename T> void Munkres<T>::initial_assignment() {
            ...
            for (std::set<size_t>::iterator row_it = next_row; row_it != open_rows.end(); row_it =
                    next_row) {
                
                ++next_row;
                
                for (std::set<size_t>::iterator col_it = open_cols.begin();
                        col_it != open_cols.end(); col_it++) {
...
// no open zeros left.
        ...
        template<typename T> Index Munkres<T>::new_match() {
            ...
            while (true) {
                ...
                for (std::set<size_t>::iterator row_it = next_row; row_it != open_rows.end();
                        row_it = next_row) {
...
// (i.e. minimal assignment) with this change.
        ...
        template<typename T> T Munkres<T>::min_open_value() {
            ...
            for (std::set<size_t>::iterator row_it = open_rows.begin(); row_it != open_rows.end();
                    row_it++) {
...
// matching problem)
        ...
        template<typename T> void Munkres<T>::reassign_matches(Index new_match) {
            ...
            for (std::vector<Index>::iterator it = unassign_indices.begin();
                    it != unassign_indices.end(); it++)
                unassign(it->row, it->column);
            
            for (std::vector<Index>::iterator it = assign_indices.begin();
                    it != assign_indices.end(); it++)
