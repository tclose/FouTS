...
namespace BTS {
	...
	namespace MCMC {
      ...
      const std::string Walker::STEP_LOCATION_DEFAULT = "/home/tclose/data/mcmc/parameters/walker.sta";
      ...
      void                          Walker::step(const MR::Math::Vector<double>& current, MR::Math::Vector<double>& proposed, double scalar) const {

        assert(current.size() == proposed.size());

        for (size_t elem_i = 0; elem_i < current.size(); elem_i++)
          proposed[elem_i] = prop_distr->sample(current[elem_i], step_sizes[elem_i] * scalar);
