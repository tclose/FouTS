	...
	namespace MCMC {
      ...
      class Walker {
        ...
        public:
          ...
          Walker(Distribution* const prop_distr = 0) : prop_distr(prop_distr) {}
          ...
          Walker(const Walker& m)
            : step_sizes(m.step_sizes) { if (m.prop_distr) prop_distr = m.prop_distr->clone(); }


          Walker&                     operator=(const Walker& m)
            { step_sizes = m.step_sizes; if (m.prop_distr) prop_distr = m.prop_distr->clone(); return *this; }


          void                        step(const MR::Math::Vector<double>& current, MR::Math::Vector<double>& proposed, double scalar = 1.0) const;
