    ...
    namespace MCMC {
        ...
        namespace Proposal {
            ...
            Momentum::Weighted::Weighted(Distribution* const proposal_distribution,
                    ...
                    : Momentum(proposal_distribution, relative_step_sizes), momen_step(
                              relative_step_sizes.size()) {
            }
            
            //!The kinetic energy is the weighted inner product of the momentum, normalized by 1/2 * log(2*pi)^D |weights|.
            double Momentum::Weighted::log_kinetic_energy(
                    const MR::Math::Matrix<double>& weights_chol) const {
...
//        MR::Math::mult(momen, CblasNoTrans, CblasLower, cholesky, random_vec);
            ...
            void Momentum::Weighted::update_state(MR::Math::Vector<double>& state,
            ...
            double Momentum::Weighted::predicted_change(
                    const MR::Math::Vector<double>& gradient,
                    const MR::Math::Matrix<double>& weights_chol, double time_direction) const {
                ...
                for (size_t elem_i = 0; elem_i < size(); elem_i++)
                    pred_change += time_direction * working[elem_i] * gradient[elem_i]
                                   * step[elem_i];
