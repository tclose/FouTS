	...
	namespace MCMC {
      ...
      class Momentum::Weighted::NonSeparable : public Momentum::Weighted {
        ...
        public:
          ...
          template <typename T> static NonSeparable  factory(const T& state,
                                                      double step_scale,
        ...
        public:
          ...
          NonSeparable(Distribution* const prop_distr = 0) : Weighted(prop_distr) {}

          NonSeparable(Distribution* const proposal_distribution, const MR::Math::Vector<double>& step_sizes, size_t num_newton_steps)
            : Weighted(proposal_distribution, step_sizes),
              tmp_momen(step_sizes.size()),
              working1(step_sizes.size()),
              working2(step_sizes.size()),
              Winv_dWdx(step_sizes.size(), MR::Math::Matrix<double>(step_sizes.size(),step_sizes.size())),
              trace_Winv_dWdx(step_sizes.size()) {}
...
//          double                                  log_kinetic_energy(const MR::Math::Matrix<double>& weights_chol) const;
          ...
          template <typename T> void              half_update_momentum(const MR::Math::Vector<double>& gradient, const MR::Math::Matrix<double>& weights, std::vector<typename T::Tensor>& weights_gradient, double time_direction, size_t num_newton_steps = 1);


          double                                  predicted_change(const MR::Math::Vector<double>& gradient, const MR::Math::Matrix<double>& fisher_chol, double time_direction) const;


          template <typename T> double            predicted_change(const T& gradient, const MR::Math::Matrix<double>& fisher_chol, double time_direction) const
            { return predicted_change(gradient, fisher_chol, time_direction); }

      };


      template <typename T> void Momentum::Weighted::NonSeparable::half_update_momentum(const MR::Math::Vector<double>& gradient,
        ...
        for (size_t elem_i = 0; elem_i < size(); ++elem_i) {
          ...
          for (size_t col_i = 0; col_i < size(); ++col_i) {
            ...
            MR::Math::Cholesky::solve(Winv_dWdx_col, weights_chol, weights_gradient[elem_i].column(col_i));
          }
        ...
        for (size_t newton_i = 0; newton_i < num_newton_steps; ++newton_i) {
          ...
          for (size_t elem_i = 0; elem_i < size(); ++elem_i) {
            ...
            tmp_momen[elem_i] = momen[elem_i] + 0.5 * time_direction * step[elem_i] * (gradient[elem_i] - trace_Winv_dWdx[elem_i] +  working2[elem_i]);
