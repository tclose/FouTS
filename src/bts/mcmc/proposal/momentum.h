    ...
    namespace Proposal {
      ...
      class Momentum {
        ...
        public:
          ...
          template <typename T> static Momentum      factory( const T& state,
                                                              double step_scale,
        ...
        public:
          ...
          Momentum(Distribution* const prop_distr = 0) : prop_distr(prop_distr) {}
          ...
          ~Momentum()
            { if (prop_distr) delete prop_distr; }

          Momentum(const Momentum& m)
            : momen(m.momen), step(m.step) { if (m.prop_distr) delete prop_distr; prop_distr = m.prop_distr->clone(); }

          Momentum&                               operator=(const Momentum& m)
            { momen = m.momen, step = m.step; if (m.prop_distr) delete prop_distr; prop_distr = m.prop_distr->clone(); return *this; }


          void                                    set(Distribution* const proposal_distribution, MR::Math::Vector<double> relative_step);
          ...
          virtual void                            half_update_momentum(const MR::Math::Vector<double>& gradient, double time_direction = 1.0);


          virtual void                            update_state(MR::Math::Vector<double>& state, double time_direction = 1.0) const;


          template <typename T> void              half_update_momentum(const T& gradient, double time_direction = 1.0)
            { half_update_momentum(gradient, time_direction); }


          template <typename T> void              update_state(T& state, double time_direction = 1.0) const
            { MR::Math::Vector<double>& state_vector = state; update_state(state_vector, time_direction); }


          void              half_update_momentum(const MCMC::State& gradient, double time_direction = 1.0)
            { half_update_momentum((const MR::Math::Vector<double>&)gradient, time_direction); }
          ...
          double                                  predicted_change(const MR::Math::Vector<double>& gradient, double time_direction = 1.0) const;

          template <typename T> double            predicted_change(const T& gradient, double time_direction = 1.0) const
