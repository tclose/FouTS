...
namespace BTS {
  ...
  namespace Analysis {
    ...
    template <typename T> void scan (
                                Prob::Likelihood& likelihood,
                                Prob::Prior& prior,
                                const T& origin,
                                const T& axis,
                                size_t num_steps,
  ...
  namespace Analysis {
    ...
    template <typename T> void scan (
      ...
      if (origin.size() != axis.size())
        throw Exception ("Size of origin state (" + str(origin.size()) + ") does not match size of axis state (" + str(axis.size()) + ").");

      std::string gradient_location = File::strip_extension(output_location) + ".gradient." + File::extension(output_location);
    ...
    template <typename T> void scan (
      ...
      for (typename Fibre::Base::Set<T>::const_iterator it = sequence.begin(); it != sequence.end(); ++it) {
    ...
    template <typename T> void scan (
      ...
      if (origin.size() != axis1.size())
        throw Exception ("Size of origin state (" + str(origin.size()) + ") does not match size of axis1 state (" + str(axis1.size()) + ").");

      if (origin.size() != axis2.size())
        throw Exception ("Size of origin state (" + str(origin.size()) + ") does not match size of axis2 state (" + str(axis2.size()) + ").");
    ...
    template <typename T> void scan (
      ...
      if (origin.size() != axis3.size())
        throw Exception ("Size of origin state (" + str(origin.size()) + ") does not match size of axis3 state (" + str(axis3.size()) + ").");
