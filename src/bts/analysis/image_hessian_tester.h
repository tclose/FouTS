/*
 ...
 FouTS is free software: you can reimageibute it and/or modify
 it under the terms of the GNU General Public License as published by
 the Free Software Foundation, either version 3 of the License, or
 (at your option) any later version.

 FouTS is imageibuted in the hope that it will be useful,
        ...
        template<typename ImageClass, typename State> class ImageHessianTester {
            ...
            public:
                ...
                typedef ImageClass& (ImageClass::*Function)(
                        const State&, Image::Container::Buffer<State>&,
                        Image::Container::Buffer<typename State::Tensor>&);    //, typename Image::Container::Buffer<typename State::Tensor>&
            ...
            public:
                ...
                void test(Function function, State& state, double step_size,
                          Image::Container::Buffer<typename State::Tensor>& analytic_hessian,
                          Image::Container::Buffer<typename State::Tensor>& numeric_hessian);
...
namespace FTS {
    ...
    namespace Analysis {
        ...
        template<typename ImageClass, typename State> void ImageHessianTester<ImageClass, State>::test(
            ...
            Image::Container::Buffer<State> gradient(dims, num_encodings), perturbed_gradient(dims,
                    num_encodings);
            ...
            Image::Container::Buffer<MR::Math::Matrix<double> > numeric_hessian_matrices(dims,
                    num_encodings);
            ...
            for (size_t row_i = 0; row_i < state_vector.size(); ++row_i) {
                ...
                for (typename Image::Container::Buffer<State>::iterator vox_it =
                        perturbed_gradient.begin(); vox_it != perturbed_gradient.end(); ++vox_it)
                    for (size_t encode_i = 0; encode_i < num_encodings; encode_i++)
                        numeric_hessian_matrices(vox_it->first)[encode_i].row(row_i) =
                                ((vox_it->second[encode_i] - gradient(vox_it->first)[encode_i]) / step_size);
                ...
                for (size_t y = 0; y < image.dim(Y); y++)
                    for (size_t x = 0; x < image.dim(X); x++) {
                        ...
                        for (size_t encode_i = 0; encode_i < num_encodings; encode_i++) {
                            ...
                            numeric_hessian(x, y, z)[encode_i] =
                                    numeric_hessian_matrices(x, y, z)[encode_i];
                        }
                        
                        if (analytic_hessian.is_empty(x, y, z))
                            for (size_t encode_i = 0; encode_i < image.num_encodings(); encode_i++)
                                analytic_hessian(x, y, z)[encode_i] = typename State::Tensor(
                                        state_tensor).zero();
