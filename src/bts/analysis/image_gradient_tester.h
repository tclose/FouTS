...
namespace BTS {
  ...
  namespace Analysis {
    ...
    public:
      ...
      typedef ImageClass& (ImageClass::*Function)(const State&, typename Image::Container::Buffer<State>&);
    ...
    public:
      ...
      void                                    test(Function function, State& state, double step_size, Image::Container::Buffer<State>& analytic_gradient, Image::Container::Buffer<State>& numeric_gradient);
...
namespace BTS {
  ...
  namespace Analysis {
    ...
    template <typename ImageClass, typename State>
        void     ImageGradientTester<ImageClass, State>::test(Function function,
                                                                State& state,
                                                                double step_size,
      ...
      Image::Container::Buffer< MR::Math::Vector<double> > numeric_gradient_vectors (image.dims(), image.num_encodings());
        ...
        for (size_t y = 0; y < image.dim(Y); y++)
          for (size_t x = 0; x < image.dim(X); x++)
            for (size_t encode_i = 0; encode_i < image.num_encodings(); encode_i++)
              numeric_gradient_vectors(x,y,z)[encode_i].resize(state_vector.size(), 0.0);
      ...
      for (size_t elem_i = 0; elem_i < state_vector.size(); ++elem_i) {
        ...
        for (Image::Buffer::iterator vox_it = image_intensities.begin(); vox_it != image_intensities.end(); ++vox_it)
          for (size_t encode_i = 0; encode_i < image.num_encodings(); encode_i++)
            if (!perturbed_image_intensities.is_empty(vox_it->first))
              numeric_gradient_vectors(vox_it->first)[encode_i][elem_i] = (perturbed_image_intensities(vox_it->first)[encode_i] - vox_it->second[encode_i]) / step_size;
        ...
        for (size_t y = 0; y < image.dim(Y); y++)
          for (size_t x = 0; x < image.dim(X); x++) {
            ...
            for (size_t encode_i = 0; encode_i < image.num_encodings(); encode_i++) {
              ...
              numeric_gradient(x,y,z)[encode_i] = numeric_gradient_vectors(x,y,z)[encode_i];
            }
