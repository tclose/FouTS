...
namespace FTS {
    ...
    template<typename T> class Triple {
        ...
        public:
            ...
            void copy_to(double* array) {
                array[X] = (double) p[X];
                array[Y] = (double) p[Y];
                array[Z] = (double) p[Z];
            }
            void copy_to(size_t* array) {
                array[X] = (size_t) p[X];
                array[Y] = (size_t) p[Y];
                array[Z] = (size_t) p[Z];
            }
            
            Triple<T>& set(T x_value, T y_value, T z_value) {
                p[X] = x_value;
                p[Y] = y_value;
                p[Z] = z_value;
                return *this;
            }
            Triple<T>& set(const T point[3]) {
                set(point[X], point[Y], point[Z]);
                return *this;
            }
            Triple<T>& set(T value) {
            ...
            Triple<T> abs() const {
                return Triple<T>(fabs(p[X]), fabs(p[Y]), fabs(p[Z]));
            }
            
            Triple<T> invert() const {
                return Triple<T>(1 / p[X], 1 / p[Y], 1 / p[Z]);
            }
            ...
            Triple<T> operator+(T M) const {
                return (Triple<T>(p[X] + M, p[Y] + M, p[Z] + M));
            }
            Triple<T> operator-(T M) const {
                return (Triple<T>(p[X] - M, p[Y] - M, p[Z] - M));
            }
            ...
            Triple<T> operator+(const Triple<T>& A) const {
                return Triple<T>(p[X] + A[X], p[Y] + A[Y], p[Z] + A[Z]);
            }
            Triple<T> operator-(const Triple<T>& A) const {
                return Triple<T>(p[X] - A[X], p[Y] - A[Y], p[Z] - A[Z]);
            }
...
//      template <typename U> Triple<T>         operator/ (U M) const;
            ...
            template<typename U> bool upper_bounded(U bound) const {
                return (p[X] <= (T) bound) && (p[Y] <= (T) bound) && (p[Z] <= (T) bound);
            }
            ...
            template<typename U> bool upper_bounded(const Triple<U>& bound) const {
                return (p[X] <= (T) bound[X]) && (p[Y] <= (T) bound[Y]) && (p[Z] <= (T) bound[Z]);
            }
            template<typename U> bool lower_bounded(const Triple<U>& bound) const {
                return (p[X] >= (T) bound[X]) && (p[Y] >= (T) bound[Y]) && (p[Z] >= (T) bound[Z]);
            }
            
            T dot(const Triple<T>& A) const {
            ...
            const Triple<T>& normalise() {
                MR::Math::normalise(p);
                return (*this);
            }
            
            Triple<T> cross(const Triple<T>& A) const {
                return (Triple<T>(p[Y] * A[Z] - p[Z] * A[Y], p[Z] * A[X] - p[X] * A[Z],
                        p[X] * A[Y] - p[Y] * A[X]));
            }
            
            Triple<T> min_axis() const {
                Triple<T> axis(0.0, 0.0, 0.0);
        ...
        protected:
            ...
            explicit Triple<T>(const T point[3]) {
                set(point);
            }
...
#include "bts/common.h"
    ...
    template<> inline Triple<int>& Triple<int>::invalidate() {
        p[X] = p[Y] = p[Z] = std::numeric_limits<int>::min();
        return *this;
    }
    
    template<> inline Triple<size_t>& Triple<size_t>::invalidate() {
        p[X] = p[Y] = p[Z] = std::numeric_limits<size_t>::max();
        return *this;
    }
    ...
    inline Triple<double> operator+(const Triple<int>& P, double M) {
        return Triple<double>(((double) P[X]) + M, ((double) P[Y]) + M, ((double) P[Z]) + M);
    }
    inline Triple<double> operator+(const Triple<size_t>& P, double M) {
        return Triple<double>(((double) P[X]) + M, ((double) P[Y]) + M, ((double) P[Z]) + M);
    }
    inline Triple<int> operator+(const Triple<size_t>& P, int M) {
        return Triple<int>(((int) P[X]) + M, ((int) P[Y]) + M, ((int) P[Z]) + M);
    }
    inline Triple<int> operator+(int M, const Triple<size_t>& P) {
        return Triple<int>(((int) P[X]) + M, ((int) P[Y]) + M, ((int) P[Z]) + M);
    }
    
    inline Triple<double> operator+(double M, const Triple<int>& P) {
        return Triple<double>(((double) P[X]) + M, ((double) P[Y]) + M, ((double) P[Z]) + M);
    }
    inline Triple<double> operator+(double M, const Triple<size_t>& P) {
        return Triple<double>(((double) P[X]) + M, ((double) P[Y]) + M, ((double) P[Z]) + M);
    }
    
    inline Triple<double> operator+(int M, const Triple<double>& P) {
        return Triple<double>(P[X] + (double) M, P[Y] + (double) M, P[Z] + (double) M);
    }
    
    inline Triple<double> operator+(const Triple<double>& f, const Triple<int>& i) {
        return Triple<double>(f[X] + (double) i[X], f[Y] + (double) i[Y], f[Z] + (double) i[Z]);
    }
    inline Triple<double> operator+(const Triple<double>& f, const Triple<size_t>& ui) {
        return Triple<double>(f[X] + (double) ui[X], f[Y] + (double) ui[Y], f[Z] + (double) ui[Z]);
    }
    
    inline Triple<double> operator+(const Triple<int>& i, const Triple<double>& f) {
        return Triple<double>(f[X] + (double) i[X], f[Y] + (double) i[Y], f[Z] + (double) i[Z]);
    }
    inline Triple<double> operator+(const Triple<size_t>& ui, const Triple<double>& f) {
        return Triple<double>(f[X] + (double) ui[X], f[Y] + (double) ui[Y], f[Z] + (double) ui[Z]);
    }
...
//  template <typename T> template <typename U> Triple<T>         Triple<T>::operator/ (U M) const  { return (Triple<T> (p[X]/M, p[Y]/M, p[Z]/M)); }
    ...
    template<typename T> inline T dist2(const Triple<T>& a, const Triple<T>& b) {
        return ((a - b).norm2());
    }
    template<typename T> inline T dist(const Triple<T>& a, const Triple<T>& b) {
        return ((a - b).norm());
    }
    ...
    template<typename T> Triple<T> parse_triple(const std::string& s) {
        ...
        while (start_number != std::string::npos && (dim_i < 3)) {
            ...
            if (num_string.find_first_of('-', 1) != std::string::npos || num_string.find('.',
                                                                                 num_string.find(
                                                                                         '.')
                                                                                 + 1)
                                                                         != std::string::npos)    // If there is a '-' character anywhere other than the first character, or if there is more than one decimal triple then throw an error.
                throw Exception(
                        "Coordinate, " + num_string + " at dim: " + str(dim_i + 1)
                        + ", is invalid");    //
        ...
        if (dim_i != 3)
            throw Exception(
                    "Incorrect number of values in sequence '" + s + "' for conversion to triple.");
