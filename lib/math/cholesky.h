...
namespace MR
{
  namespace Math
  {
    namespace Cholesky
    {
      ...
      //! returns the determinant of \a A using %Cholesky decomposition
      template <typename T> inline double     det_from_decomp(const Matrix<T>& D) {

        assert(D.rows() == D.columns());

        double det = 1.0;

        for (uint elem_i = 0; elem_i < D.rows(); ++elem_i)
          det *= D(elem_i, elem_i);

        det = MR::Math::pow2(det);
