...
%   End arguments %
  ...
  elseif strcmp(extension, 'tnr')
    
    base_extension = file_extension(base);
    base_base = file_base(base);
    
    [analytic, labels] = load_unzip_tensor([base_base '.analytic.' base_extension '.' extension]);
    numeric = load_unzip_tensor([base_base '.numeric.' base_extension '.' extension]);    
